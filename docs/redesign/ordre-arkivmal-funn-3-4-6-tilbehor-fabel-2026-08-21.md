...
> **🟢 STATUS 2026-08-22:** **Funn #3** levert (branch `fix/arkivmal-funn-3-4`, commit `b75b880c` — delt `beregnUtsnittVindu`, detalj = oversikt croppet til vindu; påvirker arkiv-PDF + sjekkliste-PDF/mobil). **Funn #4** levert i samme branch: hull 1 (radkort skalar-kommentar via delt `byggMerknad`, drawing_position byte-identisk) + hull 2 = **F7-D1** (folded inn — «Registrert utenfor rader»-blokk, kommentar + vedlegg). Alle fire kommentar-nivåer enhetstestet. F7s skjermbevis-DoD er ÅPEN (Kenneth kjører — se F7-ordren). **Funn #6** levert (branch `feat/funn6-tilbehor-fjerning`): delt `tilbehorVisning`-predikat (web + mobil `RapportObjektRenderer.tsx`) brukt på begge FeltDokumentasjon-monteringssteder (`FeltWrapper` + `RepeaterObjekt`). Deny-list PER felttype (A-lesning, Kenneth-bekreftet): `date`/`date_time`/`drawing_position`/`location` ren fjerning (0 prod-data); `repeater` objektnivå-tilbehør read-only KUN når data finnes (has-data-gate — mobil FeltDokumentasjon self-hider ikke). `text_field`-barn i repeater-rad beholder tilbehør. **Print-veien (F7) urørt.** Måling meldt: mobil `drawing_position` viser fornuftig placeholder etter fjerning (ikke tomt skall); mobil paritetsavvik H8 (kan ikke sette posisjon på repeater-rad) står som egen sak. Enhetstestet (`tilbehorVisning.test.ts`).
