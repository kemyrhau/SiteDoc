...
- **Tilpassbare rollenavn:** Hvert rolle-steg kan ha egendefinert `label` (f.eks. "Spørsmål" i stedet for "Registrator"). Lagres i `Dokumentflyt.roller` JSONB-felt — array av `{ rolle, label? }` (se `schema.prisma:1192`; struktur i [dokumentflyt.md § 7](dokumentflyt.md))
