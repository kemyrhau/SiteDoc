...
**H1 — Bestiller er ikke en stasjon i dagens maskin (kjernefunnet).** Skissens Flytboks 2 er et sted dokumentet STÅR («her bestiller man et arbeid utført», med send/send tilbake/avvis). I koden har bestiller ingen stasjons-statuser: received/in_progress er i praksis utfører-stasjonen (besvar/send/avvis bor der), responded er godkjenner-stasjonen. Bestillers defaults er avsender-handlinger (send kladd, trekk tilbake) + lukking. Når registrator sender, finnes det ingen status som betyr «hos bestiller». Dette er trolig grunnen til at de siste hullene ikke lar seg lukke: **statusene koder ikke posisjon på linjen.** **MÅLT + KORRIGERT (cowork 2026-07-26):** `DokumentflytMedlem` **HAR et `steg`-felt** (`steg Int @default(1)`) med unique-constraints på `(dokumentflytId, faggruppe/medlem/gruppe, rolle, steg)` — så **ordnet posisjon finnes i medlemslaget**. Holderen spores i tillegg via `DocumentTransfer.recipientUserId/recipientGroupId + senderRolle`. MEN statusmaskinen (`VALID_TRANSITIONS`) + matrisen (`FlytRettighetOverride @@unique[rolle, fraStatus, tilStatus]`) bruker **verken `steg` eller posisjon** — kun rolle × status. Konklusjon: posisjon-stillaset finnes (`steg`), men rettighets-/status-koordinatsystemet ignorerer det → per-boks-konfig umulig; 4-boks kjøres som 4 roller på ~3 status-stasjoner. **N-boks er NÆRMERE enn full ombygging:** `steg` er der; det som mangler er (a) status frikoblet fra posisjon og (b) matrise/rettigheter nøklet på `steg`/stasjonstype × retning. **Validerings-regel (Kenneth-vedtak): registrator kun på `steg = 1`** (flere personer ok = flere opprettere; ingen registrator på steg > 1). Mangler håndheving i dag.
...
**H3 — Videresend-defaults motsier vedtak 3 (dok↔kode-avvik) — SKJERPET 2026-07-26 (Kenneth-vedtak).** Vedtak 3 (2026-07-22): videresend flyttes fra utfører/godkjenner til administrator «via matrise-defaults». Koden i dag: `forwarded` ligger fortsatt i utførers (received, in_progress) og godkjenners (responded) default-sets. Målt skjerpelse: mottakerlisten (`byggVideresendValg`, videresend-valg.ts) bygges av ALLE faggrupper i prosjektet med mal-matchende flyt — uten filter på avsenderens egne medlemskap. Dagens default er dermed en rettighetslekkasje: enhver utfører/godkjenner kan flytte dokumenter inn i flyter de ikke har noen rolle i. **Vedtatt (Kenneth 2026-07-26):** (a) default AV for alle flyt-roller, PÅ kun prosjektadmin; (b) når et firma slår videresend PÅ for en flytrolle via matrisen, begrenses mottakerlisten til flyter avsenderen selv er medlem av — kun admin beholder full prosjektliste. Skisse-avviket (videresend under Godkjenner) er dermed avgjort: AV som default.
...
> **H6-REVISJON (Kenneth-vedtak 2026-08-21) — «Lukk» som slette-port.** Ref: [`docs/redesign/svar-flytmodell-lukk-slett-fabel-2026-08-21.md`](../../redesign/svar-flytmodell-lukk-slett-fabel-2026-08-21.md). Den opprinnelige H6-omleggingen (2026-07-26) fjernet `approved→closed` HELT («Godkjent lukkes ALDRI»), fordi Lukk sto som en KONKURRERENDE terminal ved siden av Godkjent. Vedtaket gjeninnfører Lukk — men med endret rolle: **Lukk er nå administrativ exit + eneste port til sletting**, ikke en konkurrerende terminal. Godkjent er fortsatt stoppsted i FLYTEN; Lukk tar dokumentet UT av flyten til `closed`, hvorfra det kan gjenåpnes eller slettes (slettevakt: `draft` || `closed`). **Eierskap endret mot H6-forslaget over:** Lukk er **KUN admin** (sitedoc_admin + prosjektadmin) — IKKE godkjenner, IKKE bestiller. Klient-gate: `posisjonHandlingTillatt` (`closed`→`false`) + `erAdmin`-snarvei. Server-gate: `verifiserRetningsrett` admin-bypass (`tilgangskontroll.ts:836/:848), IKKE `ROLLE_HANDLINGER_DEFAULTS` (måling 2026-08-21: rolletabellen konsumeres ikke av server-status-mutasjonen; den bruker `verifiserRetningsrett` + `isValidStatusTransition`). Gjenåpne uendret (registrator + prosjektadmin). `cancelled` retirert (0 prod-rader). Statusmaskin + univers + posisjon-gate + 12 nye vedtaks-tester: `packages/shared/src/utils/statusHandlinger.test.ts` + `index.ts`.
...
**REVERSERT (Kenneth 2026-07-26, kveld): N-boks er IKKE utsatt — det er dagens realitet.** Kenneth-presisering: flyter med 2/3/4/5 bokser KAN settes opp i dag (medlemsoppsettet begrenser ikke antall/kombinasjon) — da må systemet også støtte dem. Konsekvens: H1 rykker fra «måles først» til KJERNEKRAV, og F6 (received→approved) omtolkes fra 2-boks-patch til første symptom på det generelle kravet. Målet: rettigheter uttrykkes STASJONSRELATIVT — fram / tilbake / endepunkt / lokalt, relativt til boksen dokumentet står i — ikke som absolutte rolle×status-celler. Da er 2-boks og 5-boks samme regelverk. Dagens statusmaskin kan bestå som tilstandslag (kladd/sendt/mottatt/under arbeid/besvart/avvist/lukket) mens posisjon leses fra flytens stasjonsliste (steg-feltet i DokumentflytMedlem er en ubrukt kandidat — målt av cowork i § 6). Migrering og omfang må utredes som egen ordre FØR flere enkeltoverganger patches inn à la F6.
...
- Boksvisningen grupperer cellene per stasjon med **retningsspråk som bærende begrep (Kenneth 2026-07-26): Send høyre → (mot endepunktet) · ← Send venstre (tilbake, be om noe) · ■ Endepunkt (godkjenn/avvis/lukk) · lokale (besvar/slett)**. Retning er stasjonsrelativt og gyldig i 2-boks som i 5-boks — stasjonsnavn og statusnavn er det ikke. UI-utfordringen er å vise retning tydelig uten pil-salat: løsning i mockupen er at retningen ligger i GRUPPEOVERSKRIFTEN (ikke per knapp), boksene står fysisk på linjen slik at venstre/høyre er bokstavelig, og endepunkt-handlinger markeres som stopp (■), ikke bevegelse. Dokument-UI-et (utenfor matrisen) bør arve samme språk: «Send videre →» / «← Send tilbake» på handlingsknappene, med mottakerboksens navn som undertekst. **Presisering (Kenneth 2026-07-26, skjermbilde):** «Trekk tilbake» er IKKE en venstre-send — dokumentet står da i en boks lenger frem, og handlingen HENTER det hjem. Egen kategori i UI: **↩ Hent tilbake (dokumentet står lenger frem)**, skilt fra ← Send venstre (som flytter dokumentet fra der det STÅR). Tre retningskategorier totalt: send høyre (herfra, frem) · send venstre (herfra, tilbake) · hent tilbake (fjernvirkning, hjem hit). **Og (Kenneth, oppfølging): «Besvar» klassifiseres som venstre-send** — svaret går til den som ba om noe, ikke en lokal handling. Det generaliserer H2: HVER boks som ikke er venstre-endepunkt trenger en venstre-send (besvar/send tilbake/be om noe); i dag har kun utfører (besvar) og godkjenner (send tilbake) den, bestiller mangler den helt. NB modellspenning å avklare i posisjonsutredningen: i dagens maskin lander «besvart» hos GODKJENNER (høyre), mens begrepet sier svaret går VENSTRE — nok et symptom på at status ikke koder posisjon.
...
## 6. Addendum etter cowork nå-rapport (F0–F6, 2026-07-26)

Nå-rapporten endrer grunnlaget — evalueringen over var målt mot F1–F5. Delta:

- **H1 BEKREFTET som måling:** matrisen er rolle×status; `steg`-feltet finnes i DokumentflytMedlem men brukes ikke av status/matrise. Posisjons-koordinatproblemet består. Cowork flagger selv F6 (received→approved) som symptom-PATCH for 2-boks-tilfellet, ikke H1-løsning — riktig kategorisert.
- **H8 AVKLART:** test viste F0–F6-develop, ikke drift.
- **Tilgangsspørsmålet (2-boks) MÅLT: JA** — synlighet via flyt-medlemskap (any-rolle, tilgangskontroll.ts:1022), uavhengig av utfører-faggruppe. 2-boks-flyt reg→godkjenner virker nå (synlighet + F6-overgang).
- **NYTT beslutningspunkt for Kenneth — F6 vs H1:** received→approved lar godkjenner godkjenne uten at noe er besvart, i ALLE flyter (statusmaskinen er global). I en 4-boks-flyt betyr det at godkjenner kan hoppe over utfører. Bør enten (a) aksepteres som bevisst snarvei med matrise-default AV for godkjenner i standardflyt, eller (b) reverseres når posisjonsmodellen (H1) løses ordentlig. Ikke la patchen stå ukommentert.
- **NYTT — døde residualer:** received beholder in_progress + cancelled uten matrise-celle (cowork: «døde»). received→in_progress er lesekvitterings-autoen (restvedtak 2) — IKKE død; må ikke ryddes bort. received→cancelled er reelt død etter F2 → kandidat for fjerning i konsolideringsordren.
- **H2 og H4 (godkjenner-avvis) står:** ingen in_progress→(tilbake) og ingen responded→dismissed i gjeldende VALID_TRANSITIONS.
- **Fabel-kryssjekk:** fabel må måle mot F0–F6-develop (denne transition-tabellen), ellers kryssjekkes utdatert virkelighet.

## Neste steg (foreslått rekkefølge)
1. Kenneth: kryssjekk mot fabels evaluering → felles hull-liste. Fabel må få N-boks-kravet (§ 3-reverseringen) som premiss.
2. ~~Cowork: mål H1 + H8~~ MÅLT (§ 6). NY ordre i stedet: utredning av stasjonsrelativ posisjonsmodell (omfang, migrering, steg-feltet) — før flere enkeltoverganger patches.
