...
- **✅ Kloss 2 — adminNiva + matrise-UI (LANDET, merge `a3e2cc66`, develop, 2026-07-23, PR #3).** **Kenneth-vedtak underveis: kun sitedoc + prosjektadmin — firma-admin droppet som flyt-admin-nivå** (faller til vanlig flyt-rolle). `adminNiva: "sitedoc"|"prosjekt"|null` containet til flyt-funksjonene (boolean-shim `true=sitedoc` holder Kloss 1-testene bit-identiske). Prosjektadmin = egen redigerbar kolonne, default full innenfor statusmaskinen (isValid-arv, ikke «tom=nektet» — ville regressert). Matrise-admin-UI `dashbord/firma/flyt-rettigheter` (sitedoc-gatet), 5 celle-tilstander, `FlytRettighetLogg`-skriving, `kanRedigereFlytMatrise = sitedoc_admin only`. **Fase A-korreksjon:** ordrens «mobil prosjektadmin under-servert»-hypotese var feil (mobil matet av `minFlytInfo.erAdmin`, hadde alt paritet) — eneste reelle divergens var firma-admin-fantomet, nå fjernet. **Non-regresjon bevist:** mutasjonene validerer `isValidStatusTransition` uavhengig, så prosjektadmin-innstrammingen tar ingen reell rett.
- **🟡 Kloss 3 — endringslogg-fane + les/rediger-fane** (§ 2, vedtak 4+5): endringslogg-fanen + les/rediger-fanen (ren visning) ble levert i Kloss 2. I praksis dekket; egen runde kun ved behov for utvidelse.
