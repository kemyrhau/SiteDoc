...
- **✅ Kloss 1 — config-plumbing (LANDET, merge `33c32f1f`, develop).** Delta-substratet: tabellene, `ROLLE_HANDLINGER_DEFAULTS`, `celleTillatt()` m/ override-only-snitt, `overrides?`-param, loader trådd via `verifiserFlytRolle`. Bit-identisk bevist. **Rev. 7-revisjonen (orgId droppet, global) er LANDET via Kloss 2d.**
...
- **✅ Kloss 2b — firma-innstilling «auto-prosjektadmin» (LANDET, merge `cca3f471`).** Opt-in `OrganizationSetting.autoProsjektAdmin` enum; `autoLeggFirmaAdmins` i alle tre opprettelses-stier, dedup + no-op-safety. UBERØRT av 2d (per-firma medlemskap, ikke matrisens orgId).
- **✅ Kloss 2c — Admin-flate (§ 1c) + cellespec-kontrast (§ 2) (LANDET, merge `4d563c89`).** Matrisen på topbar Admin-shield + fem celle-tilstander (form+farge). Firma-dropdownen som 2c la til er FJERNET igjen av 2d (global matrise).
- **✅ Kloss 2d — global konfig (LANDET, merge `42b77e0c`).** `orgId` droppet fra `FlytRettighetOverride`/`Logg` (migrering `20260724130000`, TRUNCATE + drop); loader/tRPC/2c-UI orgId-frie; matrisen laster direkte uten firma-valg. Shared `statusHandlinger` + Kloss 2b urørt. Realiserer rev.7 § 0/§ 1.
