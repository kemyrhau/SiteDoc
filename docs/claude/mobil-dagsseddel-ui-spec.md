...
│    [+ Legg til timer-rad]                    │
│    ↳ herav maskin: Gravemaskin EQ-042 5,0 t  │  ← SUPERSEDERT (del 6 P1): nå INLINE m/«MASKIN»-merke
...
| **Timer-rad** | lønnsart · aktivitet · fra/til · timer · ECO + valgfri **fritekst** (produksjonsbeskrivelse). **Fra/til ↔ timer auto-synkes** (pause-bevisst): endrer du fra/til beregnes timer (spenn − lunsjpause); skriver du timer beregnes til-tid. Pausevindu skiftrelativt (`standardPauseEtterTimer`, default 4,0 t). Se [timer.md § Pause-bevisst tid-synk](timer.md). | ✅ (T.12 fritekst + auto-synk `f385ba99` deployet develop) |
| **Maskin-rad** | **⚠️ SUPERSEDERT 2026-07-13 (del 6 P1, `12826cbb`/`82d16d3f`):** maskin vises nå **INLINE** i rad-lista med slate «MASKIN»-merke — IKKE lenger nestet «↳ herav maskin». Ny maskin legges via valgfri maskin-seksjon i «+ Legg til timer»-dialogen (egen «+ Legg til maskin»-knapp fjernet; maskin.timer = timerradens antall). ~~Nestet under gruppen, «↳ herav maskin».~~ **Soft-skjul** når Equipment-cache tom. **Soft-varsel** ved manglende maskinførerbevis (ikke blokkér). **«+ Legg til maskin» synlig når cache er populert.** | 🟡 logikk deployet (T.11 + soft-skjul). **Device-funn #4:** «+ Legg til maskin» manglet på enhet — verifisert årsak: knappen finnes (`MaskinSeksjon.tsx:152/189`) men gates av `redigerbar && harEquipmentCache`; **tom Equipment-cache** ga ingen knapp, ikke manglende affordance. Re-test etter EQ-042-seed + re-login (cache-refresh) bekrefter. |
