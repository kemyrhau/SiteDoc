...
>
> **Timer-modulen Fase 3 — Runde 1A (katalog-admin) IMPLEMENTERT 2026-05-01 på `feature/timer-1a`:** ny `timer.*` tRPC-router i `apps/api/src/routes/timer/` (onboarding/lonnsart/aktivitet/tillegg) + `prismaTimer` lagt i tRPC-context. Seed-funksjoner fylt med faktisk innhold per timer.md: 16 lønnsarter Nivå 1 + 25 lønnsarter Nivå 2 + 3 aktiviteter + 3 tillegg + 5 utleggskategorier. Idempotente upserts (skip ved eksisterende seedNivaa-rader). Web-sider under `/dashbord/firma/timer/` (layout med sub-nav, onboarding, lonnsarter, aktiviteter, tillegg, page.tsx redirect). Sidebar-element «Timer» (Clock-ikon) i firma-layout, gates på `harTimerModul`. ~85 nye i18n-nøkler (nb+en) + 3 generiske (`ja`, `nei`, `handling.handlinger`). Onboarding-side med tre scenarioer (Nivå 1, Nivå 1+2, tom katalog). Aktivering setter `harTimerModul=true`. CRUD-tabeller med deaktiver-knapp (soft-delete via Restrict-FK). `pnpm build --filter @sitedoc/web` grønt — 5 timer-ruter kompilert. Klar for test-deploy.
...
| timer.md | 2026-05-01 | Beslutninger 2026-04-29: P1, P2 #3/#6/#7/#8/#13/#14. YAML-header tilføyd. Screening 2026-04-29 utført: 23 indekser tilføyd (Funn F.12), AnsattKompetanse-referanse tilføyd (F.7), YAML utvidet med A.28+C.14 (F.9). Verifikasjon 2026-05-01 mot kodebase: ny «Implementasjonsstatus per 2026-05-01»-seksjon øverst med komplett tabell (Forutsetninger fra Fase 0/0.5 ✅ | Timer-spesifikk DB+UI ❌ skal opprettes Fase 3 | Spec-status per tema ✅/🟡). Korrigert tidligere upresis blokker-merknad: eneste reelle blokker er **Proadm drømmescenario auto-avledning** (BACKLOG, ikke nødvendig for MVP) — godkjenningsflyt mot ansatt er fullt spec'd. **2026-05-01 (Infrastruktur-commit):** packages/db-timer/-pakken opprettet med 7 Runde-1-tabeller (lonnsarter, aktiviteter, tillegg, expense_categories, daily_sheets, sheet_timer, sheet_tillegg) i postgres-schema `timer`. Kjernen-migrasjon: `Organization.harTimerModul` + `OrganizationSetting` utvidet med dagsnorm/overtidsmatTerskel/tillattSelvAttestering/timerLockEtterDager. moduleGate-helper + seed-skjelett etablert. 4 senere tabeller (sheet_machines, sheet_materials, sheet_expenses, arbeidstidskalender) tas i Runde 2/3. **Tilleggsbeslutning 2026-05-01:** `timerLockEtterDager Int? null` (default ingen alders-grense; status styrer låsing) — Variant A vedtatt etter Kenneth-spørsmål. Klar for Runde 1A (katalog-admin) når Kenneth gir grønt lys |
| varsling.md | — | — |
