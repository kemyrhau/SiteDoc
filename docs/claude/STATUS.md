...
- 2026-09-04 — **`migrering-reporttemplate.md` § Status: PLANLAGT → DELVIS IMPLEMENTERT** (branch `feat/firma-malarkiv`, AM4 bolk 1): additiv migrering `20260904120000_malarkiv_hms_kolonner` (4 kolonner) + `firmamal.*` tRPC-ruter bygget (`apps/api/src/routes/firmamal.ts`). `sist_verifisert_mot_kode`/`sist_endret` → 2026-09-04. § Status utvidet med hva som er levert (bolk 1) og hva som gjenstår (bolk 2: UI + seeding). Ingen ny `docs/claude/`-fil; telling uendret (drifter allerede — se ⚠️).
