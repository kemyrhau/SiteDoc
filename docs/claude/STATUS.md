...
| fase-0-beslutninger.md | 2026-04-27 | **Sannhetskilde:** Anker for Fase 0-koding. K1 2026-04-28: C.13 + 2 § J-rader (QA-runde-2, tidl. § G). § E steg 8: B-2-note 2026-04-28. **2026-04-29:** A.25 OrganizationRole + A.26 Smart modulProcedure + A.27 Firma-HMS-lese-tilgang (Alt A) + A.28 Kompetansematrise (Kompetansetype + AnsattKompetanse, SmartDok-verifisert) tilføyd. § E utvidet med steg 14 (OrganizationRole). C.14 Fase 0.5-rad for kompetanse. A.29 Offline sync-konflikt-strategi (conflict-modal + 60s grace-periode) tilføyd, A.23 utvidet med peker til A.29. **2026-04-30:** A.30 byggeplassId-NULL-semantikk vedtatt (A1: NULL = «gjelder hele prosjektet», allerede implementert i kode). C.16 Vareforbruk-modul (forbruksregistrering per prosjekt, ikke lagerstyring — SmartDok-verifisert) — Fase 3-implementasjon. A.25 Tildelings-policy klargjort (company_admin tildeler OrganizationRole via verifiserFirmaAdmin) + A.27 peker + § E steg 14 utvidet med endepunkts-spec. B.7 Fase 0 minimum-implementasjon (velg første aktive User-rad, org-velger-UI utsettes). § F Fase 0.7 Data-rens registrert (prod-forberedelse — opprett A.Markussen, slett standalone-brukere før § E steg 13b). Eksisterende § F/G/H renames til § I/J/K. **2026-05-01: § E-koding startet på `feature/fase-0-fundament`.** E.1 Activity-tabell (commit `13a746a7`), E.2 OrganizationSetting (`4a155c28`), E.3 ProjectOrganization-rename + rolle-felt (`1bff8672`), E.4 Project.primaryOrganizationId nullable + bakfyll fra ProjectOrganization rolle='hovedeier' (`137eed6f`), E.5 ProjectModule-utvidelse — organizationId + status 3-nivå (aktiv/arkivert/slettet per A.17) parallelt med `active` per A.18 to-stegs (drop active i steg 2 post-Fase 1), 11 kode-steder oppdatert til status-baserte sjekker (`d9bfafc4`), E.6 OrganizationPartner — firma-eid UE/byggherre/leverandør-bibliotek (variant B felt-omfang) + Faggruppe.partnerId nullable FK (`22a772b6`), E.7 OrganizationTemplate — firma-mal-bibliotek (full speiling av ReportTemplate + OrganizationTemplateObject) + ReportTemplate.organizationTemplateId/promotedToFirma (`7709ea32`), E.8 BibliotekMal-utvidelse — 4 nye felter (kategori default 'sjekkliste', domene default 'kvalitet', kobletTilModul nullable, verifisert default false) lukker P-KRITISK-3 hardkodings-drift; bibliotek.ts:106-107 leser nå feltene i stedet for hardkoding. Ny åpen § C.17 registrert: BibliotekMal/OrganizationTemplate språkstøtte (besluttes Fase 2 — Alt A schema-utvidelse vs Alt B dynamisk OPUS-MT). E.10 ProjectMember.periodeSlutt 🔄. Timer/Maskin-revurdering utsatt til etter Fase 0 |
