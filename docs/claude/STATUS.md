...
| infrastruktur.md | — | — |
| kontrollplan.md | — | — |
| maskin.md | 2026-05-01 | Krever justering for fase-0 (per CLAUDE.md). K2.4+K2.5 2026-04-28: jsonb-format + service-varsel-trigger. **2026-05-01 (Blokk A docs):** to nye seksjoner — § Ansvarlig per utstyr (hybrid-modell: single primær + optional m:n for tilleggsansvarlige, UI-mønster med (+)-knapp, tilgangsregler, A.Markussen-mapping) + § Cross-modul-integrasjon (Domain Service-lag i `apps/api/src/services/maskin/`, forbidden direkte `prismaMaskin`-import utenfor maskin-mappen, soft-skjul ved deaktivert modul). **2026-05-01 (Blokk A schema, `de3fb1d0`):** Migrasjon applied — EquipmentAnsvarlig + 15 nye Equipment-felt + 4 nye indekser. **2026-05-01 (Blokk B Vegvesen-integrasjon):** service-lag (vegvesen-api/vegvesen/vegvesen-worker/moduleGate) + 3 nye tRPC-endepunkter (hentFraVegvesenForhandsvisning synkron mutation, opprettMedVegvesen Variant A med kjennemerke-validering, oppdaterFraVegvesen admin-only kø-basert) + 60s polling-worker + UI-flyt med forhåndsvisning + fallback + 36 i18n-nøkler. normaliserRegnummer() i shared brukes i 3 lag (klient/Zod/server). Klar for test-deploy |
