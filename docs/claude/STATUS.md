...
| infrastruktur.md | — | — |
| kontrollplan.md | — | — |
| maskin.md | 2026-05-01 | Krever justering for fase-0 (per CLAUDE.md). K2.4+K2.5 2026-04-28: jsonb-format + service-varsel-trigger. **2026-05-01 (Blokk A docs):** to nye seksjoner — § Ansvarlig per utstyr (hybrid-modell: single primær + optional m:n for tilleggsansvarlige, UI-mønster med (+)-knapp, tilgangsregler, A.Markussen-mapping) + § Cross-modul-integrasjon (Domain Service-lag i `apps/api/src/services/maskin/`, forbidden direkte `prismaMaskin`-import utenfor maskin-mappen, soft-skjul ved deaktivert modul). Schema-migrasjon planlagt (Blokk A — separat commit) |
