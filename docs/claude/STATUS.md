...
- 2026-09-22 — **`simulator-opus-oppkobling.md` (+53) + `simulator-runbook.md` (+178) utvidet for Xcode 27** (branch `feat/simulator-xcode27-oppkobling`): Xcode 27 brøt simulator-oppkoblingen. Dokumentert med eksakte feilmeldinger — SimulatorKit-symlinken (kommando, hvorfor, at den endrer Xcode-bunten, hvordan den fjernes), oppstart fra null, dev-login, env-presedens, prod-sperre og 7 nye feilsøkingsrader. 🔴 **Mest sannsynlige gjentakelse:** en Xcode-oppdatering vil trolig fjerne symlinken uten å si fra, og feilen ser da ut som «idb virker ikke lenger» — står i feilsøkingstabellen. Symlinken STÅR på Kenneths maskin (Kenneth-gatet til testrunden er ferdig); Xcode-bunten er dermed modifisert mellom øktene. Utvidelse av eksisterende filer → telling uendret; ingen ny `docs/claude/`-fil.
- 2026-09-17 — **`DEPLOY-RUNBOK.md` § 7 Seeding mot test (NY seksjon) + `LOKALT-OPPSETT.md` drift rettet** (branch `docs/seederegler-runbok`): seede-veien mot `sitedoc_test` samlet ett sted (målt av mal-Opus 2026-09-17) — `scp`→`docker cp`→`psql` (steg 1 agent, 2–3 Kenneths TTY), `-d sitedoc_test`≠`-d sitedoc`, seed legger-til-aldri-sletter-ekte-hendelser, `.ts`-seeds går ikke på server-ny (ingen node/`tsx`; bor i `packages/db/prisma`+`packages/db/scripts`, kjøres lokal/tunnel), postgres binder `127.0.0.1` (`infrastruktur.md`), `SEED_CONFIRM_DB`-prodvakten (`seed-bibliotek.ts:51`). `LOKALT-OPPSETT.md:48` rettet: e2e-seeds «kjøres av Kenneth på server-ny» var utdatert → via SSH-tunnel + peker til RUNBOK § 7. Begge `sist_verifisert_mot_kode` → 2026-09-17. Status/telling uendret (fokusert seksjon/drift-fiks, ingen ny fil). *(Ordre kalte pakken `packages/db-seeds` — finnes ikke; rettet til `packages/db`.)*
- 2026-09-07 — **NY fil `na-rapport-reisetid-2026-09-07.md`** (måling for cowork, ingen branch/kode): grunnlag for Kenneths valg av kjøretidskilde til A.Markussens reisetid-krav (kjøretid > 30 min → reisetid-lønnsart). **Hovedfunn: regelen er allerede bygget ende-til-ende** — `Oppmotested.lat/lng` PÅKREVDE (`schema.prisma:2489`), `Byggeplass.latitude/longitude` nullable m/ prosjekt-fallback (`reisetidMatrise.ts:68`), `OrganizationSetting.reiseTerskelMin @default(30)` + `reiseOverTerskelType @default("reisetid")` (`schema.prisma:424-428`), OSRM-kjøretidsmatrise (`rute-service.ts` + `ReisetidMatrise`), mobil auto-gen med 30-min-terskel og reise-lønnsart (`StartSluttDagKort.tsx:498-513`). Ingen dedikert «Reisetid»-art; bokføres på `Reise/transport til prosjekter` (`seed/index.ts:119`) via `reiseLonnsartId`/navne-match. **Ikke målt (krever psql/DB-tilgang):** deploy-tilstand test/prod + fire live-tall (oppmøtesteder, byggeplass-koordinatdekning, lønnsart-rad for SITEDOC MYRHAUG, matrise-rader). Ren måling, ingen anbefaling.
- 2026-09-07 — **NY fil `DEPLOY-RUNBOK.md` (+1) + deploy-dedup av 6 filer** (branch `docs/deploy-dedup`): deploy-kommandoene lå i 12 filer i ulike varianter (rotårsak til fem feilkommandoer på to døgn). `DEPLOY-RUNBOK.md` er nå ENESTE kilde (test § 1 · prod § 2 · OTA § 3 · env-filer § 4 · rekkefølge § 5 · **ny § 6 Rollback**). Guttet kommandoblokkene i `deploy-detaljer.md`, `infrastruktur.md`, `docker/DOCKER-NOTES.md`, `eas-build-veileder.md` (kun § «Slik publiserer du») og `ny-server-veileder.md` (kun § 3 — konkurrerende runbok, funnet i målingen) → pekere; hver beholder sitt *hvorfor*. Flyttet til runboken: daemon-restart-berging/diagnose (§ 6), `eas update:rollback` (§ 6), verifiser-i-image-grep (§ 2). ufw-brannmur → `ny-server-veileder § 6`. Slettet: PM2 `ssh sitedoc`-blokk (Kenspill legacy) + legacy per-pakke `.env` (WSL-server). Målt avvik meldt: gamle blokker kalte `deploy-test.sh` uten påkrevd `<hash>` og migrerte bare 1 db-pakke — scriptene (fasit) migrerer alle fire. `CLAUDE.md` alt deduplisert av cowork (39 982 tegn). Rene docs, ingen kode.
- 2026-09-04 — **`migrering-reporttemplate.md` § Status: bolk 2 UI ført** (branch `feat/firma-malarkiv-ui`, AM4 steg 2-5): firma-arkivsiden + promotering/badges i MalBygger + ny-mal «Fra firmaarkivet» + L2-badges + firmamal-seeding i `modul.aktiver`. Tre nye `firmamal.*`-ruter (`kanPromotere`/`listeForProsjekt`/`oppdaterKopiFraHovedmal`), `mal.hentMedId` utvidet. Ingen ny `docs/claude/`-fil; § Status utvidet med steg 2-5 + L8-restansen (egen branch). Telling uendret (drifter — se ⚠️).
- 2026-09-04 — **`migrering-reporttemplate.md` § Status: PLANLAGT → DELVIS IMPLEMENTERT** (branch `feat/firma-malarkiv`, AM4 bolk 1): additiv migrering `20260904120000_malarkiv_hms_kolonner` (**5 kolonner** etter B4-vedtaket: 4 + `laant_fra_bibliotek_mal_id` FK SetNull→BibliotekMal, strukturert avstamning i stedet for description-fritekst) + `firmamal.*` tRPC-ruter bygget (`apps/api/src/routes/firmamal.ts`). `sist_verifisert_mot_kode`/`sist_endret` → 2026-09-04. § Status utvidet med hva som er levert (bolk 1) og hva som gjenstår (bolk 2: UI + seeding). Ingen ny `docs/claude/`-fil; telling uendret (drifter allerede — se ⚠️).
- 2026-08-27 — **`timer.md` § Prosjektfilter rad-nivå (bug-fiks)** (branch `fix/rapport-prosjektfilter-radniva`): prosjektfilteret lekket rader fra andre prosjekter inn i rapport+eksport (split-sedler) — rad-nivå-`projectId`-filter lagt på alle fire relasjoner i `firmaPeriodeRapport` + `detaljEksport`, kun når `input.prosjektId` satt (bevarer kryss-prosjekt-rader ved ingen filter). Ny seksjon på eksisterende fil (Konsolidering ≠ verifikasjon) → telling uendret; ingen ny `docs/claude/`-fil.
- 2026-08-27 — **`timer.md` § Fase 4 → Oppfølger** (branch `fix/eksport-ekstern-ansattnr-maskinmerke`): to ekstern-lekkasjer Kenneth fanget i gaten dokumentert — ansattnr (pseudonymiseringsnøkkel) + maskin-anomali-merker ut av eksterne dokumenter (ansattnavn beholdes); + «sist brukt»-markering i eksport-menyen (delt `useSistBrukteMal`); + demo-seed fikk fra/til-klokkeslett. Regresjonstest `timer-rapport.test.ts`. Ny seksjon på eksisterende fil (Konsolidering ≠ verifikasjon) → telling uendret; ingen ny `docs/claude/`-fil.
- 2026-08-27 — **`timer.md` § Fase 4 + `printmotor-faser` FASE 4 LEVERT** (branch `feat/eksport-fase4-byggherredokument`): printmotor fase 4 (byggherredokumentet) dokumentert — config v2 (fire akser `mottaker`/`gruppering`/`orientering`/`topptekst`, **ingen migrering** — JSONB), delt `grupperDetaljRader` i `@sitedoc/shared` (pakker `byggDetaljRader`), `mottaker=ekstern` fjerner status+ID strukturelt, `orientering=liggende` via ny valgfri `landscape`-param i pdf-render-containeren (🔴 delt med prod, eget deploy-steg), topptekst-fletting server-side, innebygde `Lønnsgrunnlag`/`Fakturagrunnlag` aktivert. `schema.prisma`-config-kommentar oppdatert til v2-form (ingen DDL). Faseplanen: fase 4-raden → ✅ LEVERT, `status`/`sist_verifisert_mot_kode` 2026-08-27. Ny seksjon på eksisterende filer (Konsolidering ≠ verifikasjon) → telling uendret; ingen ny `docs/claude/`-fil.
- 2026-08-27 — **`timer.md` § Fase 3 + `printmotor-faser` FASE 3 LEVERT** (branch `feat/eksport-fase3-lagrede-maler`): printmotor fase 3 (lagrede utskriftsmaler) dokumentert — ny `EksportOppsett`-tabell (migrering `20260827120000`, additiv, Kenneth-gatet), `timer.eksportOppsett`-router, modal-lagring + maler-velger. Faseplanen fikk **fase 4 (gruppering)** som ny rad + begrunnelse for hvorfor Lønnsgrunnlag/Fakturagrunnlag ikke ble bygd (Kenneth-vedtak 2026-08-27). `timer.md` linje 513 drift-rettet («grå gjeninkludering er fase 3» → egen sak). **+ To defektrettinger fra fase-2-output** dokumentert i § Fase 3: status-verdi-oversettelse (delt `STATUS_I18N`, Excel+PDF) + klokkeslett Fra/Til gjennom `byggDetaljRader`. `sist_verifisert_mot_kode` (printmotor-faser) uendret 2026-08-27. Ingen ny `docs/claude/`-fil; telling uendret.
- 2026-08-26 — **NY fil `pilot-triage-2026-08-26.md`** (branch `docs/pilot-triage`): alle 42 åpne 🔴 + MASTERPLAN § Rekkefølge verifisert mot develop-kode (6 parallelle agenter). Resultat: **1 blokkerer** (mobil-annotering PNG 3,4 MB), 10 skjemmer, 20 kan vente, **14 løst i augustarbeidet**, 3 ❓. BACKLOG oppdatert på stedet: 8 poster merket 🟢 LØST m/ commit-ref, «verifisert åpen»-linje m/ pilot-vurdering+størrelse på de øvrige, 3 ❓. Ny 🟡-post: CLAUDE.md 375 tegn fra 40k-taket. Ingen kode endret, ingen rekkefølge-anbefaling (cowork sekvenserer).
...
- 2026-08-15 — **`STATUS-AKTUELT.md` arkivmal Stage 4** (branch `feat/arkivmal-rendering`): de fire målingene ført (ekte api-test-render av `AGENT-TEST-0001`, 3/4 grønt) + `@page :first`-fiksen (margin-header bærer kun dokumentreferanse; CSS-regelen fjernet). Kode: `render-templates.ts` + `arkiv-css.ts`. Ingen ny `docs/claude/`-fil; telling uendret.
- 2026-08-12 — **NY fil `retningslinjer/deaktiver-monster.md`** (branch `feat/deaktiver-monster`): styrende UI-mønster for deaktiver/aktiver på katalog-flater — `Power`-ikon, `Tooltip` m/ konsekvenstekst (aldri `title=`), `InaktivBadge`, «Vis inaktive (N)», hjelpetekst; delte komponenter i `apps/web/src/components/deaktiver/`. Ny `expenseCategory.deaktiver`/`aktiver`. 4 timer-flater hevet. (Telling drifter allerede — se ⚠️ nedenfor; ny fil ikke inn-reconciliert i denne runden.)
- 2026-08-11 — **`api.md` § Lagringsstatistikk** (branch `feat/lagringsstatistikk`): ny `lagring`-router (SUM file_size per prosjekt/modell, cache 1t) dokumentert — `oversikt` (sitedoc-admin) + `firmaOversikt` (firma-admin), akse `primaryOrganizationId`, foreldreløse-post (fakturerbart ≠ faktisk diskbruk), og hvorfor `drawings.file_size` IKKE ble strammet (DWG-layout-skrivevei lager null). `sist_verifisert_mot_kode` uendret 2026-08-11. Ingen ny `docs/claude/`-fil; telling uendret.
- 2026-08-11 — **`timer.md` § Modelljustering** (branch `feat/utlegg-ordning-justering`): ny § dokumenterer `sats`→`lonnstillegg`-omdøping (homonym-fiks), `fakturert` tatt ut av valgbare ordninger (enum beholdt), nye `satsbasert`+`muligSkattepliktig`-markeringer på ExpenseCategory, U5 upsert-test, migrering `20260811130000`. CHECK-enum-linjer + tabellrader oppdatert `sats`→`lonnstillegg`. Gate 2 (`refusjonsKontonummer`) notert som ikke bygget. `sist_verifisert_mot_kode` uendret 2026-08-11. Ingen ny `docs/claude/`-fil; telling uendret.
- 2026-08-11 — **`timer.md` § U4 mobil (kamera-primær)** (branch `feat/utlegg-u4-mobil`): ny § dokumenterer offline utlegg-registrering på mobil (`UtleggSeksjon.tsx`), klient-stempling av `ordningVedFoering`+`foertVed` ved føring (server re-utleder aldri ved sync), `katalogForMobil`-offline-cache, `syncBatch`/`hentEndringerSiden`-utvidelse for utlegg + vedlegg. Tre navngitte oppfølgere (tillegg-`createdAt`=synk-tid-hull, utlegg-vedlegg sync-orden, fast prosjekt i modal). `sist_verifisert_mot_kode` uendret 2026-08-11. Ingen ny `docs/claude/`-fil; telling uendret.
- 2026-08-11 — **`timer.md` § U5 firma-admin overstyring-UI** (branch `feat/utlegg-u5-overstyring`): ny § dokumenterer `expenseCategory.settOrdning`/overstyring-CRUD + `firma/timer/utleggskategorier`-flate + prosjektadmin read-only + sats-hint-fiks. `sist_verifisert_mot_kode` 2026-08-10 → 2026-08-11. Ingen ny `docs/claude/`-fil; telling uendret.
...
- `historikk-2026-06.md`
- `historikk-2026-07.md`
- `historikk-docs.md` (arkiv, samme sjanger som prod-historikkene)
- `OPPSUMMERING-timer-arkitektur.md`
- `qa-rapport-2026-05-17.md`
- `funn-impersonering-og-prosjektoppsett.md`

(`STATUS-AKTUELT.md` telles — levende status med egen rad.)
...
| lokal-dev.md | 2026-07-15 | **Meta-fil:** Lokal dev-flate på Mac — koble til, feilsøke (`AccessDenied` = tom lokal DB, ikke OAuth), oppdatere test-data, teste nye funksjoner, hva localhost ikke dekker. **Empirisk verifisert mot maskinen** 2026-07-15 (`lsof -i :5432` → Homebrew pg16; restore kjørt → 31 brukere; login bekreftet på localhost:3100). Indeksert i DOC-MAP (4 rader) — CLAUDE.md >40k. Rad tilføyd 2026-07-15 |
