...
### 🔴 «Opprett firma» (sitedoc_admin) fungerer ikke

Fanget 2026-06-24 (web-admin-testing på prod, Kenneth = `sitedoc_admin`). SiteDoc Admin → Firmaer → «Opprett firma»-modal (Org.nr valgfritt + Firmanavn, f.eks. «Sitedoc»). To symptomer: **(1a)** klikk «Opprett» → firma opprettes ikke (feilmodus — feilmelding/stille/konsoll — **ikke fanget ennå, kreves for diagnose**); **(1b)** «Hent fra Brønnøysund»-knappen (org.nr-oppslag) er **grået ut / fungerer ikke** — sannsynlig `disabled`-gate (venter på gyldig org.nr) eller ikke-wired Brønnøysund-proxy. Sannsynlig område: `admin.opprettFirma`/`organization.opprett`-mutasjon, `verifiserSiteDocAdmin`-gate, + Brønnøysund-oppslag-handler. Tre symptomer i samme admin-flyt → trolig en feature som trenger egen gjennomgang, ikke en rask exit-patch. Blokkerer ikke #32-prereqs (A.Markussen finnes), men hindrer onboarding av nye firma. Relatert: #2 «kan ikke opprette prosjekt uten eksisterende firma» er **IKKE bug** (firma-påkrevd, låst 2026-05-20, anti-orphan) — men #1 gjør at firmaet ikke kan lages først, så de oppleves sammen.
