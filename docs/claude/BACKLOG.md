...
**🟡 Maskin-velger i dagsseddel-modal — søk + kategori-filter + sortering:** Funn ved prod-verifisering 2026-07-04 (maskin-føring happy-path). Kjøretøy-/maskin-velgeren når man registrerer en maskin-rad på en dagsseddel er en flat liste uten søk, kategori-filter eller sortering. Konvensjons-luke mot [adaptiv-dropdown-regelen](../../CLAUDE.md) (lærende inputs → søkefelt ved >7 elementer) + [Filter-standard](../../CLAUDE.md) (`MultiComboks`/`SearchInput`). For firma med mange maskiner blir velgeren uhåndterlig. Fiks: fritekst-søk + kategori-filter (kjøretøy/anleggsmaskin/småutstyr) + fornuftig sortering (sist brukt / navn). Mobil-UI hovedfokus (feltarbeider), men gjelder web-dagsseddel også.

**🟡 Maskin ≤ arbeidstimer-avhengighet — gjør proaktiv (i dag reaktiv feil):** `validerMaskinUnderArbeid` håndhever `sum(maskin.timer) ≤ sum(timer.timer)` per (projectId, ECO)-bucket, men **reaktivt** — brukeren får en feilmelding først ved lagring/submit. Uintuitivt: man skjønner ikke hvorfor maskin-raden avvises uten å kjenne invarianten. Fiks-alternativer: (a) disable «legg til / lagre» med tooltip som forklarer gjenstående arbeidstime-kapasitet, eller (b) vis en inline arbeidstime-rad/-indikator i modalen så taket er synlig før man treffer feilen. Gjelder både mobil og web dagsseddel-UI. Funn ved prod-verifisering 2026-07-04.
