...
- **HMS-prefix-UX-felle — amber-hint i mal-modal** 🟡 — Når en mal har `prefix` som matcher reservert HMS-mønster (SJA, RUH, AVVIK — case-insensitiv) men HMS-haken er av, vises ingen advarsel. Brukeren tror «SJA-001»-malen er en HMS-mal fordi prefiksen leser slik, men dokumentene havner i Oppgaver-fanen (siden `domain="bygg"`) og telles ikke i HMS-KPI eller statistikk. Stille feilklassifisering. **Tiltak:** legg inn amber-hint i opprett- og rediger-modal som leser «Prefiksen ser ut som HMS — vil du krysse av HMS-haken?» når mønsteret matcher uten at hake er på. Ikke blokker — gi hint. Matcher dagens mønster (HMS-endrings-advarsel er allerede amber-tekst). Identifisert 2026-05-29 under prefix/domain-undersøkelse etter MalListe-fiks. Anslag: 1 fil, ~15 linjer.
- **Subdomain↔category-sammenheng-validering** 🟡 — HMS-fanene er hardkodet til å hente fra spesifikke tabeller per subdomain: avvik+ruh fra `task`, sja fra `checklist` (etter 2026-05-29-rydding). Hvis en bruker endrer `category` på en HMS-mal slik at den ikke matcher fanen — f.eks. `subdomain="avvik"` + `category="sjekkliste"` — opprettes dokumentet i feil tabell og forsvinner stille fra alle visninger (HMS-fanen ekskluderer fordi datakilden er feil tabell; Oppgaver/Sjekklister-fanen ekskluderer fordi `domain="hms"` filtreres bort der). **Tiltak:** server-side validering i `mal.opprett` og `mal.oppdaterMal` som krever konsistent `subdomain`↔`category`-kombinasjon. Mapping (vedtatt 2026-05-29): avvik+ruh → oppgave, sja → sjekkliste. Throw med klar feilmelding ved mismatch. Identifisert 2026-05-29 under RUH-bytte-arbeidet. Anslag: 1 fil (`apps/api/src/routes/mal.ts`), ~20 linjer + Zod-refinement.
