...
- **Firma-administrasjons-navigasjon strukturell rydding (~10-12t)** 🟡 — 3 lag: ~10 ruter trenger `organizationId` som input (lag 2, delvis dekket via Blokk C 2026-05-04 — `admin.hentAlleProsjekter`/`admin.hentAlleOrganisasjoner` har orgId-input; resterende firma-ruter må auditeres), ~10 sider må sende `useFirma().valgtFirma.id` (lag 2, delvis), rename av «Firmainnstillinger» → «Prosjekteier» ✅ ferdig (`f3b8bb1a` + `e7168b32`). Ikke-blokkerende. Audit per rute gjenstår — verifisert 2026-05-28.
...
- **Vis som bruker (impersonering)** 🟡 — backend-grunnmuren DEPLOYET via `ecdc60a4 refactor(trpc): delt lagContextStamme for api + web`: Context har `imperseringAktiv: boolean` + `actualUserId: string | null` slik at tRPC-prosedyrer kan skille mellom faktisk innlogget bruker og impersonert. UI-velger (sitedoc_admin → velg firma → velg bruker → bytt kontekst) gjenstår. Egen sesjon for UI-implementasjon.
