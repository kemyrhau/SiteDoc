...
- **iOS-simulator henger på evig spinner — IPv6/NordVPN (ROTÅRSAK FUNNET 2026-06-06)** 🟡 (dev-miljø) — Fetcher til api-test (og andre IPv6-hoster) henger ved `await fetch` i simulatoren mens server svarer 200 og curl/Mac virker. Rotårsak: ødelagt IPv6-ruting fra NordVPN-`utun`-rester; simulatorens happy-eyeballs staller på død IPv6-sti. **Ikke kode/server/Cloudflare/ngrok.** (Tidligere Cloudflare-WAF-hypotese avkreftet.) Full diagnose + løsninger (reboot / IPv6 Link-local only / `networksetup -setv6off Wi-Fi`) i [simulator-ipv6-nordvpn.md](simulator-ipv6-nordvpn.md).
- **AuthProvider `verifiser`-fetch mangler timeout/AbortController** 🟡 (kode-robusthet) — En hengende fetch (f.eks. ved død IPv6-sti, se over) gir evig spinner i stedet for å degradere til cachet bruker/innlogging. Legg AbortController-timeout (~10–15s) på verifiser-fetchen i `apps/mobile/src/providers/AuthProvider.tsx` (+ vurder dev-login + tRPC/React Query-timeout). Bør fikses uansett miljøårsak. Detaljer i [simulator-ipv6-nordvpn.md § Separat kode-robusthetsbug](simulator-ipv6-nordvpn.md).
