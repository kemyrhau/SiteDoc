...
- **Gjenbrukbar filter + sortering-komponent (dynamisk)** 🟡 — Kenneth 2026-07-24: én dynamisk/gjenbrukbar filter+sort-komponent for konsistent brukervennlighet + design på tvers av sider (ikke bespoke per side). **Fabel-designgjennomgang bestilt** (kartlegg eksisterende mønstre + foreslå samlende komponent + veileder, estimér størrelse). Grunnlag: sjekklister-listen (kolonnefiltre Prefix/Status/Tittelemne/Ansvarlig/Opprettet av/Tidsfrist/Flyt + «Velg parameter») · [Filter-standard `MultiComboks`+`SearchInput`](../../CLAUDE.md) (2026-05-29) · Toppbar-filtre-standard (2026-05-30). Mål: (1) én gjenbrukbar komponent (filter + sort), (2) **veileder** for hvordan filter/sort settes opp i SiteDoc, (3) første konsument = **endringslogg-fanen** i flyt-rettighetsmatrisen (celle-filter + hvem-søk + sortering, «spore én instans av gangen» — data finnes alt i `flytMatrise.hentLogg`, filtrering kan gjøres klient-side). Størrelse avgjør backlog vs. fremskyndes. Ikke blokker A-3b.
