...
- **🟡 Byggeplass/underprosjekt-timeregistrering (mobil).** I dag kan man kun «Legg til prosjekt» på en sedel — ikke byggeplass eller underprosjekt. Separat fra forenkling-økta; hører til [byggeplass-strategi.md](byggeplass-strategi.md)-fasen + registrerings-forenkling-økta. Ønsket:
  ...
  **Status (kode-review 2026-06-23 — se [timer-gps-prosjekt-utredning.md § 2026-06-23](timer-gps-prosjekt-utredning.md)):** = **Beslutning 6** i utredningen, fases:
  - **Sedel-nivå byggeplass (én/dag): nesten server-klart.** `dagsseddelLocal.byggeplassId` + mobil-sync + `syncBatch` (sedel-nivå, propagerer til rader) finnes allerede. Mangler kun: kopier `arbeidsdag.byggeplassId` → draft (`opprettDagsseddelForSegment:107`, hardkoder i dag `null`) + byggeplass-velger (punkt 1–2 over). Neste byggbare runde (B2+B6 sedel-nivå).
  - **Per-rad / «splitt dagen mellom byggeplasser» (punkt 3): oppfølger.** Krever server `syncBatch` rad-input + mobil rad-tabeller (`sheetTimerLocal`/`sheetMachineLocal.byggeplassId`) — `@@unique(userId, dato)` gjør sedel-nivå = én byggeplass/dag, så splitt-dagen krever per-rad-modell.
