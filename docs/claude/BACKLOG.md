...
- **🟡 UF-4 (recall) — UTSATT TIL EGEN SERVER-RUNDE.** Arbeider-recall av sendt (men ikke godkjent) sedel: «sent → draft». **Verifisert 2026-06-22 at modellen IKKE støtter dette ennå** — status-enum er `draft|sent|returned|accepted` (ingen `cancelled`) og det finnes **ingen recall/withdraw-mutasjon** (server eller mobil). Krever ny tRPC-mutasjon `gjenåpneDagsseddel` (`sent → draft` kun når null rader attestert; `accepted` blokkeres) + mobil-knapp. **Ingen DB-migrering** (gjenbruker eksisterende enum-verdier). Låser opp UF-1s `sent`-case (som inntil videre blokkerer append + viser «dagen er sendt»). Ikke mobil-only → egen runde.
