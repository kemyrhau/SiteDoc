...
- **Activity-logging — aktivere `activity_log` for tilstandsendringer** 🔴 — `activity_log`-skjema finnes (15 kolonner, payload jsonb, retention-felter, anonymisering) men **ingen kode skriver til tabellen**. Bekreftet 2026-05-28: 0 rader siste 24t, ingen spor av template-sletting eller andre hendelser. **Krav (juridisk/kontraktsmessig):** All tilstandsendring i dokumentflyt-domenet MÅ logges — status-overganger (`draft → sent → received → approved/rejected`), mottaker-endringer, kommentarer, vedlegg, underskrifter. Audit-trail er ikke valgfritt for dokumentflyt (kontraktsmessig dokumentasjon). **Implementasjonsrekkefølge:** (1) dokumentflyt-mutations FØRST — `dokumentflyt`-router + `tasks`/`checklists` med flyt-koblinger, (2) `report_templates` (template-sletting/-endring), (3) `project_members` + `groups` (tilgangs-endringer er audit-kritiske), (4) bredere CRUD via Prisma middleware som sikkerhetsnett. **Arkitektur-skisse (5 dager):** Hybrid Prisma middleware + eksplisitt tRPC-skriving, AsyncLocalStorage for userId-context, whitelist per modell, retention 18 mnd via PM2 cron. Erstatter T7-2b3 før/etter-snapshot-utvidelse (samme tabell, samme infrastruktur).
