...
| 22 | Dokumenter leser malstruktur LIVE — firmaarkiv-↻ foreldreløsgjør dokumentdata (målt 2026-09-16) | `firmamal.ts:818` `oppdaterKopiFraHovedmal` gjør `deleteMany`+recreate av objekt-treet med NYE id-er, KUN `verifiserAdmin`-gatet, ingen dokument-sjekk. **Direkte MalBygger-redigering er ALLEREDE vernet** (endringsvern `mal.ts:666-681,848-866`, 2026-09-07) — dette er det ene gjenstående hullet. Firma-/kapittelnivå trygt (ingen Checklist/Task peker på OrganizationTemplateObject) | 🔴 | Anbefalt: gi `oppdaterKopiFraHovedmal` samme `tellDokumenterMedInnhold`-guard som `slettObjekt` (lavest kost, ingen skjema). Full måling + 4 løsninger + read-only SQL: [funn21-strukturkopi-maaling.md](funn21-strukturkopi-maaling.md) + [funn21-omfang.sql](funn21-omfang.sql) | 🔴 **Kenneth velger løsning etter SQL-måling (D avgjør om det haster)** |
...
| Sak | Levert |
|---|---|
| **«X versjoner bak» + ↻ i prosjekt-mallista** | `MalListe.tsx` navn-celle: badge + ↻ som kaller `firmamal.oppdaterKopiFraHovedmal` (samme som MalBygger). Krevde `copiedFromOrgTemplate` i `mal.hentForProsjekt` (`mal.ts`). 🔴 **Advarsel `firmamal.ts:766-768`: full erstatning gjør dokumentdata foreldreløs — bevisst klikk, ingen bekreftelse (speiler MalBygger).** ✅ **STEG 1 (`fix/vern-oppdater-kopi`, dokgen funn #21): serveren NEKTER nå ↻ (`PRECONDITION_FAILED`) når prosjektmalens nåværende objekter har innhold i et aktivt dokument — samme telling/feilkode som `slettObjekt` (`tellDokumenterMedInnhold` eksportert fra `mal.ts`). Utveien er «Hent fra arkiv» (ny prosjektmal).** 🔴 **STEG 2 (egen runde, Kenneth-vedtak 17.09 «sperre nå, diff/merge senere»): ↻ må matche gammelt mot nytt objekt (etikett + type) og BEHOLDE objekt-id-en der feltet er det samme — først da betyr Kenneths regel «forsvant et felt med data?» noe (i dag «forsvinner» hvert felt teknisk fordi alle id-er byttes). Sperren fra steg 1 er MIDLERTIDIG og skal løftes når diff/merge finnes. Egen runde med dry-run — den flytter dokumentdata.** |
...
> **[triage 2026-08-26]** Pakke A splittet: **A1 DOMPurify 🟢 LØST** (`lib/sanitize.ts` wired i 4 filer) · **A2 @fastify/static 🟢 LØST** (lockfile 9.3.0 + root-lås `server.ts:129`) · **A3 Next-bump 🟢 LØST** (`^14.2.35`) · **A4 hardkodet Norkart-nøkkel ÅPEN** — Kan vente, noen runder (`GeoReferanseEditor.tsx:262`, roter+env-flytt) · **A5/A6 ftd-worker-pins 🟢 moot** (worker ikke deployet) · **A7 proxy-headers ❓** ikke verifiserbar (Cloudflare/proxy utenfor repo).
...
> **[triage 2026-08-26]** 🟢 LØST — fullført (ikke fjernet) på web+mobil m/ tester (`TegningPosisjonObjekt.tsx`); ikke lenger en tom felle.
...
> **[triage 2026-08-26]** verifisert åpen — Kan vente — egen fase: delvis (pkt 2/3/4/5/8 åpne, pkt 6 tegningsutsnitt-markør nå bygd). Byggherre-web.
...
> **[triage 2026-08-26]** verifisert åpen — Kan vente — noen runder: funn 1 løst, funn 5 moot (logg ikke i PDF); funn 6-rest (klokkeslett, `formaterDatoKort` kun dato) sann.
...
> **[triage 2026-08-26]** verifisert åpen — Kan vente — egen fase: pkt 2 avklart av D4-vedtak; pkt 4 + 5a/5b (samlerapport kompakt/utvidet) sanne — `byggArkivSamling` slår kun sammen fulle ark.
...
> **[triage 2026-08-26]** 🟢 LØST — arkivmalen rendrer `org.name`+org.nr alltid (`ramme.ts:41-49`); gammel `header.ts`-fra→til slettet 2026-08-24.
...
> **[triage 2026-08-26]** verifisert åpen — Skjemmer — noen runder: funn 2/3 åpne (`RapportObjektVisning.tsx:550` 3s-fallback, høyre utsnitt) + funn 4 (auto-åpne tegning) ikke bygget; funn 1 delvis. Befaring/print, ikke timeføring.
...
> **[triage 2026-08-26]** 🟢 LØST — `7dd4df8d` involutiv invers (`georeferanse.ts:265-282`), akkurat delplanens fiks; 3-punkts workaround unødvendig.
...
> **[triage 2026-08-26]** verifisert åpen — Kan vente — egen fase (rest): kobling bygget begge veier m/ test (`kontrollplanKobling.ts`, tiltak 1+2); kun fristvarsling mangler (ingen scheduler i api).
...
> **[triage 2026-08-26]** 🟢 LØST (i praksis) — `validerRegistratorForst` (`dokumentflyt.ts:36`, `c2e236f8`) kaster ved kilde + presis mal-velger-melding; sekundærfunn (person vs faggruppe) ikke adressert.
...
> **[triage 2026-08-26]** verifisert åpen — Skjemmer — egen fase: `RapportObjektVisning.tsx:42` `if (tom) return null`; lukkes først når `skriv-ut` går til arkiv-PDF (fase 3b). Byggherre-dokument.
...
> **[triage 2026-08-26]** verifisert åpen — Skjemmer — noen runder: `prosjektoppsett` fikk topp-knapp (`:261`) men ikke sticky; `beforeunload`-vakt = 0 filer; øvrige `oppsett/*` ukartlagt.
...
> **[triage 2026-08-26]** 🟢 LØST — testen grønn nå (3/3); fikset i statusmaskin-redesignet (`f48c8003`/`bdef517f`).
...
> **[triage 2026-08-26]** 🟢 LØST/foreldet — `in_progress` kollapset helt (`statusHandlinger.ts:20-22`); transisjonen finnes ikke lenger å måle.
...
> **[triage 2026-08-26]** verifisert åpen — Kan vente — egen fase: `useSistBrukteMal.ts:11` sier kilden er klient-localStorage (interim); `byttMal` kun psi/kontrollplan. Interim funker.
...
> **[triage 2026-08-26]** verifisert åpen — Skjemmer (mobil-kjerne) — egen fase: `oppgave/index.tsx:218` bruker `OpprettDokumentModal`; ekte ett-klikk→chips-skjerm ikke bygget. Pilotens få-klikk-kjerne.
...
> **[triage 2026-08-26]** verifisert åpen — Skjemmer — noen runder: `Trash2` importert uten bruk (`oppgave/[id].tsx:27`); ingen slett-kommentar-mutasjon → feilkommentar kan ikke fjernes på mobil.
...
> **[triage 2026-08-26]** verifisert åpen — Kan vente — noen runder: ingen kollaps-logikk for konsekutive statuspar (web+mobil); rent kosmetisk.
...
> **[triage 2026-08-26]** 🟢 LØST — erstattet av M2 autolagret-mikrotekst + `LagreIndikator` (`oppgave/[id].tsx:451-454/590`); ingen suksess-Alert igjen.
...
> **[triage 2026-08-26]** verifisert åpen — Skjemmer (mobil-nær) — egen fase: `Task` mangler `byggeplassId` (`schema.prisma:1105`), `oppgave.opprett` setter kun `drawingId` → byggeplass-uten-tegning droppes stille. Krever migrering + design.
...
> **[triage 2026-08-26]** verifisert åpen — Kan vente (dev, ikke pilot) — én runde: skriptet finnes ikke (`scripts/` har kun `.sql` + `smartdok/`).
...
> **[triage 2026-08-26]** verifisert åpen — Kan vente — egen fase: Activity-rad forbedret, men konvensjonen uendret (`:53/:73/:76` `(planlagt)`, seks former i én fil, ingen gate). Intern docs-hygiene.
...
> **[triage 2026-08-26]** verifisert åpen — Skjemmer — noen runder: 6 `DISPLAY_TYPER`-def + 4 `REDIGERBARE_STATUSER`/`erSynlig`-kopier, 0 i `@sitedoc/shared`; `oppgave.oppdaterData` uten append-only-vakt = latent korrekthetsrisiko (sjekkliste/oppgave-nær).
...
- **`DISPLAY_TYPER` — 6 definisjoner, 3 ulike innhold.** 4× `{heading, subtitle}` (hookene) · 1× `+location` (web-renderer `:29`) · 1× `+location, info_text, info_image` (mobil-renderer `:32`). Legger noen en ny visnings-type i en renderer, hopper ikke hookenes init/validering over den.
  - **MÅL 2026-08-23 (F2-rest-økta): unifisering til én kilde er IKKE billig — de tre innholdene er to ulike semantikker, ikke ren drift.** (a) **Hook-settet** `{heading, subtitle}` betyr «ikke-datafelt — hopp over i skjema-init/validering». (b) **Renderer-eksporten** (`+location`, +`info_text/info_image`) betyr «render UTEN FeltWrapper/FeltDokumentasjon-tilbehør». `location` er i (b) men ikke (a) fordi den holder en verdi (håndteres av `SKJULT_I_UTFYLLING`), mens `heading/subtitle` aldri gjør det. En naiv `new Set` delt av begge ville enten (i) la hookene initialisere `location` som datafelt, eller (ii) la renderen droppe FeltWrapper på felt som trenger den. **Kostnad for ekte unifisering:** splitt i TO delte Set i `@sitedoc/shared` (`STRUKTUR_TYPER` for hookene, `DISPLAY_TYPER` for renderne), importer i 6 sites (web+mobil × renderer/hooks) + 3 testmocker som i dag overstyrer til `new Set<string>()`. Middels: 1 shared-fil + 9 importbytter, ingen atferdsendring hvis settene bevares eksakt. **Ny divergens etter F2-rest:** web-renderer fikk `+video` (ren visning), mobil-renderer holder `video` ute (interaktiv «watched») — reell semantisk forskjell, ikke drift; en delt renderer-`DISPLAY_TYPER` må enten godta et per-plattform-tillegg eller harmonisere video-semantikken først.
...
> **[triage 2026-08-26]** verifisert åpen — Kan vente — én runde: funn 1 (størrelse) borte (39 625 B) men ingen gate; funn 5 står (`:111` «14 språk ~2500 nøkler», reelt 15/2909).
...
> **[triage 2026-08-26]** 🟢 LØST — ikke lenger UVERIFISERT: accepted-vakt `dagsseddel.ts:4510-4520` + TOCTOU-fiks `:4854-4878` (`updateMany` notIn accepted → conflict + rollback).
...
**Distinkt fra to naboer — ikke bland:** (a) blocker #1 (rader forsvinner på mobil) er et *visnings*-fenomen; dette er *server-tilstand*. (b) F4-2s `gjenaapneAttestering` gjør `accepted`→`sent` **bevisst**, som en leder-handling med precondition + auth. Dette er den samme overgangen uten vakt, utløst av en arbeider-push.

**Hvorfor avklare, ikke fikse ennå:** av-attestering er lønns-nært, så alvoret er høyt *hvis* stien er nåbar — men reachability er ikke bevist. Push-triggeren sender kun `syncStatus="pending"`-sedler, og M4 la en gjenåpne-vakt på arbeider-siden. Spørsmålet er om det finnes en sti der en lokalt-`pending` sedel eksisterer mens server er `accepted` (lokal redigering i vinduet mellom attestering og neste pull? stale enhet som pusher gammel tilstand?).

**Neste steg (ingen kode før avklart):**
1. Verifiser om `innkommendeStatus`-grenen faktisk kan nås — hvilken vakt står mellom, og dekker M4-vakten denne stien?
2. Er den nåbar: er `accepted`→`sent` her en **tilsiktet** «arbeider vinner»-regel, eller en glipp? Grenen er skrevet med vilje (`? "sent" :`), så intensjonen må avklares før fiks.
3. DB-sjekk på test: finnes sedler som har gått `accepted`→`sent` uten spor av `gjenaapneAttestering`?

**Proveniens:** develop-Opus exit-rapport 2026-07-15, punkt 4c. Cowork verifiserte at hverken `innkommendeStatus` eller linjereferansen finnes i BACKLOG/STATUS-AKTUELT/timer.md → funnet var ikke ført noe sted. Uten exit-runden hadde det forsvunnet med økta.

### 🟡 F5 matpause: genereringen kan gi Σ(rad.pauseMin) ≠ dagsseddel.pauseMin i én kant (2026-07-15)

Faller pausevinduet **utenfor alle arbeidsvinduer** (`pauseMin > 0`, men ingen lunsj-kryssende carve-rad å legge bæreren i), får ingen rad bæreren ved generering → Σ(rad.pauseMin) = 0 mens `dagsseddel.pauseMin` > 0. Invarianten i `skrivPauseTilstand` (`apps/mobile/src/services/matpause.ts`) holder ved hver **senere** endring; det er kun genererings-øyeblikket som kan divergere. **Maskin-kapasitetsregelen er upåvirket** — den leser sedel-nivå `pauseMin`. Nevnt i develop-Opus' F5-gate-rapport, aldri ført til docs (cowork verifiserte 0 treff 2026-07-15).
...
> **[triage 2026-08-26]** verifisert åpen — Kan vente — egen fase: prosess-/tooling-mangel, ikke kodefeil; `prosjektoppsett-veileder.md` = «Plan (ikke implementert)». Krever engangsoppsett + regel.
...
> **[triage 2026-08-26]** ❓ ikke verifiserbar — beskrevet mekanisme matcher ikke koden (`grupperMedOverskrift` grupperer på `heading`, zone-sort har trygg `undefined`-fallback), men eksplisitt guard + e2e-fixture mangler → krever device-repro på Release-bygg for å friskmelde.
...
> **[triage 2026-08-26]** verifisert åpen — Skjemmer — én runde: sammenslåing nå automatisk (`forsonSedelIdentitet` `timerSync.ts:242`), men API sender framtidig «dine timer slås sammen» (`dagsseddel.ts:5139`) → mikrotekst feil. ~50 mobilbrukere treffer den.
...
> **[triage 2026-08-26]** verifisert åpen — Kan vente — én runde: navngitt `innstillinger.lenke.timerOnboarding` finnes ikke (relikvi); nb/en korrekte, piloten norsk → påvirker ikke A.Markussen.
...
> **[triage 2026-08-26]** verifisert åpen — Kan vente — én runde: `/dashbord/[prosjektId]/dokumentleser/page.tsx` — eneste inbound = `sok-dekning.test.ts:74`-unntak, ellers ulenket → sletting-kandidat.
...
> **[triage 2026-08-26]** ❓ ikke verifiserbar — server-env (`web.env` vs `web-redesign.env`) + Entra/Google-registreringer, ikke i repo. Berører ikke mobil timeføring på `sitedoc.no`. Kan vente.
...
- **🟡 Rader forsvinner på mobil etter attestering — DIAGNOSE 2026-07-14 (develop-Opus, statisk mot develop): primærmistanke MOTBEVIST, narrowet til tombstone-edge, trolig lukket av S3.** Repro (2026-07-10, ren flyt): mobil registrerer rad → send → synk (rad bekreftet web) → leder attesterer web → mobil refresh → 0 rader / «Ingen timer-rader», header «X t ført», web har raden (fersk sedel). **Kjerne-deduksjon:** «web har raden» beviser at server IKKE er wipet → push-overskriving (opprinnelig mistanke) er MOTBEVIST av symptomet selv. Rent mobil-lokalt. **Sporing (alle ryddet, fil:linje):** (1) `syncBatch` full-erstatning allerede lukket av S3 2026-07-11 (`dagsseddel.ts:4096-4149`; `deleteMany` målrettet `id:{in:timerIder}` + gated `length>0` → tom push wiper ingenting); (2) mobil push-trigger pusher kun `syncStatus="pending"` (`timerSync.ts:300-307`) — synket sedel re-pushes ikke; (3) lokal rad-query har intet `attestertStatus`-filter (`[id].tsx:143-148`) — sedel finnes, rader genuint slettet ikke skjult; (4) `hentEndringerSiden` inkluderer attestert-rader (`dagsseddel.ts:3524,3539`, `not:"erstattet"`); (5) `attesterRader` sletter ingen rader (`dagsseddel.ts:2307-2311,2384`). **For ren flyt finnes ingen kodesti til 0 rader på dagens develop.** **⚠️ LATENT FELLE for neste kolonne (develop-Opus exit 2026-07-15):** pull-en sletter **alle** lokale rader FØR den reinnsetter i en løkke — ikke i én transaksjon. Legger noen en `NOT NULL`-kolonne på `sheet_timer_local` uten å oppdatere pull-insertens defaults, vil et kast midt i løkka etterlate sedelen med **0 rader** (slettingen er alt committet). F5s `pauseMin` er trygg (`pauseMin: t.pauseMin ?? 0`), men mønsteret er en felle hver gang datalaget utvides. Vurder tx-innpakking når noen uansett rører den stien.

**Residual (ÉN mekanisme):** pull-ens ikke-transaksjonelle slett-så-reinnsett (`timerSync.ts:635-687`) etterlater 0 rader kun ved tombstone-skip (`:666`) — krever *forutgående lokal rad-sletting* der synken deretter gikk conflict/avvist/feilet (levende tombstone blokkerer gjeninnsetting). IKKE den rene flyten. **Konklusjon:** reproduksjonen (07-10) er FØR S3 (07-11, shippet i #38) → trolig allerede lukket. **Neste (ingen kode):** simulator-re-test ren flyt på gjeldende develop bekrefter; hvis fortsatt repro → instrumentér pull (sedel i svar? server-rader ikke-tom? levende tombstone?). Sekundær-obs: pull-watermark klient-`Date.now()` (`timerSync.ts:432`) vs server-`updatedAt` (`dagsseddel.ts:3528`) → klokkeskew kan misse attestert sedel (stale rad, ikke 0).
- **🟢 Attestert sedel = workflow-deadlock — LØST 2026-07-11 (F4-2: ny leder-mutasjon `gjenaapneAttestering` `accepted`→`sent` + web «Gjenåpne attestering»-knapp i `accepted`-grenen `AttesteringDetalj.tsx`, prod `d1b96cd5`). Leder har nå web-vei til å angre fullført attestering. Analyse beholdt som kontekst:** Worker kan ikke gjenåpne en attestert sedel (by design, M4-vakt). M4-meldingen sier «be leder returnere dagsseddelen». **Retur-handlingen FINNES i kode** — `returnerRader`/`returner`-prosedyrer (`apps/api/.../dagsseddel.ts`) + `ReturnerDialog` i `apps/web/src/components/timer/AttesteringDetalj.tsx` (`returnerRader.useMutation`). **MEN** retur-knappen rendres kun når `sheet.status === "sent"` (`kanHandle = sheet.status === "sent" && antallValgt > 0`; handlings-blokken faller til status-melding ellers). Og `attesterRader` flipper sedelen til `status: "accepted"` når **alle** rader er attestert (post-transaction). Konsekvens: en **fullt attestert (accepted)** sedel har **ingen** retur-knapp på web, OG worker kan ikke gjenåpne → deadlock. M4-meldingen «be leder returnere» stemmer kun mens sedelen er **delvis** attestert (fortsatt `sent`). Relæets «leder har ingen retur-knapp» er upresist — knappen finnes, men forsvinner ved `accepted`. **Undersøk/avgjør:** enten (a) tillat retur også på `accepted`-sedel (leder-handling som setter tilbake til `sent`/`returned`), eller (b) endre M4-meldingen så den ikke lover en handling som er borte i `accepted`-tilstand. Kandidatmengde bekreftet: prosedyrer `attester`/`attesterRader`/`returner`/`returnerRader` i `dagsseddel.ts`; web `AttesteringDetalj.tsx` + `AttesteringDetalj_Edit.tsx`.
...
> **[triage 2026-08-26]** verifisert åpen — Kan vente — egen fase: data festet på A.Markussen via import, men INGEN PowerOffice-/lønnseksport-adapter i repoet (grep=0) → eksport kjører ikke i sept uansett.
...
> **[triage 2026-08-26]** verifisert åpen — Kan vente — egen fase: ingen prosedyre sjekker `Lonnsart.kode` før attestering; eksport-modulen den skulle gate finnes ikke → ikke pilot-blokker.
...
> **[triage 2026-08-26]** verifisert åpen — Kan vente — noen runder: semantisk felt mangler (grep=0), migrering velger laveste-`rekkefolge`; A.Markussen-stjerne alt korrigert via import → latent for nye orgs.
...
- ~~**F2-rest — `info_text`/`info_image`/`video` mangler web-render.**~~ **LEVERT 2026-08-23** (branch `feat/instruksjonstyper-web-pdf`). (1) Web-komponenter `InfoTekstObjekt`/`InfoBildeObjekt`/`VideoObjekt` (ren visning) registrert i `KOMPONENT_MAP` + lagt i renderer-`DISPLAY_TYPER` (uten FeltWrapper-tilbehør). Web behandler `video` som ren visning (ordre: «no user value»), mens mobil holder den interaktiv («watched») — bevisst forskjell, notert til DISPLAY_TYPER-unifiseringen under. (2) 🔴 Arkiv-PDF droppet alle fire eksplisitt (`felt.ts:37` frosset) → arkiv-override `arkivmal/instruksjonsfelt.ts` (D2-mønster, intercept i `innhold.ts`): info_text/info_image grå instruksjonskontekst, video referanselinje, **quiz spørsmål + avgitt svar + riktig/feil** (dokumentasjonsdata — dropp var datatap, F7-klasse). **Rest:** `info_image`-pikslene embeddes kun når URL er inlinet data-URI; `config.imageUrl` (`/api/uploads/...`) har ingen api-leser-inlining i dag (nettverksfri container) → viser bildetekst som kontekst. Full pikselembedding = eget api-leser-steg. **Rest 2:** instruksjonstyper som barn i en repeater-rad rendres av `radkort.ts`/`repeater.ts`, ikke `innhold.ts` — ikke dekket (kantscenario; PSI-typer er normalt toppnivå).
...
## 🟡 D7 byggeplassfilter er klientside på web, serverside på mobil (ført ved exit-gate 2026-08-21)

**Forbehold gatet av fabel 2026-08-21** — akseptert for pilotvolum, ført her fordi det
skalerer annerledes enn mobil.

Web filtrerer sjekklistelista på `aktivByggeplass` **klientside** (i `filtrerte`-useMemo,
OR-null-semantikk som speiler serveren). Mobil sender `byggeplassId` til
`sjekkliste.hentForProsjekt` og filtrerer **serverside**
(`apps/mobile/app/sjekkliste/index.tsx:64-67`).

**Utførers begrunnelse (akseptert):** klientside bevarer «X av Y»-telleren og unngår
invalidation-risiko ved kontekstbytte.

**Når dette må revurderes:** et prosjekt med mange sjekklister henter hele lista til
nettleseren før filtrering. Ved pilotvolum er det uproblematisk; ved vekst er
serverside-filtrering (som allerede finnes i `sjekkliste.ts:133,150`) den riktige veien.

**Måletrigger:** når et prosjekt passerer ~500 sjekklister, mål lastetid på
sjekklisteoversikten mot mobil.
