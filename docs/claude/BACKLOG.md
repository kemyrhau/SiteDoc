...
- **Vis som bruker (impersonering)** 🟡 — DEPLOYET kjernen. Schema (`Session.impersonatedUserId/originalUserId/impersonationExpiresAt`), 3 server-prosedyrer, context-håndtering, UI (`ImperserKnapp`, `ImpersoneringBanner`), i18n-nøkler, `utloperVed`-fix (2026-05-28) — alt på plass. **Audit-logging IMPLEMENTERT PÅ DEVELOP 2026-05-28** via ny `ImpersonationAudit`-tabell (Variant B, isolert). `startImpersonering` skriver INSERT, `stoppImpersonering` skriver UPDATE (`avsluttetGrunn = "manuell"`). Defensiv `.catch` på audit-kall slik at det ikke blokkerer impersoneringen. `console.log`-mønstre fjernet. **Gjenstående:** Lese-prosedyre + UI for audit-loggen — venter på tilgangs-oversikt-UX-sesjon.
