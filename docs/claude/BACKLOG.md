...
- **CLAUDE.md krymper igjen (39740/40k, 260 under)** 🟡 (2026-06-21) — server-topologi-noten (server-ny/Kenspill-legacy/test-host-UBEKREFTET) spiste opp mesteparten av trim-headroom-en. Ny trim-runde før neste tilføyelse: flytt mer deploy-/server-detalj til `infrastruktur.md`/`DOCKER-NOTES.md` og behold kun overordnede pekere i hovedfila.
...
- **Fillagring: CLAUDE.md sier «S3-kompatibel», virkeligheten er server-lokal disk** 🟡 (fanget under Funn #2, 2026-06-21) — REST `/upload` (`apps/api/src/routes/upload.ts`) lagrer til `process.cwd()/uploads`, servert statisk via `@fastify/static` (`server.ts:77`). **Ingen S3/R2/MinIO-klient finnes** i `apps`/`packages` — gjelder HELE bilde-stacken (sjekkliste/oppgave/rapport/tegninger/tillegg-kvittering), ikke bare timer. Ekte object-storage er en tverrgående infra-endring (migrer eksisterende `/uploads`-filer + bytt upload-rute + `fileUrl`-format). Ikke-blokkerende; CLAUDE.md tech-stack-linja bør enten korrigeres til «lokal disk (S3-klar)» eller S3 faktisk innføres.
