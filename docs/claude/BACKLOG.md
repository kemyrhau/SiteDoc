...
### 🔴 BEKREFTET BUG (høy prioritet): duplikat-dagsseddel fra «+ Ny»-flyt → sync-stuck (2026-06-22)

- **🔴 Duplikat-dagsseddel fra `+ Ny`-skjermen.** `lagre()` i `apps/mobile/app/timer/ny.tsx:207` **inserterer alltid en ny `dagsseddelLocal`** med fersk UUID (`:193`) **uten å sjekke om det finnes en eksisterende sedel for `(userId, dato)`**. Server håndhever `@@unique([userId, dato])` (`db-timer/schema.prisma`) → andre sedel for samme dag **kolliderer ved sync** → rad blir **sync-stuck** (`syncStatus: pending` som aldri lykkes) + arbeider sitter på en lokal dubl som aldri når attestering → **tom/manglende attestering hos leder**. **Verifisert i kode 2026-06-22.**
  - **Fiks:** finn-eller-åpne eksisterende dag før insert — speil idempotensen i `opprettDagsseddelForSegment` (`StartSluttDagKort.tsx:578–589`: `select` på `(userId, dato)` → naviger til eksisterende `id` i stedet for å inserte). `ny.tsx` skal `router.replace(/timer/{eksisterende.id})` når dagen finnes.
  - **Knyttes til U-flyt** (§ 6.1 i [mobil-dagsseddel-ui-spec.md](mobil-dagsseddel-ui-spec.md)): samme **find-or-append**-prinsipp. U-flyt løser append av *ny økt* til eksisterende draft; denne saken er find-or-*open* fra manuell `+ Ny`. Samme rotmønster (manglende per-dag-idempotens på en sedel-opprettelses-inngang) → bør fikses sammen.

- **🟡 Separat: tastatur-avoidance på `+ Ny`-skjermen** — skjemaet skyves ikke opp når tastaturet åpnes (felt skjult bak tastatur). Mangler `KeyboardAvoidingView`/scroll-justering. Kartlagt 2026-06-22.
- **🟡 Separat: «kan ikke velge lønnsart på Ny-dagsseddel-skjerm» (UX)** — `+ Ny`-skjermen lar ikke arbeider velge lønnsart ved opprettelse (kun prosjekt + aktivitet). Lønnsart settes først inne på sedelen. Avklar om lønnsart-velger hører hjemme på opprettelses-steget eller om dette er forventet flyt. Kartlagt 2026-06-22.
- **🟡 Separat: to-inngangspunkt-konsolidering** — det finnes to veier til ny dagsseddel: manuell `+ Ny` (`ny.tsx`) og auto-draft via «Start/Slutt dag» (`StartSluttDagKort` → `opprettDagsseddelForSegment`). De har **divergerende opprettelseslogikk** (idempotens kun i sistnevnte — jf. duplikat-bug over). Vurder å konsolidere til én delt `finnEllerOpprettDagsseddel(userId, dato, …)`-helper begge inngangene kaller, så idempotens + org-backfill + arbeidstid-forhåndsutfylling er ett sted. Strukturell oppfølger; fikser rotårsaken bak duplikat-bug + `organizationId:""`-kosmetikk i samme grep.
