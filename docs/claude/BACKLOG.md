...
### Geofence-modal: Leaflet-kart laster kun hjørne-fliser ✅ FIKSET PÅ DEVELOP 2026-07-04

Observert 2026-07-04 (prod, etter geofence-oppdagbarhet-deployen). Når geofence-modalen åpnes lastet Leaflet-kartet kun flisene i ett hjørne — resten av kart-viewporten grå/tom. Adressesøk + manuell lat/lng-inntasting virket (georef fungerer), kun kart-visningen var ødelagt.

**Rotårsak (verifisert mot kode):** ikke i byggeplasser-siden — i den delte `KartVelger`-komponenten (`apps/web/src/components/KartVelger.tsx`). `Modal` (`packages/ui/modal.tsx`) bruker native `<dialog>`; barna er **alltid montert** men `display:none` når lukket. `L.map()` initialiseres derfor ved sidelast med **0×0-container**, og det eksisterende `setTimeout(() => invalidateSize(), 100)` fyrer mens dialogen fortsatt er skjult → nytteløst. Når modalen åpnes (`showModal()` → `display:block`) kaltes ingen ny `invalidateSize`. **Ikke ren regresjon fra `b1c81629`** — rotårsaken lå i KartVelger hele tiden; `b1c81629` eksponerte den ved å legge kartet i en egen, knapp-åpnet modal.

**Fiks (PR 2, 2026-07-04):** `ResizeObserver` på kart-containeren i `KartVelger`s mount-`useEffect` → `map.invalidateSize()` når container går 0 → faktisk høyde ved modal-open. `observer.disconnect()` i cleanup. Fallback-`setTimeout` beholdt. Self-contained, fikser alle modal-hostede kart-bruk (ikke bare geofence). **Gjenstår: prod-deploy** (buntes med web-PR-en, jf. sikkerhets-fiksens prod-forespørsel).
...
**✅ Sak #5 — firma-ansatte ser eget firma — FIKSET PÅ DEVELOP 2026-07-05:** `firma-kontekst.tsx` brukte admin-only `hentTilgjengelige` → firma-ansatte (role="user") fikk ikke `valgtFirma` populert (så verken eget firma, prosjekter eller timer/maskin-flater). **Dobbel-kilde-design** (ikke naivt kilde-bytte, som ville krasjet modul-gating + lekket firma-admin-skallet): (1) `hentMineMedlemskap` (`organisasjon.ts`) beriket med `aktiveFirmamoduler` via ny modul-scope-helper `berikMedFirmamoduler` (delt med `hentTilgjengelige`) → begge query-resultater har samme shape; (2) `firma-kontekst` beholder `hentTilgjengelige` som admin-sett + legger til `hentMineMedlemskap` som kilde for `valgtFirma`-populering (auto-select: `tilgjengelige.length===1` → ellers `mineMedlemskap.length===1`); (3) nytt `kanAdministrereFirma = erSitedocAdmin || tilgjengelige.some(f => f.id === valgtFirma?.id)` — bevarer dagens regel (admin-tilgang ≡ sitedoc_admin ∨ firma_admin-medlemskap) bit-for-bit; (4) re-gate firma-admin-flatene: `firma/layout.tsx` (choke-point for alle 22 `/dashbord/firma/*`-ruter) + opprett-UI i `nytt-prosjekt` + `kom-i-gang` gater nå på `kanAdministrereFirma`, ikke på `valgtFirma`-eksistens. Prosjekt-lister (`prosjekt.hentAlle`) er allerede medlemskaps-scoped server-side → ingen lekkasje. **Kant (ikke-blokkerende):** ikke-admin i flere firmaer treffer ikke `length===1`-auto-select og kan ikke bytte (velger gated på rolle) → intet firma-innsyn; pre-eksisterende for multi-firma firma_admins, sjeldent for menige. **Oppfølger (PR 4, 2026-07-05):** maskin opprett/import-UI (`maskin/page.tsx` «Nytt»/«Import»-innganger + `maskin/nytt` + `maskin/import` page-guards) gates også på `kanAdministrereFirma` — rent kosmetisk (server beskytter allerede via `verifiserFirmaAdmin`/`autoriserAdminForFirma`), hindrer at ansatte når skjemaer som feiler. Maskin-lista/-visning forblir åpen for ansatte (de logger maskinbruk). Ny i18n-nøkkel `firma.maskin.ingenTilgangOpprett` (15 språk). **Gjenstår: prod-deploy** (buntes med sikkerhet + Leaflet).
