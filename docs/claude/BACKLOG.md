...
- **MalbyggerV2 — fire-fane redesign (`MalListeV2.tsx`)** 🟡 — Ny komponent på `/dashbord/oppsett/produksjon/maler-v2` bak feature-flag. Iterér der, bytt over i én PR når validert. Gammel `MalListe.tsx` fjernes ved overgang. Faner: **Oppgave | Sjekkliste | HMS | Avklaring** (siste utelates til Avklaring-modul er implementert — se entry over). Begrunnelse: dagens UI har «type-radio + HMS-hake»-modell som blander to akser (kategori + domain). Fire-fane-modell gjør valgene visuelt likestilt og forberedt på Avklaring som tredje kategori. Forutsetninger før start: (1) avklare om HMS skal være egen fane eller filter på Oppgave/Sjekkliste, (2) Avklaring-modul må være spesifisert. Minimal-fiks (HMS-checkbox-gate) deployet 2026-05-29 — løser akutt rotårsak uten å forskuttere redesign-valg.
