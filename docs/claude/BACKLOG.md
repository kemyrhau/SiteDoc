...
### Geofence-modal: Leaflet-kart laster kun hjørne-fliser 🟡

Observert 2026-07-04 (prod, etter geofence-oppdagbarhet-deployen). Når geofence-modalen åpnes laster Leaflet-kartet kun flisene i ett hjørne — resten av kart-viewporten er grå/tom. Adressesøk + manuell lat/lng-inntasting virker (georef fungerer), kun kart-visningen er ødelagt. **Klassisk Leaflet-i-modal:** kartet initialiseres før modalen har fått endelige dimensjoner → Leaflet regner feil størrelse og henter kun fliser for det den tror er viewporten. **Sannsynlig fiks:** `map.invalidateSize()` etter at modalen er åpnet/målt (f.eks. i en `setTimeout(0)`/`requestAnimationFrame` ved open, eller på modalens transition-end). Fil: `apps/web/src/app/dashbord/oppsett/byggeplasser/page.tsx` (geofence-modalen). **Verifiser om regresjon fra `b1c81629`** (geofence-seksjonen ble flyttet til egen modal der) eller pre-eksisterende. Lav-middels prioritet — blokkerer kart-basert geofence-setting, men adresse/lat-lng er workaround.
