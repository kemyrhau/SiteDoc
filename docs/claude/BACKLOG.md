...
- **Cloudflare↔Expo-Go: POST-respons når ikke iOS-simulator** 🟡 (dev-miljø) — `/dev-login` (og potensielt andre POST) svarer 200 fra origin, men responsen droppes på vei tilbake til Expo Go i simulator → «Network request failed». GET-responser kommer frem. Blokkerer dev-login + mulig OAuth-token-bytte i simulator. Workaround: fysisk enhet/TestFlight. Mulig fiks krever Cloudflare WAF-justering for Expo-Go-user-agent mot api-test (infra-endring, krever Kenneth-godkjenning) — ELLER lokal API + mobil mot localhost (bryter «.env peker alltid mot api-test»-regelen).
