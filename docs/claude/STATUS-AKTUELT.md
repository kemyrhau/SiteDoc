...
- **⚠️ Deploy-gate (1a+1b):** terminal-`aktivPosisjon` (1a) + HMS-posisjon-2-backfill (1b) er kjøringsverifisert men IKKE dataverifisert (sandkasse manglet terminale/sendte HMS-dok) → **spot-sjekk mot test-DB ved test-deploy** før prod.
- **Gjenstår:** Fase 2 delt utledning + `avledStatus` + non-terminal aktivPosisjon-backfill · Fase 3 server-omskriving (11 statusskriv → avledStatus, 3 rutingveier → delt utledning, isValidStatusTransition→retningsregler, fjern B-gaten) · Fase 4 web+mobil-paritet (inkl. mobil HMS-flytlinje-skjul) · Fase 5 e2e + prototype-verifisering (distinkte personer).
