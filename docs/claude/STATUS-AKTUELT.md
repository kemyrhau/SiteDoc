...
- **Fase 1b** (merget develop `5391f4f7`, branch `feat/flytmodell-1b-hms` @ `9c127e7e`): HMS flyt-binding. HMS-opprett binder nå dokumentet til prosjektets HMS-flyt (`dokumentflytId` + `aktivPosisjon:2` + `sendt:true`, graceful fallback). Sikkerhetsgate: `verifiserFlytRolle` hoppes for `domain="hms"` i `endreStatus` (nøytraliserer rutings-bivirkning — fjernes Fase 3). Backfill eksisterende HMS-dok (`20260731140000`, draft/responded→1 ellers→2). FlytIndikator skjult for HMS på web; **mobil-skjul UTSATT til Fase 4** (mangler `domain`-plumbing; mobil HMS-flytlinje viser «?» interim, atferdssikret via gaten). Ingen rutingendring, ingen brukersynlig endring. API+web typecheck exit 0.
...
- **Fase 2** (merget develop `94b99149`, branch `feat/flytmodell-fase2` @ `e87a7120`+`7b378fda`): delt utledning i `packages/shared/src/utils/flytPosisjon.ts` — 7 funksjoner (`nesteLedd`/`forrigeBallLedd`/`avledStatus`/`harBallenPosisjon`/`retningsrettigheter`/`finnPosisjon`/`gjenapnePosisjon`) + `FlytPosisjonLedd`-type. 37 enhetstester (Orienteres-hopp, siste=Godkjenn, § 2.4-gjenåpne, 3 prototype-forhåndsvalg) + divergens-test (gammel recipient-basert vs ny posisjon-basert `harBallen` — Kenneths pilot-case divergerer på 2 seere = beviser ny er riktig). Non-terminal aktivPosisjon+retning-backfill (`apps/api/scripts/`, kaller delt `finnPosisjon`, idempotent). Reconciliation Q1-Q4 (fabel): received/in_progress kollapser→«Hos N» (in_progress aldri statusfakta igjen), harBallen posisjon-basert, avvist→`dismissed` i cache, backfill = TS-script. **Kun bibliotek — ingen konsument/ruting rørt.**
- **Fase 3** (merget develop: 3.1 `8758bb1b` skygge-fakta · 3.2 `deaa578e` avledStatus=eneste status-skriver · 3.3+3.4+3.5-ruting `7a1b172e`): **server-omskriving komplett — ruting er nå posisjons-basert.** De 11 statusskrivestedene skriver fakta (aktivPosisjon/retning/terminal/sendt), `avledStatus` setter status-enum-cachen. Rutingveiene bruker delt utledning: Send→`nesteLedd` (forover), Besvar→`forrigeBallLedd` (retur bakover), forwarded→`finnPosisjon`. `verifiserFlytRolle`→`verifiserRetningsrett` (posisjons-authz via `retningsrettigheter`+`kanTerminereUtenBall`); 1b B-gaten fjernet (HMS ruter via posisjon). `firmaBehandleAvvik`→↔ terminal-override via `avledStatus`+`DocumentTransfer`. **Besvar-semantikk (fabel-bindende, Tolkning A):** utfører-submit = Send→ (forover til godkjenner); Besvar← = KUN retur bakover. Bakoverkompat: `nyStatus`-input bevart, output beholder recipient (nå posisjon-utledet). 424 shared + 16 api-tester grønne, typecheck exit 0. **in_progress + HMS-opprett-`sent` kollapser til «Hos N» (avledet).**
- **Fase 3.6** (merget develop `773c1a58`, branch `feat/flytmodell-fase3-6`, fabel-løsning 1): **`received→sent` gjeninnført** i `isValidStatusTransition` (én linje) — §8A fjernet den som recipient-løs no-op i gammel modell; posisjonsmodellen ruter `sent→nesteLedd` (guardet av `verifiserRetningsrett`, kun ballholder), så «Send → = neste ledд» (veileder § 2.2) virker fra ETHVERT ledд (1→2→3→4). **FULLFØRER P1, reverserer den ikke.** UI-tilbud (Lag B: statusHandlinger/flytmatrise) URØRT → dormant server-kapabilitet til Fase 4 steg 4 wirer knappen; gammel klient uendret. `responded→sent`/`approved→sent` forblir fjernet. 5a-e2e utvidet: Send-kjede 2→3→4 grønn (tidligere BAD_REQUEST) + «bestiller sist»-fixture. 8/8 e2e + 424 shared grønt. **Forover-semantikken er nå komplett.**
- **✅ Fase 5a** (merget develop `12d2e401`): server-integrasjons-e2e mot localhost via `createCaller` — kjører 31.07-sekvensen gjennom ekte `endreStatus` (ruting+authz+avledStatus) med distinkte personer. Pilot-fiksen bevist: Besvar går IKKE bakover til vilkårlig avsender. Opt-in `test:integration` (ikke i default `turbo test`). Regresjonsnett for pilot-fiksen.
- **✅ Deploy-gate LUKKET PÅ TEST (2026-08-01):** test-deployet til sitedoc_test. Backfill dataverifisert: terminaler 1a (godkjent 3/3, lukket 2/2 med posisjon), HMS 1b (7/8 bundet, 4 på pos 2), Fase 2 non-terminal (57 sjekklister, 0 ubestembar). **Server-ruting verifisert korrekt:** Send fra registrator satte `aktiv_posisjon=2` (bestiller) — `nesteLedd` virker. Merk: klient-DISPLAYET viser fortsatt gammel ledd-markering (Fase 4 ikke deployet — forventet gammel-klient/ny-server-mismatch, ikke bug). **Gjenstår for prod:** samme migreringer+backfill+ruting-verifisering mot prod-DB ved prod-deploy. 5a-e2e (localhost) = automatisert regresjonsnett.
- **Fase 4** (merget develop: del 1 `merge-flyt4` klient-konsolidering + steg 3+4 `feat/flytmodell-fase4-steg34`): **hele klient-siden er nå på posisjonsmodellen.** Web `flyt-ledd.ts` + mobil `dokumentflyt-ledd.ts` konsolidert mot delt `byggPosisjonsLedd` (sekvenser på `steg`, aktiv boks = `aktivPosisjon` fra server-fakta, `forventetRolleKandidater`+`ROLLE_RANG`×3 slettet). `harBallen`→`harBallenPosisjon` (Q2). **Klient-handlingsfilter = serverens `verifiserRetningsrett`** (`hentPosisjonFiltrertHandlinger`: ball-handlinger via `retningsrettigheter`, admin=hele universet, trekk-tilbake=`seerErBakover`, gjenåpne=admin) — én kilde, klient=server. **Send-fra-received wiret** (Lag B), primær «Send til N · X →» / «Godkjenn og fullfør ✓» (siste ledд). `verifiserRetningsrett` tar nå `fra-status` (skiller trekk-tilbake=`seerErBakover∨admin` fra gjenåpne=admin — fikset Fase-3.4-gap). ansvarsmerke i flyt-sheet (§ 2.6), header=nummer+hvem. `output-plumbing` (read-only select). shared 429 + web 91 + api-e2e 8 grønt. **Gjenstår (små):** «Du har ballen — {merke}»-mikrotekst i kompakt header (kosmetisk) · 13-språk-generate (v/merge).
- **✅ Pilot-fiks A+B+D/#11** (merget `29b26d47`, deployet+verifisert live): (A) obsolet utkast-mottakervelger fjernet fra flyt-bundne dok (`draftSend && !harFlyt` + `videresendOppforinger` på `!harFlyt`, C-orden); (B) primær `nesteLedd`-styrt — «Send til N·X →» fra kontroll-ledд, «Godkjenn og fullfør ✓» kun siste ledд (`received→sent`+`responded→sent` gjeninnført, ball-guardet); (D+#11) gjenåpne skriver ikke draft til cache → «Hos N» (`sendt=true`). Backfill re-avledet eksisterende feilskrevne dok.
- **✅ Runde-2** (merget `0a833261`, deployet+verifisert live; fabel-vedtak § Runde 2-pilot): **én bakover-handling** — «Send tilbake» (`responded→in_progress`) fjernet, **«Besvar til N·X ←»** eneste bakover (fra received òg); **in_progress kollapset HELT** (ingen «Under arbeid» i loggen, tidslinje = avledet vokabular); **trekk-tilbake→«Hos N»** (`retning=frem`+`sendt=true`, REVERSERER D-scoping) + guard (kun avsenderledд, aldri ferskt utkast); **«UTFØRER»→«Faggruppe»**; **dokumentflyt-navn** i sheet+web-caption; **seer-relativ «Venter på deg»**-chip; **tooltip-portal** (escaper `overflow`-klipp). Backfill utvidet (trekk-tilbake + legacy in_progress).
- **✅ Bygg-stempel** (merget, del av batch): `GIT_SHA`+byggtid i image → `/version` + diskret Innstillinger-linje. `--build-arg` via Mac-side SHA (`.git` ekskl. rsync) — se `deploy-detaljer.md § Bygg-stempel`.
- **✅ Polering P1+S2** (merget `0daa89e1`): (P1) «Send» skjult i split-▾ på siste ledд (`erSisteLedd`→dropp `sent` i delt `hentPosisjonFiltrertHandlinger`, web+mobil); (S2) admin-flytmatrisens døde in_progress-stasjon ryddet (negativ-kontroll bestått — kun firma-admin-konfig, ingen dokumentflate).
- **✅ Deploy-gate INNFRIDD på test:** hele modellen live-verifisert av Kenneth (distinkt-person 4-ledds pilot: Send/Besvar/Godkjenn/Gjenåpne/Trekk-tilbake, merkelapper, statuser, tooltips, «Venter på deg»). Ingen kjente restanser.
- **Gjenstår (valgfritt):** re-deploy av polering (kosmetisk, ingen backfill) · **prod-deploy** når moden (samme migreringer+backfill+ruting-verifisering mot prod-DB) · Playwright pilot-e2e-spec på `feat/flytmodell-5b-uie2e` (remote-rigg, post-deploy) · trekk-tilbake-status-semantikk (fabel, parkert) + bøtte 4 (byggeplass-kontekst #2/#3 → fabels kontekst-fra-innlogging-spor). E2e-nett: 5a server-integrasjon (localhost) + shared-enhetstester.
...
- **P4b web ett-klikk opprett** (merget `4858d342`): delt `DokumentKontekstChipLinje` (prosjekt·byggeplass·faggruppe·mal, inline overstyring, hevede trakt-primitiver fra `KontekstChip`), utfyllingsmodus + redigerbar tittel på detaljsiden, flyt-gruppert mal-velger, `useSistBrukteMal` (klient-lokal interim). **pkt 0 tilgjengelighets-filter** (`mal.hentForProsjekt` additivt `opprettbar`, delt m/ opprett-validering — maler som ville avvises vises ikke lenger, web+mobil). E2e fanget + fikset stale-closure-bug (auto-hopp fra toppknapp virket aldri — gjaldt òg P2). Fabel-godkjent (harness+e2e). `verifisering/p4b-ettklikk-web-verifiseringslogg.md`. Klikk ≤2. Malbytte-server-sak backlogget.
- **P4a mobil iOS-modal** (merget `0cb74bbe`): serialiser `<Modal onDismiss>` + Platform-gren + `internSynlig`-speil → auto-opprett-skip trygt ved entydig kontekst (spinner → rett i utfylling). GPS best-effort. Foundation for mobil opprett-flyt. **Venter Kenneths fysiske iOS-test på develop (fix-forward hvis noe henger).** `verifisering/p4a-ios-modal-verifiseringslogg.md`.
- **P4c timer** (venter start): 7→1-2 klikk, arver chip-komponenten.
...
**Verifikasjon:** full build grønn (prisma generate ×4, typecheck 4/4, `next build` 2/2). **Backlogget (gatet):** `DocumentTransfer.handling`-markør (tidslinje-skille Besvar/Returner) + guard-mot-ikke-hms-behandler-ledd. **DEPLOYET PROD `881e66e6`** (Blokk 8) — received-fiksen virker, MEN innførte read-only-regresjon (melder kan ikke fylle ut, fordi `erMelder && status==='draft'`-grenen aldri traff siden HMS aldri var draft) → løst av Diff 2/5a ved rotårsaken.
