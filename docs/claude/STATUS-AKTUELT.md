...
| **redesign** | `SiteDoc-redesign` | 🟢 **LEDIG** — `b72e011e` detached (merget commit) | Dobbelt-lån-vakt i firmaarkivet + ærlig VERSJON-kolonne **merget runde 88 (`78a6bbbf`)**: server-vakt `findFirst` FØR `create` i `laanFraSentralarkiv` (treff → CONFLICT), «Firmaversjon»-kolonne. api 462→465 (3 dobbeltvakt-tester). Forrige: «Hent fra arkiv» merget runde 87 (`a517c3ec`). 🔴 **Flere åpne tråder — se under.** ⚠️ **«Hent fra arkiv» ikke lukket som ferdig før pilotsjekk + fabels designgate** |
| **dokgen** | `SiteDoc-dokgen` | 🟢 **LEDIG** — `44487811` detached (merget commit) | Papirkurv-forhåndsvisning + dempet «Tøm» + slett-vakt på levende kobling **merget runde 85 (`652cdbda`)**. Løste låsningen Kenneth var blokkert av. ⚠️ **Dokgen meldte senere en gate-rapport for `44487811` og ba om grønt lys — men arbeidet var ALLEREDE merget** (`44487811` er ancestor til develop). **Ingen ny merge — ikke bestill det igjen.** 🔴 **Rettelse: tallet `api = 444` i merge-ordre 85 var COWORKS, ikke dokgens** — dokgens commit-melding inneholdt aldri noe testtall. **Riktig tall er 458, målt uavhengig av både dokgen og merge-agent. Ingen tester gikk tapt.** Venter neste FUNN-ordre |
...
| **merge-agent** | `SiteDoc-merge` | 🟢 **LEDIG** — `merge-restart` @ develop | Runde 88 sist: merget dobbelt-lån-vakt (`78a6bbbf`). Web-deploy samles med neste |
...
- 🔴 **Pilotsjekk «Hent fra arkiv» (åpen etter runde 87–88):** Kenneth har nå lånt inn alle seks NS 3420-malene inkl. KD1, men **sluttmålingen er ikke kjørt skriftlig.** Prosjektadmin mister direkteimport fra NS 3420-biblioteket (tilsiktet vedtak) — pilotsjekken er det eneste som kan gjøre det til et problem. **Kenneth kjører den; ingen agent når DB.** «Hent fra arkiv» skal IKKE lukkes som ferdig før den er svart
- 🔴 **`@@unique` på `OrganizationTemplate(organizationId, laantFraBibliotekMalId)` gjenstår (åpen etter runde 88):** dobbelt-lån-vakten (runde 88) er kun server-side `findFirst`+CONFLICT — DB-garantien mangler. **Bevisst utenfor scope:** de tre KB4-duplikatene på test må avklares først, ellers feiler migreringen. **Skjemaendring Kenneth gater.** Jf. «stille tomhet»-regelen krav (b)
