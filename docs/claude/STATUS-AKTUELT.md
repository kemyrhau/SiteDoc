...
### ✅ Timer-mobil F2/F3/F5 (feltfunn del-6) — MERGET develop, venter EAS #40 + prod spor b (2026-07-14)

Tre mobil-timeføring-fikser fra Kenneths feltfunn (byggeplass + matpause), fabel design-OK. Fasit: [timer-mobil-f2f3f5-spec.md](timer-mobil-f2f3f5-spec.md). **Merget develop `11bfeb2b` (F2/F3/F5) + `95796f7e` (edge #1 dynamisk matpause-etikett + copy) + i18n 13-språk `8cada21c`. Server-migrering `20260714120000_sheet_timer_pause_min` kjørt på test; prod venter spor b.**
- **F2 ✅** — `ByggeplassVelger` tri-tilstand (henter/offline/bekreftet-tomt) + auto-refresh ved tom cache+online + fabels `projectId`-reset-catch + 4 i18n-nøkler. Cowork-gate PASS.
- **F3-datalag ✅** — per-rad `byggeplassId`: `sheetTimerLocal`-kolonne + idempotent ALTER + sync begge veier + syncBatch input/createMany-override (`t.byggeplassId ?? lokal.byggeplassId ?? null`, sedel-fallback) + `hentEndringerSiden`-respons. Bakoverkompat. Cowork-gate PASS; test-api deployet + p6 server-verifisert (rad_bp ≠ sedel_bp).
- **F3-UI ✅** — kombinert prosjekt+byggeplass-`pageSheet` (`ProsjektByggeplassVelgerModal`: favoritter→resten, INGEN GPS-tier, F2 tri-tilstand + reset-catch) + rad-sekundærlinje «Prosjekt · Byggeplass» + hybrid-hurtigsti (byggeplass-only → delt `oppdater`; prosjektbytte → full rad-modal via `tvungetValg`) + `byggeplassId` gjennom insert. 5 i18n nb+en. Cowork code-gate PASS + fabel visuell + simulator-runtime bekreftet.
- **F5 ✅** — matpause-bærer per timer-rad. `pauseMin` INT NOT NULL DEFAULT 0 lokal + server `sheet_timer.pause_min` (migrering `20260714120000_...` **kjørt på test, prod venter**) + sync begge veier. Bærer = **lunsj-kryssende carve-rad** (Valg A); flytt-ikke-radio (`services/matpause.ts`). **Kryss-modul-invariant:** `dagsseddel.pauseMin = Σ(rad.pauseMin)` → maskin-kapasitetsregelen urørt, `sheet_machines` INGEN pauseMin. **Edge #1:** dynamisk minutt-etikett (`pauseOverlappMin`), uhaket rad = «Matpause trukket» uten parentes. Server-verifisert (pause_min=30, Σ=30). Modul-avhengighets-gate PASS.
- **Gjenstår:** EAS #40 (mobil-leveranse, Kenneths go) + prod spor b (full develop→main når del 6 m.m. godkjent, inkl. server-migrering `20260714120000_sheet_timer_pause_min`). Docs: [timer.md § F5](timer.md) + [timer-mobil-f2f3f5-spec.md](timer-mobil-f2f3f5-spec.md).
