...
> 🎨 **REDESIGN (branch `redesign/navigasjon`, bak `nyNavigasjon`-flagg — IKKE i prod-løpet):** Steg (ii) hub + funn-1b-fix (`5b576d25`) og steg (iii) sidebar + kontekst-chip + P31 Kontakter-lesevisning implementert 2026-07-05. Ny `NavSidebar` (PROSJEKT+FIRMA-soner + Innstillinger-footer), `KontekstChip` («{Firma} / {Prosjekt} ▾», 1b-lastetilstand), gating G1–G12 ekstrahert til delt `sidebar-elementer.tsx`. **Flagg av = dagens UI (byte-identisk arrays + filter-predikat bevist).** Full fremdrift + T/G-paritet: [redesign-paritetssjekkliste.md § Fremdrift](redesign-paritetssjekkliste.md). Flagg-test på test via `?nyNav=1`. **Dev-login (agent-testing, Nivå A+B):** seedet testbrukere (sitedoc_admin/company_admin/arbeider) + `/dev-login` m/ whitelist + `DEV_LOGIN_SECRET`-header, gated til test — se [dev-login-agent.md](dev-login-agent.md). **UX-runde s1 + polish v4/v5 (2026-07-06, ✅ merget develop `5e16987b`):** NavSidebar kollaps-knapp (60px ikon-skinne, localStorage), mappetre skjul/vis-pil (`SekundaertPanel` valgfri `skjulNokkel`) + brødsmulesti i mapper-innholdsheader, v4 polling av oversettelses-jobbstatus (amber→grønn uten refresh), v5 infoboks-tall («{n} dokumenter ({g} kan oversettes automatisk nå)»). Alt bak `nyNavigasjon` (flagg av byte-identisk). + Metro `blockList`-fiks (`.env.eas.local`). 6 nye i18n-nøkler × 15 språk. **✅ s1 designgodkjent av fabel + v4/v5 web-verifisert (alt bestått) 2026-07-06 → STEG V LUKKET.** **Dev-login simulator-oppsett ✅ KOMPLETT + verifisert grønt** (localhost-tunnel + prebuild; rotårsak: Cloudflare-kant → Local Network-privacy → stale container-secret) — se [dev-login-agent.md](dev-login-agent.md) + BACKLOG (utført). **Seed i `sitedoc_test`:** testbrukere (admin/firma/arbeider) + Agent-testprosjekt + oversettelse-testmappe (Markussen B12). **Steg vi (2a mobil-tabs) ✅ LUKKET (fabel-designgodkjent 2026-07-07, merget `35503d98`):** ny RN tab-struktur bak `nyNavigasjon` (Hjem·Tegninger·Dokumenter·Timer·Mer + Kontakter K6 + toggle sitedoc_admin), Timer-tab = valg B (ekstrahert `DagsseddelListe`, uendret timer-stack), i18n 17×15, alle skjermer simulator-verifisert (`docs/redesign/screenshots/2a-simulator-2026-07-07/`). Degradert Tegninger (uten ★/punkt-antall/thumbnail — backend-oppfølgere). Ny `simulator-runbook.md`. **Steg vii (2c dokumentleser) ✅ LUKKET (fabel-designgodkjent 2026-07-07, 8/8 fangster mot `06-design.png`, merget develop `8c774368`):** 2c-leser bak `nyNavigasjon` (`dokument/[id].tsx`) — sticky språkpille-rad (kilde-merke, aktiv fylt blå, amber «…» m/ 8s-poll), grønt banner «Oversatt til {språk} · vis original» (motor droppet v1), `+ Oversett`-pille (ny `mappe.oversettDokument`, isActive-guard, rører ikke motor-config), bunn-ark-sammenlign (original + OPUS-MT/Google/DeepL-motorkort, gjenbrukt web-logikk). Backend `hentDokumentBlokker` utvidet (sourceLanguage/projectId/jobber). Token-fiks `sitedoc-primary`→`sitedoc-blue` (mobil-tokens). i18n 17×15. Fangster i `docs/redesign/screenshots/2c-simulator-2026-07-07/`. Tom-tilstand kun nåbar via direkte navigasjon (uparset gated ut av lista). **Status: steg ii–vii LUKKET. Neste milepæl: restanse-runde (P-a statusprikk → T9 mobil-web-hamburger → FM5/K2 «Mine timer»→brukermeny) — GO gitt 2026-07-07.** Oppfølgere i BACKLOG (§ Redesign-mobil): s3 utlogging-redirect, stale toggle, enhets-lagret flagg (pilot-beslutning), agentprosjekt-innholdsseed, kildespråk≠prosjektspråk-oversettelsesgap (via cowork). **MS-login mobil lokal dev:** placeholder client-ID bevisst (BACKLOG) + AADSTS50011 test-callback (BACKLOG).
...
- **Del 1 — maskin-velger søk/filter/sortering.** Ny delt web-komponent `MaskinVelger.tsx` (`apps/web/src/components/timer/`): `SearchInput` + kategori-chips (Alle/Kjøretøy/Anleggsmaskin/Småutstyr m/antall+ikoner, enkel-select) + sortering **brukt-på-seddelen → internNummer (numerisk) → navn**. Brukt i alle fire web-callsites: `MaskinRadDialog` (primær), `KompaktMaskinRad` (`RedigerRadModal`), `RedigerMaskinRad` (+ `SplittRadModal` via denne). Modalene slutter å caste bort `kategori`. Mobil `EquipmentVelgerModal` (`MaskinSeksjon.tsx`): søk fantes → la til kategori-chip-rad + samme sortering + «brukt»-markør.
- **Del 2 — maskin ≤ arbeidstimer proaktiv (b+disable).** Inline kapasitet-linje «Arbeid i gruppen: X t · maskin så langt: Y t · ledig: Z t», rød + Lagre disabled ved overskridelse. Web `MaskinRadDialog` (reaktiv for gjeldende (projectId, ecoId) fra hele sedelen) + mobil `MaskinRadModal` (SQLite-spørring på current bucket, robust mot prosjekt/ECO-bytte). `pauseMin` tres ned `[id].tsx → ProsjektGruppe → EcoBucket → MaskinSeksjon → modal`.
- **Delt sannhetskilde (kritisk):** ny `packages/shared/src/utils/maskinKapasitet.ts` (`beregnMaskinBrudd` / `overstigerMaskinTak` / `maskinBucketKapasitet`, epsilon `0.001`, pause-modell `pauseMin/60`, bucket-nøkkel `${pid}|${eco}`). **Serverens `validerMaskinUnderArbeid` delegerer nå hit** (kun Decimal→number-konvertering igjen) — klient-disable (web + mobil) kaller samme funksjon → umulig å drifte fra hverandre. Bonus: eksisterende ikke-blokkerende advarsler (attestering per-rad + mobil `EcoBucket`-indikator) rutet gjennom samme helper; mobil-indikatoren inkluderer nå pause (var utelatt før).

Ingen schema/migrering (ren UI + delt util). i18n: 3 nye nøkler (`timer.maskin.bruktPaaSeddel/kapasitet/overstigerArbeid`) nb+en + auto-oversatt til 13 språk. Typecheck rent (mine filer; mobil-baseline-feil urørt). **Distribusjon:** web deployer ved neste prod-deploy; mobil-delen akkumuleres til neste EAS-bygg (ikke egen fyring). **Reload:** Expo JS/TS (Fast Refresh). Detaljer: [timer.md](timer.md) + [maskin.md](maskin.md).

### Timer auto-lønnsart ③ (overtid strukturert + garantert standard) — IMPLEMENTERT PÅ DEVELOP 2026-07-05 (web klar for prod, mobil venter EAS-batch)

Lukker de to payroll-BACKLOG-postene (③a 🔴 feil-match, ③b 🟡 manglende standard). **Migrasjons-bærende** (db-timer).

- **③a — strukturert overtid.** Nytt `Lonnsart.overtidsnivaa Int?` (nullable) erstatter fritekst-navne-regexen (`/overtid/i && /50/`) i mobil auto-gen. Overtid velges nå via `velgOvertidLonnsart` (type=ordinaer + `overtidsnivaa`-match, aldri navn). Lærling-varianter beholdes `overtidsnivaa=null` i backfill → aldri auto-valgt for normal arbeider (kjernen i bugen). Web admin-UI: «Overtidsnivå»-select i `LonnsartDialog` + kolonne + Zod 50/100/null på `lonnsart.opprett`/`oppdater`.
- **③b — garantert standard.** Data-backfill setter `erStandardvalg` for orgs med ≥1 ordinær men ingen standard (foretrekk `Timelønn` seedNivaa=1, ellers laveste-rekkefolge ordinær). Auto-gen gjetter aldri (= B); F-G rød banner beholdes for null-ordinære-tilfellet.
- **⭐ Forward-compat:** overtid-klassifiseringen isolert i delt `packages/shared/src/utils/lonnsregel.ts` (`klassifiserArbeidstid` + `velgOvertidLonnsart`) — Nivå 1-2 lønnsregel-konfig kan bytte regel-kropp uten call-site-endring (se [BACKLOG § Lønnsregel-konfig](BACKLOG.md)).
- **Fallback:** amber banner i `[id].tsx` når auto-utkast nådde norm men firmaet mangler overtid-lønnsart — aldri feil-match, aldri stille drop.

**Migrering** `20260705120000_lonnsart_overtidsnivaa` (db-timer): `ADD COLUMN overtidsnivaa` (nullable, metadata-only) + ③a-backfill (kun seed-navn, diskriminerende WHERE) + ③b-backfill (NOT EXISTS-guard). **Prod-deploy careful** (to-stegs, `prisma generate` mellom `migrate deploy` og build, migrerings-gate `/sitedoc`). i18n: 6 nye nøkler nb+en + auto-oversatt til 13 språk. Typecheck rent (shared/db-timer/api/web; mobil-baseline urørt). **A.Markussens overtid-lønnsarter (170/172/175/177) settes manuelt i admin-UI før deres auto-gen stoles på.** Mobil-del rir neste EAS-batch. **Reload:** Expo JS/TS. Detaljer: [timer.md § Overtid-klassifisering](timer.md).
