...
- **⚠️ Deploy-gate (1a+1b+2+3):** terminal-`aktivPosisjon` (1a) + HMS-posisjon-2 (1b) + non-terminal backfill (2) er ikke dataverifisert (sandkasse tynn); **ruting/authz (3) er bevist på ren-funksjons-nivå, IKKE full tRPC-e2e mot DB.** → **Ved test-deploy: (a) spot-sjekk backfill mot test-DB, (b) live-verifiser rutingen som innlogget** på en flyt med DISTINKTE personer per ledd (Send treffer neste ledd, Besvar går tilbake til utfører, godkjenn krever ballen — pilot-fiksen). FØR prod.
