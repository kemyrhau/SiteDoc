...
- **🛑 ESKALERT TIL COWORK — mobil rad-sletting propagerer ikke** (S3): lokal `db.delete` + `.run()` sletter lokalt, men `syncBatch` `deleteMany({id:{in:<payload>}})` inkluderer ikke slettede id-er (`dagsseddel.ts:4025-4026`: «SLETTING propagerer ikke automatisk») + pull-apply re-innsetter fra server (`timerSync.ts:591-602`) → raden kommer tilbake ved reload. **Rammer timer/maskin/tillegg likt** (kandidatmengde: hele `apps/mobile/src` + `apps/api/src/routes/timer` — ingen tombstone/deleted-ids for sheet-rader). Krever synk-mønster-arbeid → cowork-eid. Se [BACKLOG](BACKLOG.md).
