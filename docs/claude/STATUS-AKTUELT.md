...
|---|---|---|---|---|---|
| **Firmarolle-konsolidering** | `SiteDoc-mobil-device` | `fase1-firmarolle-vakt` | `apps/web/src/kontekst/firma-kontekst.tsx` | 2026-08-10 | 🟢 **Fase 1 (divergensvakt) kodet + browser-verifisert** — Mathias-tilstand (user + firma_admin) warner (b), samstemt bruker tier. Egen gate før Fase 2 (`kanAdministrereFirma` overalt; 8 kode-lesninger, 18 kommentarer). Migrering ikke nødvendig (`admin.ts:455` er Fase 3). **Navngitt oppfølger (egen sak):** multi-firma firma-admin — `valgtFirma` settes ikke ved >1 medlemskap (`firma-kontekst.tsx:83-87`) ⇒ all firma-gating dør; krever firma-velger m/lagret valg + `hentBrukersOrg`-primærorg. |
...
| **Seed manglende firmakatalog** | `SiteDoc-utlegg` | `feat/seed-manglende-katalog` | `apps/api/src/services/seed/index.ts` · `apps/api/src/routes/admin.ts` · `seedManglende.test.ts` · `docs/claude/timer.md` | 2026-08-10 | 🟢 **Kodet, diff-klar (fra develop).** `admin.seedManglendeFirmakatalog` (sitedoc_admin) — idempotent, **kun `expenseCategories`** (A.Markussen prod: 0 utleggskat.). Enhets-testet 0→5/re-kjør=hoppet. Build 2/2. Ingen migrering. Oppfølger: robuste seedNivaa-guarder + settFirmamodul-wiring; prosjektmodul-variant (998/RUH). **Ingen prod — Kenneth kjører** |
