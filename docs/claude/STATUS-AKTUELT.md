...
- **⚠️ Deploy-gate (1a+1b+2):** terminal-`aktivPosisjon` (1a) + HMS-posisjon-2 (1b) + non-terminal backfill-script (2) er kjøringsverifisert men IKKE dataverifisert (sandkasse tynn) → **spot-sjekk/kjør mot test-DB ved test-deploy** før prod.
- **Gjenstår:** Fase 3 server-omskriving (11 statusskriv → avledStatus, 3 rutingveier → delt utledning, isValidStatusTransition→retningsregler, fjern 1b B-gaten, firmaBehandleAvvik→↔ terminal-only) — **live ruting endres, gatede delsteg** · Fase 4 web+mobil-paritet (inkl. mobil HMS-flytlinje-skjul, bytt konsumenter til posisjon-basert harBallen) · Fase 5 e2e + prototype-verifisering (distinkte personer).
