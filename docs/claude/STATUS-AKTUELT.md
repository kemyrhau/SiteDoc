...
| **Dataeksport (server-side dokumentgenerering)** | `SiteDoc-mobil-device` | `feat/eksport-infrastruktur` (fase 1) | `packages/db/prisma/schema.prisma` + migrering `20260811160000` · `apps/api/src/services/eksport/*` · `apps/api/src/routes/eksport.ts` · `tilgangskontroll.ts` · `router.ts` · `server.ts` · `apps/api/package.json` (`archiver`) | 2026-08-11 | 🟢 **Fase 1 (infrastruktur) diff-klar** (ORDRE blokk 28, fabel-godkjent retning). Ny `EksportJobb`-tabell (kjerne-db, kanon-jobbtabell som `FtdTranslationJob`; CHECK på type+status; **ikke batch-forutsettende** — `type` skiller pakke/enkeltdokument for fase 3). Poll-worker (`eksport-worker.ts`, mønster fra vegvesen-worker: recovery + poll + watchdog + **utløpsrydding** 7 dager) startet i `server.ts`, **én jobb om gangen**. `archiver` **stream-zip til disk** under `uploads/privat/eksport/` → beskyttet av eksisterende signatur-gate. `byggEksportArkiv` (fase 1 = manifest-konvolutt + LES-MEG; fase 2 fyller innhold). Nedlasting via kortlevd `signerFilSti`. **`verifiserKanEksportere` på ETT sted** (prosjektadmin+firma-admin, klar for stramming i abonnements-«halen»). 4 unit-tester. Migration validert lokalt. **Migrering gates av Kenneth.** **Fase 2** (filer+manifest+CSV) etter fase 1-gate; **fase 3** (PDF-renderer, egen container) venter fabels mal-mockup. **Ingen prod.** |
