...
- **✅ F4-4 (projectId-poison i mobil-synk) — develop 2026-07-11, gatet, venter prod før #38:** `syncBatch`-input krevde sedel-nivå `projectId: z.string().uuid()`, men mobil sendte `sedel.projectId` som for en tom/plassholder-sedel er `""` (pull-plassholder `serverSedel.projectId ?? ""`) → Zod 400 på HELE batchen før prosedyren → gift-isolering avviste sedelen. Fiks: sedel-nivå projectId er en fallback-shim (T.1: rad-nivå er kanon) → `z.union([uuid, ""]).nullable().optional().transform(v => v || null)` (tåler `""` for #37 + `null` for #38 → datatap-fri bakoverkompat); `verifiserProsjektmedlem` betinget på non-null; ny `radProsjekt`-resolver + synlig `avvist`-guard for rad uten prosjekt (DB NOT NULL); mobil push sender `sedel.projectId || null` + rad-fallback `|| undefined` (aldri `""`). Gift-isolering (SYNC-1, `timerSync.ts:406`) bekreftet å redde gode sedler ved 400. **Gate:** typecheck 0 nye feil (mobile-baseline 11 uendret, api/web/shared grønt) + shared-test 44/44 + web-build 59/59. Ingen migrering. **Rører `apps/api` → MÅ prod-deployes** (server) + mobil-delen når via #38. Se [timer.md § Synk-identitet](timer.md).
- **✅ F4-3 (rå `{{dato}}` i mobil attestert-tittel) — develop 2026-07-11, mobil-only:** `apps/mobile/app/timer/[id].tsx:587` kalte den delte nøkkelen `timer.detalj.attestertTittel` («Attestert {{dato}}») uten `{dato}`-arg → rå placeholder. Datoen vises alt separat rett under (`formatTidspunkt(sedel.attestertVed)`, :592, dag+måned+tid). Fiks: mobil bruker nå den eksisterende rene nøkkelen `timer.status.attestert` («Attestert») — datoen én gang, korrekt formatert. Delt nøkkel **uendret** for web (`page.tsx:441` sender `{dato}` og har ingen separat dato-visning). Ingen i18n-tillegg (gjenbruk), ingen migrering, **rører ikke `apps/api`** → når via EAS #38, ingen prod-deploy. 
- **✅ F4-1b (arbeider-mobil-mutasjoner NOT_FOUND på pre-invariant sedler) — develop 2026-07-11, gatet, prod-deploy før #38:** mobil sender lokal id (= `clientUuid`, jf. F4-1 pull-M2), men `hentEgenDagsseddel` (`dagsseddel.ts:120`) slo opp `findUnique({ id })` på server-PK. For sedler laget FØR F4-1-invarianten (`id = clientUuid` ved create) er `id ≠ clientUuid` → «Dagsseddelen finnes ikke» på alle ~14 arbeider-kallesteder (mobil-nåbar: `gjenaapneDagsseddel`). Fiks: `hentEgenDagsseddel` slår nå opp `id` FØRST, faller tilbake til `clientUuid` (begge `@unique`, ingen migrering). Bakoverkompat: gammel klient/web sender server-id → treffer id; ny mobil → treffer clientUuid. **Rører `apps/api` → prod-deploy før #38.** Latent oppfølger (BACKLOG): `oppdater`/`tilfoy*` skriver på rå input-id (web-only i dag).
- **✅ F4-2b (gjenåpne-knapp i lederens SeddelKort) — develop 2026-07-11, web-only, gatet:** F4-2 la gjenåpne-knappen i `AttesteringDetalj`, men lederens firma-attesterings-**Attestert-fane** rendrer `SeddelKort` i read-only (`page.tsx:159`, `readOnly = aktivFane === "accepted"`) → alle handlinger inkl. detalj-lenken skjult → knappen der nås aldri (og prosjekt-nivå `hentTilAttestering` viser kun `sent`). Fiks: gjenåpne-handling **på selve kortet** (`SeddelKort.tsx`), vist kun ved `readOnly`, ekte `<Modal>`-bekreftelse, kaller eksisterende `gjenaapneAttestering` + invaliderer `hentTilAttesteringFirma`+`hentTilAttestering`. Gjenbruker F4-2s i18n. Ingen api/DB. **Deployes med F4-2-server-delen** (prod). `AttesteringDetalj`-knappen beholdes (korrekt `accepted`-guard, direkte-URL-fallback).
- **✅ F4-2 (accepted-deadlock) — develop 2026-07-11, gatet, venter prod før #38:** leder hadde ingen web-vei til å angre en fullført attestering (`attesterRader` flipper sedel→`accepted` når alle rader attestert → attesterings-/retur-knapper gated på `status==="sent"` forsvinner; server låser også retur på `accepted`). Fiks (retning **B**, gjenåpne til `sent`): ny leder-mutasjon **`gjenaapneAttestering`** (`apps/api/.../dagsseddel.ts`, input `{sheetId}`, precondition `accepted`→`PRECONDITION_FAILED`, leder-auth unike `projectId`+`krevProsjektLeder`, tx: rader→`pending` + sedel→`sent`, nullstiller attestert-felter) + web «Gjenåpne attestering»-knapp i `accepted`-grenen (`AttesteringDetalj.tsx`) med ekte `<Modal>`-bekreftelse. i18n `timer.attestering.gjenaapne.*` (nb+en+13 auto). **Rører `apps/api` → MÅ prod-deployes** (med F4-1). Ingen migrering; ny mutasjon (bakoverkompat). Se [timer.md § Gjenåpne attestering](timer.md).
- **✅ F4-1 (identitetsforsoning) — develop 2026-07-11, gatet, venter prod før #38:** rotårsak til «mobil-registreringer vises ikke på web» = identitetskollisjon `@@unique([userId, dato])` vs `clientUuid` (server-`id` ≠ `clientUuid` → dato-kollisjon ga `avvist` terminal + pull-duplikat + «pull-så-redigert»-P2002). Fiks (`apps/api/src/routes/timer/dagsseddel.ts` + `apps/mobile/src/services/timerSync.ts`): **invariant `id = clientUuid` ved create**; **S2** P2002→`conflict` m/ server-`clientUuid` (ikke `avvist`); **S3** `deleteMany` kun payload-rad-id-er → bevarer web-førte rader (**akseptert:** mobil rad-sletting propagerer ikke, ingen migrering); **M1** kollisjon-conflict = merge/re-nøkle + additiv push (ingen datatap); **M2** pull-guard mot duplikat via `(userId, dato)`-forsoning. **Gate:** typecheck **0 nye feil** (mobile-baseline 11 uendret, api/web/shared grønt) + shared-test 44/44 + web-build grønt. Bakoverkompat #37: `serverData.clientUuid` valgfritt → gammel klient faller til server-wins. **Server-delen MÅ prod-deployes før #38.** Se [timer.md § Synk-identitet](timer.md).
- **✅ F4-1c (visnings-dedupe av duplikat sedel-hode) — develop 2026-07-11, mobil-only, rent display:** Fase C-funn — cruft-enhet viste to sedel-hoder for samme dato (tom «Arbeidstimer (0.00t)»-plassholder + populert 3.50t). **Diagnose:** pre-F4-1-relikvi (gammel pull matchet kun på server-`id` → innsatte ekstra tomt server-hode ved siden av pending offline-sedel). F4-1 (M2) stopper **nye** duplikater; eksisterende pending-cruft selvhelbreder via M1 ved neste push, men avvist/stuck-cruft persisterer + er visuelt urovekkende. Fiks: visnings-dedupe i `DagsseddelListe.tsx` (`dedupPerDato` + `lesDagssedlerLokalt`) — kollaps hoder med samme dato, skjul kun **tom** plassholder (innhold = timer/tillegg/maskin-rader), ekte divergens (≥2 med innhold) viser begge med `AlertTriangle`-markering (aldri stille datatap). **Rent display: ingen DB-skriving, ingen pull-endring** — merge-autoriteten blir i M1 som designet. Ingen i18n-tillegg, ingen migrering, **rører ikke `apps/api`** → når via EAS #38, ingen prod-deploy. **Gate:** typecheck 0 nye feil (mobile-baseline uendret, fingeravtrykk-verifisert).
- **✅ F4-1d (web-førte rader usynlige for mobil — touch-parent) — develop 2026-07-11, server-only, prod-deploy før #38:** Fase C-funn 1 — web-opprettet utkast med rad (Markussen B12, 4t) synket hodet til mobil, men 0 rader (stabilt etter re-sync). **Rotårsak:** rad-mutasjoner skrev barn-rad uten å røre forelderen → Prisma bumper `DailySheet.updatedAt @updatedAt` KUN når sedel-raden selv skrives, så `hentEndringerSiden` (`dagsseddel.ts:3202`, delta-vindu `updatedAt > sistSynk`) ekskluderte sedelen etter at hodet var synket → raden nådde aldri mobil (full pull ville vist den — inkrement ikke). Fiks: delt `touchSedel(prismaTimer, sheetId)`-hjelper (`dagsseddel.ts`) som bumper `updatedAt` i **samme `$transaction`** som rad-writet. **Full kandidatmengde enumerert** (F4-1-lærdom — grep beviser ikke dekning): touch lagt på 14 stier — `tilfoy/oppdater/fjern` × `timer/tillegg/maskin`, `tilfoy/fjernTilleggVedlegg`, `flyttTimerRadEco`, `splittRad`; `redigerSedelRader` gjort **ubetinget** sedel-touch (var betinget på `pauseUpdate` → leder-radredigering uten pause-endring glapp). Allerede dekket (rører sedelen selv): `opprett/oppdater/send/gjenaapneDagsseddel/returnerRader/gjenaapneAttestering/attester/returner` + **`syncBatch` (dailySheet.upsert — IKKE dobbelt-touched)**. Bevisst ekskludert: `attesterRader` (betinget sedel-touch, men per-rad `attestertStatus` synk-es ikke til mobil + sedel-status bumpes ved terminal → ingen mobil-delta-glipp). **Gate:** api-typecheck 0 feil + web-build grønt. Ingen migrering (bruker eksisterende `@updatedAt`). **Rører `apps/api` → MÅ prod-deployes før #38.** Se [timer.md § Synk-identitet](timer.md).
