...
| **dokgen** | `SiteDoc-dokgen` | 🟡 **PÅGÅR** — `feat/kollapsbar-seksjon` @ `95a94735` (ingen egne commits ennå, ikke pushet) | «Kan slås sammen» på overskrift/undertittel. 🔴 **REBASE NØDVENDIG:** branchen står på `95a94735`, men develop er nå `71202903` etter runde 90 (som slettet 16 `maler.*`-i18n-nøkler). Dokgen legger nye i18n-nøkler — **cowork må varsle ham om å `git fetch && rebase origin/develop` FØR han bygger videre.** Tidligere: papirkurv-forhåndsvisning merget runde 85 (`652cdbda`) |
...
| **merge-agent** | `SiteDoc-merge` | 🟢 **LEDIG** — `merge-restart` @ develop | Runde 90 sist: merget «fjern Rapportmaler-flata» (`71202903`). Web-deploy samles |
