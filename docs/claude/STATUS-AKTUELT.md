...
> 5. **Timer web-vs-mobil paritet D1–D8 — LANDET develop 2026-07-09** (bolk a `b3230944` = D7/D1/D2/D3, bolk b `0985d46e` = D4/D5/D6, bolk c `7797a9b5` = D8, **bolk (d) 2026-07-09 = reglene bak fra/til-feltene: R1 prefill (siste rads tilTid ?? effektiv start; til = effektiv slutt) + `pauseFra` = effektiv skiftstart, R2 fra–til i radlista, R3 pauseOverlapp-transparens, R4 T.5-runding ved commit**). Web arbeider-flate speiler nå mobil (app = fasit). **Funksjonell test-verifisering web ✅ ferdig 8/8 (D1–D8).** **Venter kun prod-deploy.** **Gjenstår separat (BACKLOG):** D9 sesong-dagsnorm, GPS-geoforslag, D2-oppfølger (rad-flytting mellom prosjekter ved redigering — velger låst i dag), maskin-rad-prefill-avvik (web ≠ mobil), `pauseBeregning.ts`-duplikat (mobil + shared). Full detalj: [BACKLOG § Timer web-vs-mobil paritet](BACKLOG.md).
