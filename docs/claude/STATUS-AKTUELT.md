...
| **redesign** | `SiteDoc-redesign` | 🟡 **PÅGÅR** — `feat/hent-fra-arkiv`, «Hent fra arkiv» steg 1–2 | Fabels mockup + fjern to-nivå-hopp + stram lesing av sentralarkivet. 🟢 **Fabels svar på beslutning 1–3 committet `docs/redesign/svar-hent-fra-arkiv-beslutninger-fabel-2026-09-12.md`** (runde 87) — redesign kan fortsette. 🔴 **Coworks `inbox-hent-fra-arkiv.md` er ERSTATTET av fabels `avvik-arkivinngang-ordre-fabel-2026-09-12.md`** (committet runde 86) — coworks versjon skal IKKE relayes. Fabels er mer presis: knapp også på `/dashbord/[prosjekt]/maler`, `FirmaarkivVelger` flyttes (ikke dupliseres), låst SiteDoc-fane har forklaringstekst. ⚠️ **Scoping (cowork):** steg 1–2 (knapp/modal/faner/hengelås) bygges nå; **steg 3 versjonspublisering er egen runde** — se åpen tråd under |
...
| **merge-agent** | `SiteDoc-merge` | 🟢 **LEDIG** — `merge-restart` @ develop | Runde 86 sist: merget KB4 v2 (seed-bibliotek, create-only bekreftet) |
