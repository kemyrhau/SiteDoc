...
- **Gjenstår F3-UI:** kombinert prosjekt+byggeplass-pageSheet + rad-sekundærlinje «Prosjekt · Byggeplass» + override-hint + tråd `byggeplassId` gjennom `onLagre`→insert.
- **Gjenstår F5:** `pauseMin` lokal+server-migrering (additiv, backfill-disiplin) + bærer ved generering + flytt+recompute + rad-checkbox/toast/ekte-bekreft-modal. **Modul-avhengighets-verifisering (maskin B1-pause) FØR F5 låses.**
- **Gjenopptak:** sjekk ut branchen; begge lokale commits venter objekt-gate før push. develop-Opus exitet ved dette rene sjekkpunktet.
