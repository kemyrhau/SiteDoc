...
| **redesign** | `SiteDoc-redesign` | 🟢 **LEDIG** — `8c079fa6` detached (merget commit) | «Hent fra arkiv» steg 1–2 + funn 4 **merget runde 87 (`a517c3ec`)**: `HentFraArkivModal` (firma-/SiteDoc-faner, `arkivTilgang`-gate), `BibliotekPanel` slettet (to-nivå-hopp fjernet), Rapportmaler-flata rendyrket til lese-og-bruk. api 458→462 (4 arkivtilgang-tester). BACKLOG §2359/§2371 lukket. 🔴 **Steg 3 versjonspublisering er egen runde** (Kenneth-gatet skjemaendring). ⚠️ **Ikke lukket som ferdig før pilotsjekken — se åpen tråd** |
...
| **merge-agent** | `SiteDoc-merge` | 🟢 **LEDIG** — `merge-restart` @ develop | Runde 87 sist: merget «Hent fra arkiv» (`a517c3ec`), utløser web-deploy |
...
- 🔴 **Pilotsjekk «Hent fra arkiv» (åpen etter runde 87):** dekker Sitedoc Myrhaugs firmaarkiv pilotens NS 3420-maler? Prosjektadmin mister direkteimport fra NS 3420-biblioteket (tilsiktet vedtak) — pilotsjekken er det eneste som kan gjøre det til et problem. **Kenneth kjører den; ingen agent når DB.** «Hent fra arkiv» skal IKKE lukkes som ferdig før den er svart
