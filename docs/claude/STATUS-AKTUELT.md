...
| **redesign** | `SiteDoc-redesign` | 🟡 **PÅGÅR** — `fix/fjern-prosjektmaler` | Fjerner `[prosjektId]/maler` med redirect + kapittelgruppering i «Hent fra arkiv». 🔴 **VEDTAK SNUDD:** runde 87 valgte **vei (a)** (rendyrk flata til lese-og-bruk); **Kenneth + fabel har snudd til vei (b)** (fjern flata) — den viser samme data som `oppsett/produksjon`, bare uten fagområde/punkter/prefiks/versjon, altså ingen unik funksjon. *(a)-linja beholdt her så neste leser vet hvorfor vi ikke går tilbake.* Fabels endringsordre committet `docs/redesign/endringsordre-fjern-prosjektmaler-fabel-2026-09-12.md` (runde 89). Forrige: dobbelt-lån-vakt merget runde 88 (`78a6bbbf`), «Hent fra arkiv» runde 87 (`a517c3ec`). 🔴 **Flere åpne tråder — se under** |
