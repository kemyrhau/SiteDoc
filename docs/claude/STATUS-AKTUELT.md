...
- **Gjenstår:** Fase 4 web+mobil-paritet (bytt konsumenter til posisjons-basert `harBallenPosisjon`+delt utledning, konsolider `flyt-ledd.ts`/`dokumentflyt-ledd.ts` mot shared, UI-tekst «Send til N·X»/«Godkjenn og fullfør», mobil HMS-flytlinje-skjul) · Fase 5 e2e + prototype-verifisering (distinkte personer). **Fase 3-Opus compacter her — gjenoppta fra denne entryen + `relay/STATUS.md`.**
