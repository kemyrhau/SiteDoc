...
> 4. **Timer pause-tid-synk (`f385ba99`, develop):** landet + migrering test-verifisert. **Gjenstår:** funksjonell test-verifisering — mobil ✅ verifisert i simulator (innstilling + attestering + 07:00→11:00), web ✅ verifisert 8/8 (D1–D8, se tråd 5); Piece 2 (1b auto-utkast fra/til), midnatt-wrap-bug, rydd duplikate pause-endringer fra `redesign/navigasjon`-hovedtre. Mobil-UI via neste EAS-batch (tråd 1).
> 5. **Timer web-vs-mobil paritet + gyldighet — LANDET develop 2026-07-09** (bolk a `b3230944` = D7/D1/D2/D3, bolk b `0985d46e` = D4/D5/D6, bolk c `7797a9b5` = D8, bolk (d) `bf78889c` = R1–R4 fra/til-regler). **Ny denne økta (alle på develop):** **bolk (e) `f101890e`** = pause-bevisst maskin-rad B1–B4 (maskin trekker lunsjpause, spenn↔antall-synk + hard sperre, B3 init-antall, B4 web-prefill kanonisk) + web timer-`forventet==tall`-sikkerhetsnett; **`10622ee3`** = `tilFraAntall`-grensefiks (pausevindu-inngang) + vitest; **bolk (f) `f59a498c`** = «Gjenåpne dagsseddel» web + **attestert-vakt på server** (`gjenaapneDagsseddel` avviser når ≥1 rad attestert → be leder returnere) + **`1deaff6b`** slett-modal (`confirm()`→ekte Modal); **bolk (g) `79e786a3`** server = `fra<til`-superRefine (4 mutasjoner) + **overlapp-vakt** (hard sperre: én arbeider kan ikke være to steder — timer-rader kan ikke overlappe på tvers av prosjekt/ECO) + **`c81c4eae`** klient = hele-sedel-prefill (seneste `tilTid`) + prefill-gyldighet + 0==0-lukking. Web arbeider-flate speiler nå mobil (app = fasit) + hard server-gyldighet. **Test-verifisert web 8/8 (D1–D8).** **Venter kun prod-deploy.** **Gjenstår (BACKLOG, alt → neste EAS-batch):** mobil bolk (e) B1–B4 + bolk (f) gjenåpne-bekreftelse/`PRECONDITION_FAILED`-feilmapping + bolk (g) prefill-scope/`fra<til`/0==0; maskin-vs-maskin-overlapp (utredning); `pauseBeregning.ts`-mobil-dedup (mobil-kopi har `tilFraAntall`-feilen); `sedel.pauseMin` dekorativ; dagsnorm-varsel-vs-B2; `sheet_machines` seed timer≠spenn; D9 sesong-dagsnorm; GPS-geoforslag; D2-oppfølger (rad-flytting). Full detalj: [BACKLOG § Timer web-vs-mobil paritet](BACKLOG.md).
