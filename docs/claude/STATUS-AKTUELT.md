...
|---|---|---|---|---|---|
| **Modul-onboarding (seed-policy)** | `SiteDoc-mobil-device` | `naa-sjekk-modul-onboarding` | `packages/db/prisma/schema.prisma` + migrering `20260811120000` · `apps/api/scripts/backfill-seed-policy.ts` · `arkitektur-syntese.md` · `terminologi.md` | 2026-08-11 | 🟢 **Steg 1 (migrering + backfill) diff-klar** (fabel-retning modul-onboarding, ORDRE blokk 19). Ny tabell `OrganizationSeedPolicy` (per datatype: `standard`\|`egen_katalog`, CHECK-håndhevet begrunnelse) i `packages/db` (bevisst kjerne-unntak, dok. i terminologi.md). Backfill-script dry-run/apply, idempotent, lokalt validert (kategori 1 A.Markussen-lonnsart + kategori 2 timer-aktiv+0-katalog). **🔴 Backfill-LISTE rapporteres + godkjennes FØR --apply; Kenneth gater migrering + kjøring — ingen prod.** Steg 2–5 (guard-robusthet → dispatch → varelager-hook → status-3-verdi) venter steg 1-gate. **maskin: ingen seed-hook** (kategori/type er enums, ikke katalog — egenskap, ikke mangel). **Navngitt oppfølger (fabels «opprydding»):** `aktiverTomKatalog` bør selv skrive `egen_katalog`-policy-rader ved fremtidige kall (ikke bygget nå). |
