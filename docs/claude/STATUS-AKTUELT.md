...
| **dokgen** | 🟢 **LEVERT — venter merge** | `SiteDoc-dokgen` | **Dokumentsøk mobil** (`31002232`): delt filterpredikat (3 web-flater), søkemotor til shared, sheet m/søk+filter+sortering. Tidligere: serverlås SJA, signatur navn+tidspunkt, byggeplass-tilhørighet, utlogging + byggeplass-velger | 🔴 **Ikke simulator-verifisert** — modal-over-modal på iOS + mulig refetch-flimmer ved første søketegn |
| **redesign** | 🟡 **ARBEID** | `SiteDoc-redesign` | SJA-signaturrunder (`e2e87123`) · bekreftet-av + fritekst-lokasjon (`333359a6`) · seed + skjermbilde-underlag. TS2589 løst ved kilden | `fix/innboks-og-dokumentflyt` — Innboks-pila (variant A) + måler «Se alle»-grensen |
