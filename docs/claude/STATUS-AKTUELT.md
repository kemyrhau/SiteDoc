...
|---|---|---|---|---|---|
| **App-felt (vær + bildeNr)** | `SiteDoc-mobil-device` | `feat/app-vaer-bildenr` | `FeltDokumentasjon.tsx` (web+mobil) · 4 skjema-hooks (`bildeNr`) · `rapportobjekter/typer.ts` · `packages/shared/src/utils/bildeNr.ts` · (Sak A: `useAutoVaer.ts` + mobil-prefyll — pågår) | 2026-08-16 | 🟢 **Sak B (`bildeNr`) klar til commit:** tildeles ved opptak i alle fire hooks (sjekkliste/oppgave/HMS deler sti), delt `nesteBildeNr`/`nummererRepeaterBilder` i shared, badge i thumbnail-UI, web-galleri `await`-fiks (sekvensiell → FileList-rekkefølge). typecheck 4 pkg + 470 shared-tester grønt. **🟡 Sak A (værsnapshot) REDESIGNET (Kenneth 2026-08-16):** ikke lenger baseline-vern — fjern mobil-prefyll av date_time (måles først: hvor bredt det slår ut / begrenses til vær-koblede felt), umiddelbar henting ved «Nå»/tidsendring, offline-kø for henting mot LAGRET tidspunkt (én ventende per dok+felt), tre UI-tilstander. Ikke bygget — venter måling + design-OK. **⚠️ Fil-overlapp med «S1 Fase 1b» steg 2 på web `FeltDokumentasjon.tsx` — sekvensér merge.** Ingen migrering. **Ingen prod.** |
