...
|---|---|---|---|---|---|
| **Utlegg-ordningsmodell** | `SiteDoc-utlegg` | `feat/utlegg-ordningsmodell` | U1: `utleggOrdning.ts` · `db-timer/schema.prisma` + migrering. U3: `apps/api/.../timer/expenseCategory.ts` + `dagsseddel.ts` · `apps/web/.../timer/[id]/page.tsx` · i18n · `docs/claude/timer.md` | 2026-08-08 | 🟢 **U1 pushet. U3 web-registrering kodet** (velger 8a + tre radformer 8b + kilde-linje; build 2/2, typecheck rent). U2 utsatt. Neste: U4 mobil. **E2E-test venter på at U1-migrering kjøres på test** |
...
### Utlegg-ordningsmodell U1 + U3 (branch `feat/utlegg-ordningsmodell`) — PÅ BRANCH, venter merge (IKKE prod)
...
**U1 (kodet + gatet + pushet):** Tre ordninger utledes, aldri valgt av feltarbeideren: `sats` (→SheetTillegg, lønnsart) · `utlegg` (→SheetUtlegg, beløp+kvittering→refusjon) · `fakturert` (→SheetUtlegg, avhuking, `belop=null`, eksporteres aldri). Levert: (1) delt `utledOrdning` (`overstyring ?? firma-default`) + avledningshjelpere i `@sitedoc/shared` (10 tester). (2) migrering `20260808120000_utlegg_ordningsmodell` — `ExpenseCategory.ordning` + `sheet_utlegg`/`sheet_utlegg_vedlegg`/`prosjekt_ordning_overstyring` + **CHECK på `ordning_ved_foering`/`belop`** (integritetsbærer, ikke app-flagg). (3) `timer.md` drift-reconciliert. **Build:** full `pnpm build` 3/3, 441 shared-tester, `prisma validate` + `migrate diff`-verifisert.

**U3 — web-registrering (kodet, 2026-08-08):** mockup 8a+8b. API: `timer.expenseCategory.list` (utledet ordning + kilde per prosjekt) + `dagsseddel.tilfoy/oppdater/fjernUtleggRad` (`ordningVedFoering` stemplet ved insert; `sats` avvist — bæres av SheetTillegg) + `*UtleggVedlegg`; `hentMedId` returnerer `utlegg[]`. Web (`timer/[id]/page.tsx`): unified `LeggTilVelger` (én inngang, to grupper Tillegg/Utlegg, ↑/↓/Enter, ordnings-pille som undertekst), `UtleggRadDialog` (tre radformer + kilde-linje), `RaderUtlegg`. i18n nb+en (20 nøkler). **Tillegg-flyten (sats) er urørt** — samme mutasjon/validering/lagring til `SheetTillegg`; velgeren pre-velger kategori, resten bit-for-bit. **Build 2/2, typecheck rent (api+web).** **⚠️ Oppførsel før U5:** alle kategorier står på `ordning='utlegg'` → flaten viser kun utlegg-formen på ekte data; `fakturert`/`sats` verifiseres via manuelt satt ordning. **E2E-test forutsetter U1-migrering kjørt på test (Kenneths gate).** Navngitte oppfølgere: «bro ExpenseCategory→lønnsart» (sats-utlegg) · «U-admin: utlegg i attesterings-redigering» (arbeider-sti-only asymmetri).

**Gjenstår:** U2 eksport-guard (utsatt til Proadm-motor) · U4 mobil (kamera-primær, «Reload:»-plikt; koordiner m/ simulator-Opus HMS-spor via cowork FØR mobil røres) · U5 firma-admin overstyring-UI (**inkl. eksplisitt: (a) firma-admin må sette ordning per kategori; default er `utlegg`; (b) advar ved navnekollisjon på tvers av `Tillegg`/`ExpenseCategory` — to kataloger ⇒ samme navn kan finnes begge steder**) · U6 migrering av feilførte rader (egen gate; forutsetter Proadm-sti-avklaring).
