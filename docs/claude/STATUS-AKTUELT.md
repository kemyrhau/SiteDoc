...
> ✅ **DEPLOYET TIL PROD 2026-06-23 (prod-merge `13dc110e`)** — arkivert til [historikk-2026-06.md § Timer-bunke (UF-0…UF-4, fiks A, …)](historikk-2026-06.md). **Server live (API+web):** Funn #2 kvittering-vedlegg (+ migrering `sheet_tillegg_vedlegg` anvendt), T.11 maskinbevis, UF-4 recall (`gjenaapneDagsseddel` → 405), tre-veis-farge web, T.12 web-parity. **Mobil-UX** (UF-0…UF-4, fiks A sync-gift-isolasjon, tidshjul-modal + keyboard-«Ferdig» + grønn-boks-wording, U1–U3, build-footer, lønnsart-hint) **distribueres via TestFlight prod-bygg #30** (`404e1707`, submission `8b3f8ec1`, Queued) — **A.Markussen-validering pågår, ikke hos arbeidere ennå.** Verifisert: migrering+tabell (`to_regclass`), `gjenaapneDagsseddel` 405, innlogget prod-dashboard data-intakt (ikke anonym).
>
