...
**Flytrettigheter-evaluering + fabel-kryssjekk** ([delplaner/flytrettigheter-evaluering-2026-07-26.md](delplaner/flytrettigheter-evaluering-2026-07-26.md)) — fabel + cowork konvergerte uavhengig på H1/H2/H3. **Levert etterpå:** **Godkjent-stoppsted (H6)** merget `8204c171` (`approved→closed` fjernet — Godkjent lukkes aldri; `approved→draft` Gjenåpne lagt til; `in_progress→closed` beholdt for åpent doc/KS-avvik). **H3 videresend-rettighetslekkasje** — ✅ MERGET `a4697feb`. Default AV videresend for flyt-roller (kun prosjektadmin via statusmaskin-snittet), medlemskaps-filtrert `byggVideresendValg` (defensiv guard), og videresend-cellene rendret **låst** for flyt-roller (fabel-vedtak: admin-only + UI-ærlighet, ikke villedende av-bryter). Runtime `celleTillatt`-no-op for `forwarded`-override er inert (admin-only, per vedtak). **Flyt-posisjon i dokument-headeren** — ✅ MERGET `d3c73f6f`. Dynamisk ledd-rad (evolusjon av `FlytIndikator`; delt `flyt-ledd.ts` fjernet byggLedd-duplikat, `Ledd.rolle`-etikett, siste-ledd deaktivert Send + utveier bak `visUtveier`-prop så liste-celler ikke brekker) — svarer på Kenneths test-observasjoner: *hvor* i flyten + *hvorfor* Send ikke går ved siste ledd. **byggLedd-fiks** (merget): første versjon kollapset til én boks fordi `byggLedd` grupperte på `steg` (=1 for alle — H1 i praksis); fikset til kanonisk **rolle-rekkefølge** + rolle-bevisst `finnAktivtIndex` — **verifisert på EKTE render på test** (full ledd-rad, variant-C kun ved siste ledd, Godkjent uten Lukk). **Lærdom:** real-render-gate på ekte data (ikke artefakter/syntetiske tester); `deploy-test.sh` rsyncer LOKAL develop → pull lokalt før deploy. **Flytvisning-fane (konfigurator)** — ✅ MERGET + deployet test + verifisert grønt av Kenneth 2026-07-26. Projeksjon over matrise-cellene (rolle·fraStatus·tilStatus), grupperte delceller per handling (Gjenåpne/Videresend/Besvar), én kilde med matrise-fanen (amber-prikk/Tilbakestill/hengelås per delcelle). Fabel render-gate grønt. Kosmetisk header-linjebrekk → [BACKLOG](BACKLOG.md). **E2e-rigg** — ✅ DoD NÅDD: Playwright røyksuite (`tests/e2e/`, 8 tester) **3× grønn på rad** mot test.sitedoc.no (2026-07-26). Testid-lag + suite + `seed-e2e-flyt.ts` (idempotent, AGENT-TEST-0001-scoped) på develop; commit 3 (`5e664cef`, selv-helende pre-clean + faggruppe-rutet assert) inert testinfra, merges bundlet med doc-sync (dev-login-agent.md + spor1-ci-ordre.md). GitHub Actions-kobling = eget beslutningspunkt etter ~1 uke stabil. Justering: `venter-paa`-assert fjernet (faggruppe-ruting → chip legitimt fraværende i dag; fabel-avgjort at chippen SKAL vises for faggruppe → BACKLOG + testkandidat #9). **Gjenstår:** F6-oppfølger (received→approved default — antatt trygg), posisjonsutredning (H1/N-boks — `steg`-feltet finnes → nærmere enn antatt), registrator-steg-1-validering, H2 (utførers venstre-send/tilbake-kant), besvar=venstre-modellspenning. N-boks utsatt.
