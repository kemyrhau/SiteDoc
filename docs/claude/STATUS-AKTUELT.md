...
### hentEndringerSiden «erstattet»-lekkasje → historikk-tabell (fiks B) — venter gate + migreringstillatelse (2026-07-13)
...
`pnpm --filter @sitedoc/api build` **grønt**. Diff-output: ny tabell + migrering + 2 mutasjoner + 1 include + helper `byggHistorikkPost`. **Migreringen kjøres IKKE uten Kenneths eksplisitte go (DB-skjemaendring).** Verifisering etter deploy+migrering: fresh mobil-pull av `49a7c839` → 3 timer/2 maskin (ikke 9/6) + DB-sjekk at «erstattet»-radene er FLYTTET til historikk (ikke tapt).
