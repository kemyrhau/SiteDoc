...
| **redesign** | `SiteDoc-redesign` | 🟢 **LEDIG** — `c4953991` detached (merget commit) | Typefilter på SiteDoc-fanen + rollestyrte bunntekst-lenker **merget runde 94 (`ecd390c6`)**: klient-typefilter (`arkiv-fane-filter.ts`), `kanRedigereFirma`/`kanRedigereSitedoc` fra `autoriserMalTilgang`, forklarende tom-tilstand. `bibliotek.ts` kun `select` (`kategori`/`domene`), ingen serverfilter. api 465→468 (+3 rediger-signal), web 223→228 (+5 typefilter, m/eksklusjons-asserts). 🔴 **Nytt funn IKKE fikset: `faneWhere` (`firmamal.ts:64-77`) mangler negativkontroll — egen runde.** Forrige: fjern Rapportmaler-flata runde 90 (`71202903`) |
...
| **merge-agent** | `SiteDoc-merge` | 🟢 **LEDIG** — `merge-restart` @ develop | Runde 94 sist: merget arkivmodal-typefilter (`ecd390c6`). 🔴 **OTA-gjeld runde 91 (kollaps) + 93 (iOS-PDF) — én OTA dekker begge.** Web-deploy samles |
