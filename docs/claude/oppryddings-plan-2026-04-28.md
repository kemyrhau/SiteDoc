...
- [ ] **2A. C-1 (revidert): mannskap.md — restrukturer som vy-beskrivelse i PSI-konteksten** — `mannskap.md` sier «prosjektmodul» (treffer ikke anker etter 1D — vyer er ikke moduler). Handling-alternativer: (a) restrukturer mannskap.md til å beskrive mannskaps-vyen + PSI-utvidelser (innsjekk/utsjekk-mekanikk), (b) flytt innholdet til ny seksjon i PSI-relatert dokumentasjon og arkivér mannskap.md. Eksakt form: foreslås før commit.
- [ ] **2B. C-2 (revidert): forkast firma-eid PSI-utvidelse** — arkitektur-syntese § 3.4 sier PSI får `organizationId` + `projectId` nullable + `kontekstType`. Per Kenneth-presisering 2026-04-28 (1D): PSI er ren prosjektmodul med `projectId` alltid required. Handling: rens § 3.4 + fase-0-beslutninger § E-rader som berører PSI-utvidelse. PSI-utvidelse for innsjekk/utsjekk (per B1) erstatter denne.
- [ ] **2C. C-3 (revidert): erstatt «Mannskap/PSI slått sammen» med «PSI med mannskaps-vy»** — arkitektur-syntese § 1.2 sier «slått sammen». Per ankret er Mannskap ikke modul — det er vy i PSI. Handling: omformuler tabell-rad, endring-bullet, Fase 4-overskrift, § 11 ref-tekst. Bakes naturlig med 2A.
