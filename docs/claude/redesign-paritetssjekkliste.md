...
| P19 | Dokumentleser (Reader View + språkvelger + sammenlign) | `/[pid]/dokumentleser`, `/dokumenter/[id]/les` | 2c (mobil) / bevares (web) | `kreverProsjekt` | Nei | 🔨 2c mobil bygget (flagg på); web uendret |
...
| M10 | Dokumentleser + språkbytte | `dokument/[id].tsx` | **2c** (språkpille-rad + sammenlign) | ett-trykk språkbytte | 🔨 2c bygget (flagg på) — venter simulator |
...
| (vii) 2c leser | 🔨 bygget | Bak `useNyNavigasjon` (RN), kun `apps/mobile/app/dokument/[id].tsx`. Plan godkjent m/ 3 fabel-valg (ingen brødsmule v1 · bunn-ark-sammenlign · motor droppet fra banner). **Sticky språkpille-rad** (horisontal scroll, touch-mål 44px): kilde-pille m/ «· kilde», aktiv = fylt blå, språk under oversettelse = amber «…» m/ 8s-poll til grønn. **`+ Oversett`-pille** → bunn-ark m/ språkvalg → ny `mappe.oversettDokument` (upserter pending-jobb, **rører ikke `config.motor`** — worker bruker prosjektets default; i motsetning til `reOversettDokument`). **Grønt banner** «Oversatt til {språk} · vis original» (motor droppet i v1 per fabel — prosjekt-default kan være usann; backend-oppfølger: motor-per-jobb). **Sammenlign** = topp-lenke → modus-hint-banner «trykk et avsnitt» + «Avslutt» → avsnitt trykkbare → bunn-ark m/ gjenbrukt web-logikk (`modul.sammenlignOversettelse` + `reOversettDokument`, motorbytte). **Brødsmule utelatt v1** (backend-oppfølger folderId→sti). Backend: `hentDokumentBlokker` utvidet (`sourceLanguage`, `projectId`, `jobber[]`). i18n 17 nøkler × 15 språk (fr/pl `{{spraak}}`-placeholder rettet manuelt). Mobil-typecheck ren for fila. Flagg av = dagens leser uendret. **Simulator-verifisering (idb, 2026-07-07): 6/8 fanget** i `docs/redesign/screenshots/2c-simulator-2026-07-07/` (01 flagg-av · 02 kilde-pille · 03 oversatt+grønt banner · 04 oversett-ark · 07 sammenlign-modus · 08 sammenlign-ark pre-deploy). **Simulator-funn (rettet): pille-raden brukte web-tokens `sitedoc-primary`/`sitedoc-secondary` som IKKE finnes i mobil-`tailwind.config.ts` → NativeWind droppet dem (hvit pille + usynlig hvit «NB»). Fikset til `sitedoc-blue`/`sitedoc-blue-light` (mobil-tokens) — aktiv pille nå korrekt fylt blå.** **05/06 (amber «…»→grønn via poll) + full 08 (original+motorkort) gjenstår: krever test-API rebygget m/ backend-endringene** (`oversettDokument` finnes ikke på gammel test-API → radtrykk feiler; `jobber`/`projectId` mangler → ingen amber/ingen motorkort). Dvs. verifiseres etter dual-review→develop-merge→test-deploy. **Venter fabels designgodkjenning mot `06-design.png` for de 6 + re-fangst av 05/06/08 post-deploy.** |
...
- **Steg vi (2a mobil-tabs): 🔨 BYGGET (2026-07-06)** bak `useNyNavigasjon` (RN/SecureStore). Tab-struktur + Tegninger/Dokumenter/Timer/Mer + Kontakter (K6) + flagg-toggle (sitedoc_admin) + i18n 17 nøkler × 15 språk. Tegninger-tab **degradert** (uten ★/punkt-antall/thumbnail — fabel-godkjent, backend-oppfølgere notert i § Kode-diff-noter). Timer-tab = valg B (ekstrahert `DagsseddelListe`, uendret `app/timer/*`-stack). Mobil-typecheck ren for nye filer. **Venter Kenneth-verifisering:** simulator-skjermbilder per skjerm via dev-login (localhost-tunnel) mot 2a-design.
...
- **Steg vii (2c leser): 🔨 BYGGET (2026-07-07)** bak `useNyNavigasjon` (RN), kun `dokument/[id].tsx`. Språkpille-rad (kilde-merke + aktiv fylt blå + amber «…» m/ 8s-poll) + grønt banner (motor droppet v1) + `+ Oversett` (ny `mappe.oversettDokument`, rører ikke motor-config) + bunn-ark-sammenlign (gjenbrukt web-logikk). Backend `hentDokumentBlokker` utvidet (sourceLanguage/projectId/jobber). i18n 17 × 15. Flagg av = uendret. **Venter simulator-verifisering + fabels designgodkjenning mot `06-design.png`.**
- **Neste milepæl:** simulator-verifiseringsrunde steg vi + vii, deretter øktavslutnings-doc-synk. Gjenstår i handoff: steg viii (sammenligningsrunde mot prod-kopi m/ kunde) + pilot-plan.
