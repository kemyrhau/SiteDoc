...
| (vi) 2a mobil-tabs | ⏳ plan godkjent | RN-variant av `useNyNavigasjon` (AsyncStorage). **Plan godkjent av fabel 2026-07-06** (Hjem·Tegninger·Dokumenter·Timer·Mer; Timer-tab = liste + eksisterende stack; Lokasjoner ut av tabbar, nåbar). Klar for koding på GO. |
| (vii) 2c leser | ⏳ | |
...
- **Neste milepæl:** steg vi (2a mobil-tabs) — **plan ALLEREDE GODKJENT av fabel** (Timer-tab = dagsseddel-liste + eksisterende timer-stack; tegningsfelter verifiseres i byggefasen med flagg-plikt til fabel om ★/punkt-antall mangler). Ny økt kan starte koding direkte på godkjent plan når fabel gir GO.
