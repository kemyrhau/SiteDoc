...
- **UX-runde s1 + polish v4/v5 (2026-07-06, venter dual-review — flagg-på):** (s1-1) NavSidebar kollaps-knapp → 60px ikon-skinne m/ tooltip, aktiv-markering + Innstillinger-footer bevart, localStorage `sitedoc-nav-kollaps`. (s1-2) mappetre skjul/vis-pil på panelkanten (`SekundaertPanel` valgfri `skjulNokkel`, localStorage `sitedoc-mapper-panel`) + ny brødsmulesti «Alle mapper › … › {mappe}» i mapper-innholdsheaderen (S1/G12-relatert; erstatter panelet som nav når skjult). (v4) `hentDokumenter` poller hvert 8. sek mens amber-jobber finnes → chip grønn uten manuell refresh. (v5) infoboks bruker `gjenstaaende` i parentes-klausul «({g} kan oversettes automatisk nå)» (frontend-only, ny nøkkel — ingen `--force`). + Metro `blockList`-fiks. Web-build grønn. **Presis parset-telling i backend = mulig oppfølger hvis 0-tilfellet forvirrer i praksis (ikke bygget).**
