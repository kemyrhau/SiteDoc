...
| FM5 | «Mine timer» (egen rapport) | sidebar `mine-timer` / `/dashbord/timer/mine`(+`/ny`,`/[id]`) | **K2 (vedtatt): brukermeny (avatar) + søketreff «Min side › Timer»; mobil = Timer-tab** — fjernes fra firma-sidebar | `kreverFirmaModul="timer"` | Nei | |
