...
### Backend-oppfølgere fra 2a-degradering (fabel-godkjent 2026-07-06)

2a-designet (`docs/redesign/screenshots/04-design.png`) viser tre tegningsfelt som **ikke finnes i backend** i dag. Tegninger-taben er bygget uten dem (paritet brytes ikke — feltene finnes ikke å miste); disse «fullfører» designet når feltene kommer:

| Felt | Design | Backend-status | Oppfølger |
|---|---|---|---|
| ★ standardtegning | «Plan 1 ★» | `Drawing` har ingen `erStandard`/`isDefault` | Legg til `Drawing.erStandard` (schema + UI-toggle) → vis ★ på tegningsraden |
| Punkt-antall | «Rev. C · 14 punkter» | Kun `PointCloud.pointCount` (nullable), ikke aggregert på `Drawing`/IFC | Punkt-antall-aggregat per tegning → vis «· N punkter» etter revisjon |
| Thumbnail | miniatyrbilde per rad | kun `fileUrl`, ingen `thumbnailUrl` | Thumbnail-generering ved konvertering → erstatt filtype-ikon-placeholder |

**2c-leser-oppfølgere (steg vii, fabel-godkjent 2026-07-07):**

| Felt | Design | Backend-status | Oppfølger |
|---|---|---|---|
| Brødsmule i leser-header | «HMS › Datablader» | `hentDokumentBlokker` returnerer ikke mappe-sti | `folderId`→sti i queryet → vis brødsmule i 2c-header (utelatt v1) |
| Motor-etikett i banner | «Oversatt til polsk · OPUS-MT» | motor lagres kun prosjekt-nivå (`config.motor`), ikke per jobb → kan være usann for dok oversatt før motorbytte | Lagre motor per `FtdTranslationJob` → sann motor-etikett i banner (droppet v1) |
...
| P14 | Mapper — dokumentliste | sidebar `mapper` / `/[pid]/mapper` | sidebar-P + 2a Dokumenter-tab | `kreverProsjekt` | Nei | 🔨 2b: tabell bevart (flagg av uendret) |
| P15 | Mapper — embedding/AI-status per dok | i mapper-tabell | (bevares i 2b-tabell) | `kreverProsjekt` | Nei | 🔨 2b: status-kolonne bevart |
| P16 | Mapper — oversettelsesstatus per dok | i mapper-tabell (chips) | 2b oversettelsespanel + statuskolonne | `kreverProsjekt` | Nei | 🔨 **2b bygget:** OVERSETTELSE-kolonne m/ per-språk chips (grønn ✓ / amber … / grå) + høyrepanel (arv/egne, omfang N+M, «Oversett N gjenstående») — flagg på |
| P17 | Mapper — språkavvik-bekreft (`bekreftSpraakMut`) | 3-valgs varsel i dok-liste | 2b: ett-klikk «Bekreft» + «Behold» | `kreverProsjekt` | Nei | 🔨 **2b bygget:** flagg på → «Bekreft {språk} + oversett» + «Behold» (3-valg beholdt flagg av) |
| P18 | Mapper — «Les»-knapp → Reader | knapp per rad | 2b «Les» → 2c mobil | `kreverProsjekt` | Nei | 🔨 2b: «Les»-knapp bevart |
| P19 | Dokumentleser (Reader View + språkvelger + sammenlign) | `/[pid]/dokumentleser`, `/dokumenter/[id]/les` | 2c (mobil) / bevares (web) | `kreverProsjekt` | Nei | ✅ 2c mobil simulator-verifisert (steg vii); web uendret |
...
| T9 | Hamburger mobilmeny (web-viewport <md) | `Toppbar.tsx` | ny mobil-web-meny (paritet) | `kreverProsjekt`-disabling bevares | ✅ **LUKKET** (Opus web-verifisert + fabel-sluttgodkjent 2026-07-07): flagg på → speiler NavSidebar-hierarkiet (PROSJEKT-sone m/ delt `prosjektSoneElementer` uten «Mine timer» + fargeaksent + disabled uten prosjekt · FIRMA-sone · Kontakter · Innstillinger); flagg av = uendret hardkodet array (byte-identisk). A1-avvik (Mine timer i PROSJEKT-sonen) fikset `a6f8ab3f` + A1 re-test grønn. |
...
| M10 | Dokumentleser + språkbytte | `dokument/[id].tsx` | **2c** (språkpille-rad + sammenlign) | ett-trykk språkbytte | ✅ 2c simulator-verifisert + fabel-designgodkjent (steg vii, 2026-07-07) |
...
| Flagg-infra | ✅ | `useNyNavigasjon()` (`apps/web/src/hooks/`) — eneste kilde. Flagg av = eksakt dagens UI. **Bruker-lagret (Plan 2, 2026-07-07):** `User.nyNavigasjon Boolean?` (konto) er autoritativ. **Presedens (valg B): aktiv `?nyNav`-URL (flyktig demo/dev-override, aldri persistert) > konto > persistert lokal > env-default > av** — i delt `resolverNyNavigasjon` (@sitedoc/shared, `nyNavigasjon.test.ts` beviser kjeden inkl. delt-enhet-dominans, flyktig query-override, og at query fjernet → konto-styrt). Lokal cache nøklet per userId. Toggle skriver konto (`bruker.settNyNavigasjon`). Env-default (`NEXT_PUBLIC_NY_NAV_DEFAULT`) kun i redesign-stacken. Admin-tildeling: `organisasjon.settNyNavForFirma`/`settNyNavForBruker` + web-knapp på `firma/ansatte`. **Ikke gjenåpne kjeden — den er vedtatt.** **Brukermeny-toggle — KUN `sitedoc_admin` nå** (company_admin utvides ved pilotstart). **c4-fiks:** toggle-AV lander på `/dashbord`; `role="switch"`+`aria-checked`. |
...
| (iii) 1b sidebar + kontekst-chip | ✅ | Bak `useNyNavigasjon`. **NavSidebar** (PROSJEKT-sone: dagens elementer + P31 Kontakter, uendret G1–G12 + fargeaksent · FIRMA-sone admin-gated, speiler alle 15 firma/layout-innganger · Innstillinger fast nederst → huben; gjelder overalt m/flagg på, firma-layout-sidebar + FirmaKontekstVelger droppes da — løser P4). **KontekstChip** «{Firma} / {Prosjekt} ▾» erstatter FirmaVelger+ProsjektVelger. ⚠️ **Korrigert 2026-08-21:** chippen ble skrevet på nytt (gjenbrukte IKKE `ProsjektVelgerPanel`/`FirmaVelgerPanel`), så favoritter + autofokus-søk falt bort i `ad7cadc1` uten vedtak (regresjonsjakt 2026-08-21). **Gjenopprettet i `feat/kontekstvelger-1a` (2026-08-21):** favoritter (B3, delt `useFavoritter`), autofokus-søk (B4). Scope Alle/Mine er nå kun en lokal filter-pille, ikke reelt scope-valg (uavklart, egen sak). 1b-lastetilstand i teksten bevart. **P31 Kontakter** read-only lesevisning (`/dashbord/[prosjektId]/kontakter`). Gating G1–G12 ekstrahert til delt `sidebar-elementer.tsx` (`useSidebarElementer`) — HovedSidebar (flagg av) konsumerer samme, verbatim. i18n 36 nøkler × 15 språk. **Polish (fabel-gjennomgang):** Maskin flyttet til FIRMA-sonen (K1); c2 dobbel-aktiv fikset (maks én aktiv rad, lengste-prefiks-vinner + prosjekt-rader lyser ikke på firma/hub-ruter); Innstillinger-footer pinned (scroll kun i nav); «Mine timer» interim i PROSJEKT-sone (FM5/K2 → brukermeny i steg iv). Godkjente avvik: brand-blå sidebar, PROSJEKT-etikett uten prosjektnavn. **c3:** chip viser aldri «Velg firma / {prosjekt}» — firma utledes fra prosjektets primaryOrganization når ikke eksplisitt valgt (frittstående prosjekt → kun prosjekt-del). **✅ DESIGNGODKJENT + T/G-verifisert på test (fabel, 2026-07-05).** Restanse: T9 mobil-web-hamburger bruker fortsatt gammel nav m/flagg på (ikke bygget i steg iii). |
> 🔴 **USANT siden `ad7cadc1` (2026-07-22).** Favoritter forsvant da trakten erstattet
> `ProsjektVelgerPanel`. Verifisert 2026-08-21 — se [kontekstvelger-regresjonsjakt-2026-08-21.md](kontekstvelger-regresjonsjakt-2026-08-21.md).
...
| (v) 2b oversettelsespanel | 🔨 bygget | Bak `useNyNavigasjon`, kun web. **Header-knapp** «🌐 {kilde} → {mål}» åpner **høyrepanel (300px)**: kildespråk (readonly + lenke til prosjektoppsett), målspråk radio arv/egne (chips), **infoboks m/ begge tall** «N i denne mappen + M i undermapper som arver», primærknapp «Oversett N gjenstående nå». **OVERSETTELSE-kolonne** m/ per-språk chips (grønn ✓ / amber … slått sammen pending+processing, status i title / grå). **Avvik-rad** flagg på → «Bekreft {språk} + oversett» + «Behold» (3-valg beholdt flagg av). Ingen ny datamodell (`Folder.languageMode`/`languages` + `FtdTranslationJob`). Backend: ny `mappe.oversettelseOmfang` + `mappe.oversettGjenstaaende` (batch, try/catch per dok, arvende undermapper via ny `finnArvendeUndermapper`), `hentDokumenter` utvidet m/ per-språk jobb-status. i18n 17 nøkler × 15 språk. Full web-build grønn. **Test-verifisering: 8/10 bestått.** Funn-fikser: **v3 (rotårsak):** `oversettelseOmfang.gjenstaaende` telte uparsede dok (uten kildespråk-blokker) som `oversettGjenstaaende` hopper over → N sto fast på 14, 0 jobber skapt. Fikset: teller kun oversettbare dok (har kildespråk-blokker). **v3b:** optimistisk chip→amber ved 200 OK (setData). **v1:** knappetekst «gjenstående»→«dokumenter» (14 ble lest som språk). **v2:** seed-script `packages/db/scripts/seed-oversettelse-test.ts` for grønn/amber/avvik-tilstander på test. B3/B6 godkjent som-er. **v4/v5-polish committet + merget develop `5e16987b` (2026-07-06): v4 poll jobbstatus, v5 infoboks-tall. ✅ STEG V LUKKET — Opus web-verifisering bestått 2026-07-06 (alt bestått: B7-amber-chips + v4-refetch med seed).** |
| (vi) 2a mobil-tabs | ✅ LUKKET | Bak `useNyNavigasjon` (RN-variant, `apps/mobile/src/hooks/useNyNavigasjon.ts` — SecureStore via `lagreVerdi/hentVerdi`, ingen ny AsyncStorage-pakke; reaktiv modul-cache + lytter-sett; default av). **Tab-struktur** (`(tabs)/_layout.tsx`): flagg PÅ = Hjem·Tegninger·Dokumenter·Timer·Mer, flagg AV = Hjem·Lokasjoner·Mapper·Mer (via `href:null`-toggling — alle skjermer nåbare). **Tegninger-tab** (`tegninger.tsx`): segmentert [2D\|3D\|2D+3D] (3D→`/3d-visning`, 2D+3D→`/tegning-3d`), byggeplass-grupper, filtype-differensierte ikoner (PDF/DWG/IFC), IFC-kort når `kreverIfc`. **Degradert per fabel-godkjenning** (2026-07-06): uten ★-standardmarkør + punkt-antall + thumbnail (feltene finnes ikke i backend — se backend-oppfølgere under). **Dokumenter-tab** = `boks.tsx` oppgradert bak flagg (brødsmulesti «Alle mapper › …», språk-arv per mappe, oversettelsesstatus + ett-klikk språkavvik-bekreft via `bekreftDokumentSpraak`; flagg AV = dagens boks uendret). **Timer-tab** (valg B): ny `(tabs)/timer-oversikt.tsx` rendrer ekstrahert `DagsseddelListe` (ren flytting fra `timer/index.tsx`, som nå er tynn wrapper m/`medTilbakeknapp` — byte-tilsvarende dagens `/timer`); pusher inn i uendret `app/timer/*`-stack. **Mer-tab**: Kontakter-inngang (K6, ny `app/kontakter.tsx` read-only, tel:/mailto:) + `nyNavigasjon`-toggle gated `sitedoc_admin` (rolle via `bruker.hentMin`). i18n: 17 nye nøkler × 15 språk (`generate.ts`; pl/ru/fr `{{rev}}`-placeholder + lt-fallback rettet manuelt). Mobil-typecheck: mine filer rene (11 gjenværende feil er pre-eksisterende i urørte filer). **✅ LUKKET — fabel-designgodkjent 2026-07-07** mot skjermbilder (03 Tegninger-2D, 04 3D-nav, 05/06 Dokumenter+avvik, 01/02/10 Mer flagg-av/på/arbeider), merget `35503d98`. Bevis: `docs/redesign/screenshots/2a-simulator-2026-07-07/`. **Ikke verifisert (mangler testdata — IKKE «bestått»):** IFC-kort (B12 uten IFC-tegning) + amber «Oversetter …»-stat (seed-jobb ferdigprosessert). **Polish-restanse (ikke-blokkerende, fabel-notis):** farget statusprikk (grønn/amber) foran «Oversatt …» i Dokumenter-tab per 2a-mock — se § Polish-restanser. |
| (vii) 2c leser | ✅ LUKKET | **Fabel-designgodkjent 2026-07-07 (8/8 fangster mot `06-design.png`), merget develop `8c774368`.** Bak `useNyNavigasjon` (RN), kun `apps/mobile/app/dokument/[id].tsx`. 3 fabel-valg (ingen brødsmule v1 · bunn-ark-sammenlign · motor droppet fra banner). **Sticky språkpille-rad** (horisontal scroll, touch-mål 44px): kilde-pille m/ «· kilde», aktiv = fylt blå, språk under oversettelse = amber «…» m/ 8s-poll til grønn. **`+ Oversett`-pille** → bunn-ark m/ språkvalg → ny `mappe.oversettDokument` (upserter pending-jobb, isActive- + kildeblokk-guard, **rører ikke `config.motor`** — worker bruker prosjektets default; i motsetning til `reOversettDokument`). **Grønt banner** «Oversatt til {språk} · vis original» (motor droppet i v1 per fabel — prosjekt-default kan være usann; backend-oppfølger: motor-per-jobb). **Sammenlign** = topp-lenke → modus-hint-banner «trykk et avsnitt» + «Avslutt» → avsnitt trykkbare → bunn-ark m/ gjenbrukt web-logikk (`modul.sammenlignOversettelse` + `reOversettDokument`, motorbytte). **Brødsmule utelatt v1** (backend-oppfølger folderId→sti). Backend: `hentDokumentBlokker` utvidet (`sourceLanguage`, `projectId`, `jobber[]`). Token-fiks: web-tokens `sitedoc-primary`/`sitedoc-secondary` (finnes ikke i mobil-`tailwind.config.ts`, NativeWind droppet dem → hvit pille + usynlig «NB») → `sitedoc-blue`/`sitedoc-blue-light` (piller + tom-tilstands-knapp + flagg-av-språkvelger). i18n 17 nøkler × 15 språk (fr/pl `{{spraak}}` rettet manuelt). Mobil-typecheck ren. Flagg av = dagens leser uendret. **Fangster (`docs/redesign/screenshots/2c-simulator-2026-07-07/`):** 01 flagg-av · 02 kilde-pille · 03 oversatt+grønt banner · 04 oversett-ark · 07 sammenlign-modus (redesign-Opus) · 05 amber EN/RO «…» · 06 grønn via 8s-poll uten refresh · full-08 original+OPUS-MT/Google-motorkort (Kenneth live post-deploy) · 09 tom-tilstand docD blå «Last ned PDF» (deep-link). **⚠️ Verifiseringsnote:** tom-tilstanden (docD) er **kun nåbar via direkte navigasjon** (`sitedoc://dokument/<id>` / router.push) — uparsede dok (`processingState≠completed`) gates ut av Dokumenter-lista (pre-eksisterende `boks.tsx`-gating). Neste verifiseringsrunde skal ikke lete etter den i UI-lista. |
...
- **UX-runde s1 + polish v4/v5 (2026-07-06, ✅ committet + `--no-ff`-merget til develop `5e16987b` — flagg-på):** (s1-1) NavSidebar kollaps-knapp → 60px ikon-skinne m/ tooltip, aktiv-markering + Innstillinger-footer bevart, localStorage `sitedoc-nav-kollaps`. (s1-2) mappetre skjul/vis-pil på panelkanten (`SekundaertPanel` valgfri `skjulNokkel`, localStorage `sitedoc-mapper-panel`) + ny brødsmulesti «Alle mapper › … › {mappe}» i mapper-innholdsheaderen (S1/G12-relatert; erstatter panelet som nav når skjult). (v4) `hentDokumenter` poller hvert 8. sek mens amber-jobber finnes → chip grønn uten manuell refresh. (v5) infoboks bruker `gjenstaaende` i parentes-klausul «({g} kan oversettes automatisk nå)» (frontend-only, ny nøkkel — ingen `--force`). + Metro `blockList`-fiks. Web-build grønn. **Presis parset-telling i backend = mulig oppfølger hvis 0-tilfellet forvirrer i praksis (ikke bygget).** **✅ s1 designgodkjent av fabel (2026-07-06); v4/v5 web-verifisert (alt bestått) → steg v LUKKET.**
- **Dev-login simulator-oppsett ✅ KOMPLETT + verifisert grønt (2026-07-06):** localhost-tunnel (`ssh -N -L 3301:localhost:3301 server-ny` + `expo prebuild --clean`); rotårsaks-kjede Cloudflare-kant → iOS Local Network-privacy → stale container-secret. Låser opp simulator-testing for steg vi. Se [dev-login-agent.md](dev-login-agent.md) + BACKLOG.
- **Steg vi (2a mobil-tabs): ✅ LUKKET (designgodkjent av fabel 2026-07-07, merget `35503d98`)** bak `useNyNavigasjon` (RN/SecureStore). Tab-struktur + Tegninger/Dokumenter/Timer/Mer + Kontakter (K6) + flagg-toggle (sitedoc_admin) + i18n 17 nøkler × 15 språk. Tegninger-tab **degradert** (uten ★/punkt-antall/thumbnail — fabel-godkjent, backend-oppfølgere notert i § Kode-diff-noter). Timer-tab = valg B (ekstrahert `DagsseddelListe`, uendret `app/timer/*`-stack). Alle skjermer simulator-verifisert (bevis `docs/redesign/screenshots/2a-simulator-2026-07-07/`). **Ikke verifisert (mangler testdata, ikke «bestått»):** IFC-kort + amber «Oversetter …»-stat.
...
- **Restanse-runde ✅ KOMPLETT LUKKET (2026-07-07):** **P-a** statusprikk **✅** (`restanse-2026-07-07/pa-statusprikk-gronn.png`) · **Punkt 4** kildeflagg (variant A «🇳🇴 NB · kilde») **✅** (`punkt4-kildeflag.png`) · **FM5/K2** «Mine timer»→brukermeny+søk («Min side») **✅** · **T9** mobil-web-hamburger **✅** (A1-avvik «Mine timer» i PROSJEKT-sonen fikset via delt `prosjektSoneElementer` `a6f8ab3f`; A1 re-test grønn). Alle bak `nyNavigasjon`, flagg av byte-identisk. Merget develop `24083342`→…→`fa87b2f2`.
- **Bevis for web-flatene (T9/FM5):** Opus web (Claude/Opus i Kenneths innloggede Chrome) kjørte `/test-redesign-nav` på test.sitedoc.no; skjermbildene lever i Opus web-chatten, **rapport relayet via Kenneth** til fabel. Kommando-fila (`.claude/commands/test-redesign-nav.md`) rettet etter Opus web-funn (B1-URL `/dashbord/{id}`).
- **Neste milepæl:** **Bruker-lagret flagg (Plan 2) ✅ SLUTTGODKJENT + LUKKET (fabel, 2026-07-07).** Bevis: web-bevis a–d (Opus web via Kenneth) + simulator-bevis (`docs/redesign/screenshots/plan2-simulator-2026-07-07/`, brukerbytte uten restart + offline-fallback) + 13/13 presedens-test. Merget develop (`0d3f21ac` + SecureStore-kolon-fix `19f7cdcb`); presedens **aktiv `?nyNav`-URL (flyktig) > konto > persistert lokal > env-default > av**. **Test redeployet = develop.** **Gjenstår kun: steg viii-drift (Kenneth) + kunderunde + pilot** (`sitedoc_admin`→`company_admin`) per [steg-viii-kunderunde.md](../redesign/steg-viii-kunderunde.md).
