...
- **Steg vii (2c leser): 🔨 BYGGET + 6/8 fabel-designgodkjent (2026-07-07)** bak `useNyNavigasjon` (RN), kun `dokument/[id].tsx`. Språkpille-rad (kilde-merke + aktiv fylt blå + amber «…» m/ 8s-poll) + grønt banner (motor droppet v1) + `+ Oversett` (ny `mappe.oversettDokument`, isActive-guard, rører ikke motor-config) + bunn-ark-sammenlign (gjenbrukt web-logikk). Backend `hentDokumentBlokker` utvidet (sourceLanguage/projectId/jobber). Token-fiks sitedoc-blue. i18n 17 × 15. Flagg av = uendret. **05/06/full-08 + tom-tilstand (docD) re-fanges post-deploy for sluttgodkjenning mot `06-design.png`.**
- **Neste milepæl:** develop-merge (steg vii) → test-deploy → reseed → re-fangst 05/06/08+tom-tilstand → sluttgodkjenning, deretter øktavslutnings-doc-synk. Gjenstår i handoff: steg viii (sammenligningsrunde mot prod-kopi m/ kunde) + pilot-plan.
