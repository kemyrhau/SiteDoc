...
| T9 | Hamburger mobilmeny (web-viewport <md) | `Toppbar.tsx` | ny mobil-web-meny (paritet) | `kreverProsjekt`-disabling bevares | 🔨 **A2/A3 ✅ sluttgodkjent** (flagg-på disabled-tilstand + flagg-av byte-identisk — Opus web-rapport relayet via Kenneth 2026-07-07). **A1-avvik funnet + fikset:** hamburgeren viste «Mine timer» i PROSJEKT-sonen (Toppbar bygde `prosjektElementer` uten FM5-filteret) → delt `prosjektSoneElementer` (`a6f8ab3f`). **Venter A1 re-kjøring** (Opus web, etter test-web-rebuild) |
...
| Flagg-infra | ✅ | `useNyNavigasjon()` (`apps/web/src/hooks/`) — localStorage + `?nyNav=1`, eneste kilde. Flagg av = eksakt dagens UI. **Brukermeny-toggle (`settNyNavigasjon`) — KUN `sitedoc_admin` nå** (company_admin utvides ved pilotstart, hindrer at kunde-admin skrur på uferdig redesign på prod). **c4-fiks:** navigerer til ren sti (uten `?nyNav`) i stedet for reload — query-paramet overstyrte ellers localStorage og drepte toggle-AV; toggle-AV lander på `/dashbord`; `role="switch"`+`aria-checked`. Plattform-abstraksjon (AsyncStorage) noteres for 2a |
...
- **Restanse-runde (2026-07-07, merget develop `24083342`→`d68f7b00`):** **P-a** statusprikk **✅ designgodkjent** (`restanse-2026-07-07/pa-statusprikk-gronn.png`) · **Punkt 4** kildeflagg (variant A «🇳🇴 NB · kilde») **✅ designgodkjent** (`punkt4-kildeflag.png`) · **FM5/K2** «Mine timer»→brukermeny+søk **✅ LUKKET** (Opus web-verifisert) · **T9** mobil-web-hamburger — **A2/A3 ✅ sluttgodkjent**, **A1-avvik funnet + fikset** («Mine timer» vist i PROSJEKT-sonen → delt `prosjektSoneElementer`, `a6f8ab3f`), **venter A1 re-kjøring** etter test-web-rebuild. Alle bak `nyNavigasjon`, flagg av byte-identisk.
- **Bevis for web-flatene (T9/FM5):** Opus web (Claude/Opus i Kenneths innloggede Chrome) kjørte `/test-redesign-nav` på test.sitedoc.no; skjermbildene lever i Opus web-chatten, **rapport relayet via Kenneth** til fabel. Kommando-fila (`.claude/commands/test-redesign-nav.md`) rettet etter Opus web-funn (B1-URL `/dashbord/{id}`, ikke `/dashbord/{id}/dashbord`).
- **Neste milepæl:** **T9-A1 re-kjøring** (Opus web, etter test-web-rebuild av `a6f8ab3f`) → fabel-sluttgodkjenning lukker restanse-runden. Gjenstår i handoff: steg viii (sammenligningsrunde mot prod-kopi m/ kunde) + pilot-plan (flagg → company_admin).
