...
| `docker/env/api.env` | `DATABASE_URL`/`DIRECT_URL` (`postgresql://sitedoc:***@postgres:5432/sitedoc`), `PORT=3001`, `AUTH_SECRET`, `RESEND_API_KEY`, `VEGVESEN_API_KEY`, `SITEDOC_INTEGRATION_KEY`, **`MICROSOFT_MOBILE_CLIENT_ID`** (mobilens public-client app-id = `EXPO_PUBLIC_MICROSOFT_CLIENT_ID` = `234ca0e0-…`; brukt som `aud`-pin ved mobil-MS ID-token-validering) |
...
>
> ⚠️ **Unntak — mobil-innlogging kjører i Fastify-api-containeren.** `mobilAuth.byttToken` kalles av mobilen mot `api(-test).sitedoc.no` (ikke web). Derfor må `MICROSOFT_MOBILE_CLIENT_ID` ligge i `api.env`. Uten den feiler MS-innlogging fail-closed (`INTERNAL_SERVER_ERROR` «ikke konfigurert»). Se `apps/api/src/routes/mobilAuth.ts` (`hentMicrosoftBrukerinfoFraIdToken`). Sett i test FØR mobilen får ID-token-bygget via `eas update`.
>
> 🔴 **Ingen issuer-env for mobil-MS — appen er FLERTENANT.** Kunder logger inn fra sin egen Entra-tenant (A.Markussen fra sin, ikke vår), så `iss` kan ikke pinnes til én tenant. Api validerer i stedet `iss` mot ID-tokenets eget `tid`-claim og pinner kun `aud`; signatur mot Entras `/common`-JWKS. Et tidligere utkast pinnet `iss` mot en gjenbrukt web-issuer-env — det ville sperret hver kunde og ble forkastet (2026-09-07).
