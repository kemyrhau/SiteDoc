...
|---|---|---|---|---|
| **2026-09-24 ~01:5x** | `1` | **`4553aedd`** ⚠️`*` | **production** | ✅ **GJELDENDE I PROD.** Update group `1e668259-cff1-4aea-b46c-b4974dbc4111` · iOS update `01a0d0d9-d270-70e1-ad84-d62eb1361505`. Steg 2c-målingen ga **én** treff: `https://api.sitedoc.no` — ingen `api-test`. **Publisert i SAMME runde som prod-deployen** (`main` `39e14648`), slik regelen krever; etterslepet fra 06.–07.09 gjentok seg ikke. **Hva brukeren merker:** ① arkiv-PDF-en viser **områdenavn** i Lokasjon-feltet i stedet for «Et definert område» · ② utilgjengelige maler **forklarer seg** («Du er ikke registrator i {{faggruppe}}») i stedet for bare å mangle · ③ sperrede knapper bærer forklaring (`KnappMedForklaring`, 48 knapper web+mobil) · ④ bundet flyt låser dokumentet til riktig flyt · ⑤ statusfarger i paritet med web · ⑥ opprett-velger uten modal. ⚠️ **Commit-linja hadde asterisk** — coworks to ukommiterte docs-filer (`SAMARBEIDSREGLER.md`, `STATUS-AKTUELT.md`) lå i treet. **Målt at ingen av dem er i JS-bundelen**, så bundelen bærer `4553aedd`s kode nøyaktig. 🔴 **Men det skulle ikke skjedd: cowork eide begge filene og skulle gitt dem til merge-agenten før publiseringen.** Tredje gang asterisken kommer fra coworks egne docs (07.09, 10.09, nå) |
| **2026-09-10 ~22:4x** | `1` | **`7242ea83`** | **test** | ✅ **GJELDENDE PÅ TEST.** Update group `f19d0904-51a6-4579-b6f5-1585a907d99e` · iOS update `01a08d45`. 🟢 **Ingen asterisk — hashen ER develop-tippen, og mobil-JS er identisk med `74d641f5`** (docs-commiten på toppen rører ingen mobilfil; verifisert med `git diff --name-only 7242ea83 origin/develop -- apps/mobile packages/shared` = tom). Steg 2-målingen ga **én** treff: `https://api-test.sitedoc.no`. **Hva testeren merker:** «Leser»-rettigheten i dokumentflyten låser nå riktig — **også når flytleddet er bundet til en faggruppe**, som var ulåst før (`faggruppeId`-bindingen manglet helt i klientoppslaget). Tre inline-kopier av oppslaget er samlet til én delt kilde i `packages/shared`, så web og mobil ikke lenger kan divergere. ⚠️ **Resten av rundene 67–72 traff mobilen via API-et ved web-deployen, ikke via denne OTA-en** |
| **2026-09-10 ~ettermiddag** | `1` | **`a98a19cb`** | **test** | ✅ **GJELDENDE PÅ TEST.** Update group `7af7a823-163e-492b-8111-89ca21ca9229` · iOS update `01a08b72`. 🟢 **Ingen asterisk — hashen er identisk med develop-tippen** (runde 67-mergen). Steg 2-målingen ga **én** treff: `https://api-test.sitedoc.no`. **Hva testeren merker:** ① **nye prosjekter får standardgrupper og faggrupper** — de har vært tomme siden 6. april (`9a489876`), og seeden går nå fra alle tre opprettelsesveiene i én transaksjon · ② PDF-tegninger konvertert via batch-veien vises **riktig skalert med én gang**, uten at backfill-knappen må trykkes. **Resten av runde 65–66 var kommentarrydding uten atferd.** ⚠️ **Treet var urent midt i sekvensen** (cowork skrev i `CLAUDE.md` mens steg 3 pågikk) — endringen ble trukket ut igjen og flyttet til `fix/gruppe-systemnokkel`, derfor ren hash |
| **2026-09-09 ~20:5x** | `1` | **`94367c56`** | **test** | ✅ **GJELDENDE PÅ TEST.** Update group `16110d0b-534f-4d27-99d2-b2519c95b586` · iOS update `01a087b7`. 🟢 **Ingen asterisk — treet var rent, hashen er identisk med develop-tippen.** Steg 2-målingen ga **én** treff: `https://api-test.sitedoc.no`. **Hva testeren merker:** ① dialogen på en oppgave svarer offline i stedet for «skjer ingen ting» — teksten blir stående · ② 🔴 **Send/Besvar/Videresend ga FALSK SUKSESS offline** (arket lukket seg, dokumentet gikk aldri) — nå kommer en beskjed om at det ikke ble sendt · ③ PSI-signering markerer ikke seksjonen fullført før serveren har bekreftet · ④ lokasjon, prioritet, tittel, beskrivelse og dokument-oversettelse svarer ved feil. **Alle fire funnene kom fra Kenneths egen testing 08.–09.09** |
