...
- **B3 — antall forhåndsutfylles** fra prefill-spennet. Web timer+maskin ✅; **mobil maskin ✅ (M5 2026-07-10** — `MaskinSeksjon`s `timer`-init kaller `effektiveTimerFraSpenn` når prefill-spennet er gyldig). Mobil timer-modal prefyller foreløpig ikke antall (`TimerSeksjon` init tom for ny rad) — BACKLOG. Er beregnet fra ≥ til, forhåndsutfylles verken til eller antall (ingen ugyldig rad).
