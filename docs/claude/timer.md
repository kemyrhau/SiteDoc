...
- **Overlapp (server-sperre, kun web-mutasjoner):** en arbeider kan ikke registrere **overlappende timer-rader** på samme sedel, på tvers av prosjekt og underprosjekt (én arbeider kan ikke være to steder). `sjekkTimerOverlapp` i `tilfoyTimerRad`/`oppdaterTimerRad`. **Berøring i endepunkt** (12:00 slutt = 12:00 start) er tillatt. `syncBatch` — mobilens eneste skrivevei — har verken overlapp-sjekk eller `refineFraForTil`. Se [BACKLOG § Timer web-vs-mobil paritet → SYNC-2](BACKLOG.md). Maskin-rader overlapp-sjekkes ikke mot timer-rader (kun `fra<til`); maskin-vs-maskin ikke ennå (BACKLOG-utredning).
