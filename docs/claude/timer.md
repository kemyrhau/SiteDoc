...
**Radvalget legger seg OPPÅ `skalEksporteres`.** `detaljEksport` filtrerer allerede bort `skalEksporteres=false` server-side; radvalget trekker bare fra (skjuler hele radtyper) — kan aldri huke PÅ en type oppsettet slo av. Den grå «utelatt i oppsett»-gjeninkluderingen er fase 3. Maskin-håndtering i radvalget: «timer» valgt → nøstet maskin under sin timerad; «timer» avvalgt + «maskin» valgt → nøstet maskin blir egen normal rad (timeraden skjult av VALGET, ikke en anomali); løse maskiner (uten timerad / ikke-eksporterbar) vises kun når «maskin» er valgt, med sine merker.
