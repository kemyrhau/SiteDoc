...
**Dokumentet:** firmatopp (firmanavn · periode · aktive filtre prosjekt/ansatt) → Sammendrag → Timerader/Tillegg/Utlegg (maskin nøstet, tre bøtter som Excel). Footer hver side: generert-stempel + Chromium-injiserte sidetall. **ID-kolonnene er BEVISST utelatt** (koblingsnøkler for DB, ikke lesestoff — de blir i Excel). Totaler = samme kilde som Excel → stemmer per konstruksjon. **Ikke i fase 1** (fabels notat, fase 2/3): Type-kolonne, radvalg, lagrede utskriftsmaler, firma/personlig-nivå — låser ikke en modell før fase 2 er sett virke. De to aggregat-pivotene («Etter prosjekt»/«Etter dag») utelates: et dokument svarer på HVA som skjedde, ikke HVORDAN man vil skjære dataene (Kenneth-vedtak).
...
`.xlsx`-arkstruktur (6 ark): aggregat-arkene **Sammendrag** · **Etter prosjekt** · **Etter dag** beholdes uendret (fjernes ikke på en slutning om at ingen bruker dem — stille bruddform), detalj-arkene **Timerader** · **Tillegg** · **Utlegg** er tillegg. Arknavnene går gjennom `t()` (i18n gjelder også ikke-JSX; `t` injiseres i `eksporterXlsx` fra `page.tsx`) — «Etter …», ikke «Per …» (grupperings-husstil, CLAUDE.md § Språk). **Kolonneoverskriftene er ennå hardkodet norsk** (~50 strenger) — egen i18n-runde. Timerader: maskin nøstet under sin timerad via `sheetTimerId` (maskin uten timerad nederst); **maskintimer i egen kolonne** (aldri i Timer-kolonnen — attesteringsflaten summerer maskintimer separat), så arbeidstimer-kontrollsummen forblir ren. Hvert detalj-ark har **SUBTOTAL(109)-kontrollsum(mer)** (levende formel som respekterer Excel-filtrering) — regnet fra RADENE, ikke serveraggregatet: avvik mot Sammendrag = kodeveiene har drevet fra hverandre, synlig i fila før lønnskjøringen (Timerader har to summer: arbeidstimer + maskintimer). **Status-kolonnen:** «Radstatus» på Timerader/Tillegg (T.3 `attestertStatus` per rad — lønn spør om RADEN er attestert; en sedel kan stå «sent» mens enkeltrader er returnert), «Seddelstatus» på Utlegg (`SheetUtlegg` har ingen rad-status). Ulik betydning per ark ⇒ ulikt kolonnenavn, aldri én kolonne som later som konsistens. CSV forblir sammendrag (ett flatt bord). Timerad-kolonnene er en **datadrevet liste** i `timer-rapport-eksport.ts` — proadm-«underprosjekt» legges senere til som ÉN kolonne + ETT filter, uten ombygging. Siste kolonne på hver rad er en tynn **ID** (koblingsnøkkel for den framtidige dimensjonen).

**`skalEksporteres`-filter (2026-08-25, fabel-vedtak):** en lønnsart/tilleggstype merket `skalEksporteres=false` (kunde-satt avkryssing «Skal eksporteres til lønn/regnskap») utelates fra ALLE eksporter — feltet ble tidligere aldri lest (kunden fjernet haken, raden eksporterte likevel → feil lønnsgrunnlag). Én regel, ingen formål-matrise (overstyring kommer senere som synlig Tilpasset-unntak). To steder, samme regel: (1) **detalj-arkene** — `detaljEksport` filtrerer på DB-nivå via relasjons-`where` (`timer.where.lonnsart.skalEksporteres=true`, `tillegg.where.tillegg.skalEksporteres=true`); (2) **aggregat-arkene** — `firmaPeriodeRapport` tar en valgfri `kunEksporterbare`-flagg som eksport-veien (`rapport/page.tsx håndterEksport`) setter til `true`, og som hopper over ikke-eksporterbare lønnsarter i **time-summene** (ikke i sedel-metadata som statusFordeling/antallSedler — en sedel med kun ikke-eksporterbare timer er fortsatt en reell sedel). **Skjermen kaller uten flagget → viser alle timer** (attestering/oversikt trenger å se alt; ordren gjelder eksporter). **Maskin på ekskludert timerad:** maskin er ikke lønnsart-filtrert (fakturerbart), så den forsvinner ikke — men den havner IKKE under «Maskin uten timerad» (den bøtta betyr «maskin brukt uten registrert arbeid», et anomali-signal). Den får sin egen linje **«Maskin på ikke-eksporterbar timerad»** i Timerader-arket, og maskintimene teller fortsatt i maskintimer-summen. `detaljEksport` returnerer derfor tre maskin-bøtter: nøstet · uten timerad · på ikke-eksporterbar timerad. (Den nye linja går gjennom `t()` fra start — `timer.eksport.maskinIkkeEksporterbar`, 14 språk; de øvrige eksport-merkene/kolonneoverskriftene ryddes i kolonne-overskrift-batchen.)
