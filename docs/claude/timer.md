...
**Type-kolonne, ett kronologisk «Detaljer»-ark.** De tre detaljarkene (Timerader/Tillegg/Utlegg) er slått sammen til ÉTT kronologisk ark (dato asc) med en **Type**-kolonne (Timer · Maskin · Tillegg · Utlegg) — Kenneth: «tillegg og utlegg må kunne bli rad i timerader sammen med timeoversikt». Maskin beholder nøstingen under sin timerad (`sheetTimerId`). Fabels innvending mot sammenslåing (ulik radform) faller bort: tomme type-fremmede celler er uproblematiske når Type-kolonnen sier hva raden er. Kolonneunion: `Dato · Ansatt · Ansattnr · Prosjekt · Type · Betegnelse · Aktivitet · Timer · Maskintimer · Antall · Beløp · Mengde · Enhet · Beskrivelse · Status · Id` (Betegnelse = lønnsart/maskinnavn/tilleggsnavn/kategori). **Fire** SUBTOTAL-kontrollsummer nå (Timer/Maskintimer/Antall/Beløp) — ikke én generisk «antall»-kolonne (kan ikke summere timer og kroner sammen; ordren gjør summene *viktigere*). Timer-summen stemmer mot Sammendrag når Timer-rader er med.
