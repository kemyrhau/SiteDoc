...
export default function GodkjenningSide() {
  ...
  const { data: kanGodkjenne, isLoading: tilgangLaster } =
    trpc.timer.dagsseddel.kanGodkjenne.useQuery({ projectId: params.prosjektId });

  const { data: rader, isLoading } = trpc.timer.dagsseddel.hentTilGodkjenning.useQuery(
    { projectId: params.prosjektId },
    { enabled: kanGodkjenne === true },
  );

  const attester = trpc.timer.dagsseddel.attester.useMutation({
    onSuccess: (_data: unknown) => {
      void utils.timer.dagsseddel.hentTilGodkjenning.invalidate();
    },
    ...
    <div className="mx-auto max-w-5xl p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-gray-900">
          {t("timer.godkjenning.tittel")}
        </h1>
        <p className="mt-1 text-sm text-gray-600">
          {t("timer.godkjenning.beskrivelse")}
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-12 text-center">
          <p className="text-sm text-gray-500">
            {t("timer.godkjenning.ingenSedler")}
          </p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-gray-200 bg-gray-50">
              <tr className="text-left text-xs font-medium uppercase tracking-wide text-gray-500">
                <th className="px-3 py-3">{t("timer.godkjenning.kol.dato")}</th>
                <th className="px-3 py-3">{t("timer.godkjenning.kol.ansatt")}</th>
            ...
            <tbody>
              {(rader as unknown as GodkjenningRad[]).map((rad) => (
                <tr
                  key={rad.id}
                  ...
                  <td className="px-3 py-2">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button
                        ...
                        title={t("timer.godkjenning.returner")}
                      >
                      ...
                      <button
                        ...
                        title={t("timer.godkjenning.attester")}
                      >
...
function ReturnerDialog({
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setFeil(null);
    if (!kommentar.trim()) {
      setFeil(t("timer.godkjenning.kommentarPaakrevd"));
      return;
    }
  ...
  return (
    <Modal open={true} onClose={onLukk} title={t("timer.godkjenning.returnerTittel")}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <p className="text-sm text-gray-600">
          {t("timer.godkjenning.returnerBeskrivelse")}
        </p>
        <div>
          <label className="mb-1 block text-sm font-medium text-gray-700">
            {t("timer.godkjenning.kommentar")}
          </label>
          <textarea
            value={kommentar}
            onChange={(e) => setKommentar(e.target.value)}
            rows={4}
            placeholder={t("timer.godkjenning.kommentarPlaceholder")}
