        ...
        <div className="relative">
          ...
          {visMalVelger && (
            <div className="absolute left-0 top-full z-10 mt-1 max-h-60 w-64 overflow-y-auto rounded-lg border border-gray-200 bg-white shadow-lg">
              {alleMaler.length === 0 ? (
                <div className="px-3 py-2 text-xs text-gray-400">{t("dokumentflyt.ingenMaler")}</div>
              ) : (
                <>
...
const ROLLE_KONFIG: Record<string, { farge: string; ikonNavn: string; tittelNoekkel: string; rekkefølge: number }> = {
  registrator: { farge: "gray", ikonNavn: "FileEdit", tittelNoekkel: "dokumentflyt.registrator", rekkefølge: 0 },
  bestiller: { farge: "blue", ikonNavn: "Send", tittelNoekkel: "dokumentflyt.bestiller", rekkefølge: 1 },
  utforer: { farge: "purple", ikonNavn: "ClipboardCheck", tittelNoekkel: "dokumentflyt.utforer", rekkefølge: 2 },
...
function DynamiskFlyt({
  ...
  const fjernRolle = (rolle: string) => {
    ...
    if (harMedlemmer && !confirm(t("dokumentflyt.bekreftFjernRolle"))) return;
        ...
        <>
          ...
          {visLeggTilRolle ? (
            <div className="flex flex-col items-start gap-1 rounded-lg border border-dashed border-gray-300 bg-gray-50/50 px-3 py-2">
              <span className="text-xs font-semibold uppercase tracking-wide text-gray-400">{t("dokumentflyt.velgRolle")}</span>
  ...
  function renderHovedansvarligPrikk(m: DokumentflytMedlem) {
    return (
      <button
        ...
        title={m.erHovedansvarlig ? t("dokumentflyt.fjernHovedansvarlig") : t("dokumentflyt.settHovedansvarlig")}
      />
    );
  }
  ...
  function renderKanRedigereToggle(m: DokumentflytMedlem) {
    return (
      <button
        ...
        title={m.kanRedigere ? t("dokumentflyt.settLeser") : t("dokumentflyt.settRedigerer")}
      >
      ...
      <div className={`mb-1 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide ${f.tittel}`}>
        ...
        {redigerLabel ? (
          ...
          <button
            ...
            title={t("dokumentflyt.redigerRollenavn")}
          >
            {tittel}
          </button>
        )}
        {onFjernRolle && (
          <button
            ...
            title={t("dokumentflyt.fjernRolle")}
          >
            <X className="h-3 w-3" />
          </button>
        )}
    ...
    <div>
      <div className="mb-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">{t("dokumentflyt.tittel")}</h2>
            <p className="text-sm text-gray-500">{t("dokumentflyt.beskrivelse")}</p>
          </div>
          <HjelpKnapp>
      ...
      <div className="rounded-t-lg border border-gray-200 bg-gray-50 px-4 py-2">
        <div className="grid grid-cols-12 gap-2 text-xs font-medium uppercase tracking-wide text-gray-500">
          ...
          <div className="col-span-5">{t("dokumentflyt.dokumentflyter")}</div>
          <div className="col-span-3">{t("dokumentflyt.kontakter")}</div>
        </div>
      </div>
            ...
            <div key={ent.id}>
              ...
              >
                <div className="grid grid-cols-12 gap-2 w-full items-center">
                  ...
                  <div className="col-span-3 flex items-center justify-between text-sm text-gray-400">
                    <span>{kontakter.length > 0 ? `${kontakter.length} ${t("dokumentflyt.personerSuffix")}` : "—"}</span>
              ...
              {erUtvidet && (
                <div className="border-t border-gray-100 bg-gray-50/50 px-4 py-4 space-y-4">
                  ...
                  {dflyter.length === 0 && (
                    <p className="text-xs text-gray-400 italic">{t("dokumentflyt.ingenFlyter")}</p>
                  )}
