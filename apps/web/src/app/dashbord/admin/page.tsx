      ...
      <div className="rounded-lg border border-gray-200 bg-white p-5">
        ...
        {prosjekter && prosjekter.length > 0 ? (
          <div className="space-y-2">
            {prosjekter.slice(0, 5).map((p: { id: string; name: string; projectNumber: string; members: { id: string }[]; faggrupper: { id: string }[]; projectOrganizations: { organization: { name: string } }[] }) => (
              <div key={p.id} className="flex items-center justify-between text-sm">
                ...
                <div className="flex items-center gap-3 text-xs text-gray-500">
                  ...
                  {p.projectOrganizations[0] && (
                    <span className="rounded bg-purple-50 px-1.5 py-0.5 text-purple-600">
                      {p.projectOrganizations[0].organization.name}
                    </span>
                  )}
                </div>
              </div>
