      ...
      {visScopeRader && (
        <div className="border-b border-gray-100 py-1">
          ...
          >
            ...
            <span className="ml-auto text-xs text-gray-500">{prosjekter.length}</span>
          </button>
          <button
            onClick={() => {
              velgScope("mine");
          ...
          >
            ...
            <span className="ml-auto text-xs text-gray-500">{mineProsjekter.length}</span>
          </button>
        </div>
      )}
...
export function ProsjektVelger() {
  const { valgtProsjekt, prosjektScope } = useProsjekt();
  const { t } = useTranslation();
    ...
    <div ref={ref} className="relative">
      ...
      {apen && (
        <div className="absolute left-0 top-full z-50 mt-1 w-72 rounded-lg border border-gray-200 bg-white shadow-xl">
          <ProsjektVelgerPanel onValgt={() => setApen(false)} />
        </div>
      )}
    </div>
  );
