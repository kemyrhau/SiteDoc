...
export const tilleggRouter = router({
  ...
  opprett: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        return await ctx.prismaTimer.tillegg.create({
          data: {
            organizationId: orgId,
            type: input.type,
        ...
        if (
          ...
          throw new TRPCError({
            code: "CONFLICT",
            message: `Tillegg med kode «${input.kode}» finnes allerede`,
          });
        }
        throw e;
      }
  ...
  oppdater: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!eksisterende) {
        throw new TRPCError({
          code: "NOT_FOUND",
          message: "Tillegg finnes ikke eller tilhører ikke ditt firma",
        });
      }

      const data: Prisma.TilleggUpdateInput = {};
      ...
      try {
        return await ctx.prismaTimer.tillegg.update({
          where: { id: input.id },
          data,
        });
      } catch (e) {
        if (
          ...
          throw new TRPCError({
            code: "CONFLICT",
            message: "Et annet tillegg har allerede denne koden",
          });
        }
        throw e;
      }
  ...
  deaktiver: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      return ctx.prismaTimer.tillegg.update({
        where: { id: input.id },
        data: { aktiv: false },
      });
    }),
