/**
 * Delt periode-filter-logikk (løftet til @sitedoc/shared 2026-08-23, fra apps/web/src/lib/periode.ts).
 *
 * ÉN kilde for både web-`PeriodeFilter` og mobil-RN-varianten. Logikken er ren TS (ingen
 * DOM/react) → begge flatene importerer den samme, så hurtigvalg-settet og grensematematikken
 * ALDRI kan drifte mellom web og mobil. `HURTIGVALG_STANDARD` er modul-konstant: den strukturelle
 * garden mot at en flate trimmer settet på egen hånd (Kenneth-vedtak). Bilder-siden som trenger
 * `3mnd` gir et eksplisitt `valg`-tillegg i web-komponenten.
 */
...
/** Standard hurtigvalg-sett (tegningssiden + mobil). Bilder-siden legger til «3mnd» via prop. */
export const HURTIGVALG_STANDARD: PeriodeHurtigvalg[] = ["idag", "uke", "mnd", "3mnd", "alle", "egendefinert"];

/** i18n-nøkkel per hurtigvalg (delt av web + mobil, `periodeFilter.*`). */
export const PERIODE_NOEKKEL: Record<PeriodeHurtigvalg, string> = {
