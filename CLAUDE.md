...
**Blokk A (P1 Fase 1 — prosjektliste filtreres på valgt firma) IMPLEMENTERT på develop 2026-05-04.** Andre del av admin-navigasjon-analyse-2026-05-03.md (tiltak #2 i prioritert rekkefølge). Sitedoc_admin med valgt firma i FirmaVelger ser nå kun prosjekter med matchende `primaryOrganizationId`. Vanlig bruker / company_admin uberørt. Bakfyll test-DB: 2 prosjekter satt til Byggeleder (Test redigert mal + Markussen Boligfelt B12). Server: `prosjekt.hentMine` + `prosjekt.hentAlle` tar valgfri `organizationId: z.string().uuid().optional()`. For sitedoc_admin: `organizationId` gitt → `where.primaryOrganizationId = organizationId`; ikke gitt → status quo (alle). Klient: 4 callsites migrert — `kontekst/prosjekt-kontekst.tsx` (driver ProsjektVelger), `app/dashbord/page.tsx`, `app/dashbord/prosjekter/page.tsx`, `app/dashbord/timer/mine/page.tsx`. Tom-state for sitedoc_admin med valgt firma og 0 prosjekter: «Ingen prosjekter for [firma]. Opprett et prosjekt for å komme i gang.» (1 ny i18n-nøkkel `dashbord.ingenProsjekterForFirmaBeskrivelse` i nb+en, action-knapp gjenbrukes). React Query auto-invaliderer ved firma-bytte — ingen manuell cache-rens. `pnpm --filter @sitedoc/api typecheck` + `pnpm build --filter @sitedoc/web` grønt (35s). Klar for test-deploy.
