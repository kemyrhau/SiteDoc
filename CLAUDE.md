...
**admin/prosjekter respekterer FirmaVelger IMPLEMENTERT på develop 2026-05-05.** Lukker inkonsistens der `/dashbord/admin/prosjekter` viste alle prosjekter på tvers av firmaer selv når sitedoc_admin hadde valgt et firma i FirmaVelger. Server: `admin.hentAlleProsjekter` får valgfri `organizationId`-input som filtrerer på `primaryOrganizationId` (samme mønster som `prosjekt.hentAlle` fra Blokk A 2026-05-04). Klient: `useFirma()` brukes til å sende `valgtFirma?.id`; header-tittel + empty-state-tekst dynamiske basert på valgt firma («Alle prosjekter» → firmanavn). 2 filer endret (~7 linjer), ingen migrasjon, ingen i18n. `pnpm --filter @sitedoc/api typecheck` + `pnpm build --filter @sitedoc/web` (32.7s) grønt. Klar for test-deploy.
