...
**Steg 1b Fase C (firma-kontekst — innstramning + Eier-firma-rename) IMPLEMENTERT på develop 2026-05-03:** Tredje og siste del av tre-fasers strategi. Gjør `organizationId` PÅKREVD på alle 9 router-filer (write-mutationer + skall-bare-relevante queries). Read-only katalogruter (`timer.{lonnsart,aktivitet,tillegg}.list`, `timer.onboarding.status`, `kompetansetype.hentAlle`) beholder valgfri `organizationId` med fallback siden de kalles fra prosjekt-baserte sider hvor ansatte skal se sitt eget firmas data uten å bytte. Helper-funksjonene `verifiserFirmaAdmin` (alle 9 filer) er forenklet til thin wrapper rundt `autoriserAdminForFirma` — fallback-grenen droppet. Klient-side: alle ~30 callsites endret fra `organizationId: orgId` til `organizationId: orgId!` (non-null assertion — etablert mønster siden `firma/layout.tsx` allerede gates på `valgtFirma`). Lag 3 — rename: `oppsett.firmainnstillinger` (sub-element under «Prosjekteier» i prosjekt-sidebar) → «Eier-firma» (nb) / «Owner company» (en). H1-overskrift på `/dashbord/oppsett/firma` byttet fra «Firmainnstillinger» til «Eier-firma». Annet: foreldrekategorien «Prosjekteier» (allerede etablert) beholdt; kun sub-elementets navn endret for å unngå redundans. Fix i `oppsett/firma/page.tsx`: `lagre()` har early-return ved `!organisasjon` før mutate (orgId må være string, ikke `string | undefined`). `pnpm typecheck` + `pnpm build` grønt. Klar for test-deploy. Fase C avslutter Steg 1b — ingen flere faser.
