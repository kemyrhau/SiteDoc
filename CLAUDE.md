...
**Steg 3 (maskin-import med firma-kontekst) IMPLEMENTERT på develop 2026-05-03.** 3a: Server-side erKunde-validering i `maskin.import.importerForhandsvisning` + `importerBekreft` via ny `krevErKundeFirma`-helper i `tilgangskontroll.ts` (FORBIDDEN hvis ikke kunde-firma). Klient-tom-state «Velg et kunde-firma fra toppmenyen» når sitedoc_admin ikke har valgt firma. 3b: Drag-and-drop på opplast-sone med `onDragOver`/`onDragLeave`/`onDrop`-handlere + visuell feedback (border + bg-farge endres ved drag-over). Klikk-funksjonalitet beholdt via label/input-mønster. 1 ny i18n-nøkkel `firma.maskin.import.velgFirma` (nb+en). `tsc` + `pnpm build` grønt. Klar for test-deploy.
