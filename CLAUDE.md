...
- **Commit + push:** Opus pusher sin egen feature-branch (`git push -u origin feat/x`) så snart egen gate er grønn (bygg + tester), uten å vente på klarsignal. Aldri `develop` eller `main`. Rebase på egen feature-branch pushes med `--force-with-lease` — aldri `--force`, og aldri mot `develop`/`main`. 🔴 Etter at «klar for merge» er gitt, er branchen FROSSET: ingen ny push uten å melde ny hash først. Merge til `develop` skjer bare etter at designgaten (der den gjelder) og coworks tekniske gate har gitt klarsignal. **Merge til `develop` og push av `develop` eies av kontroll-Claude/Kenneth** fra `SiteDoc-merge`; det er der merge-orden bestemmes. **Treet er detached og slettes lett — verifiser at det finnes (`git worktree list`) FØR du gir en merge-kommando; gjenopprettings-bash i [parallell-arbeid-lock.md § Gjenopprett merge-treet](docs/claude/parallell-arbeid-lock.md).** Kenneth koordinerer/relayer. Full rutine: [kontroll-claude-veileder.md § 10](docs/claude/kontroll-claude-veileder.md).
